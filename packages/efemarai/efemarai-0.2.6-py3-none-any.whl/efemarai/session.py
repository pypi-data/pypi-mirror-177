...
class Session:
    ...
    def _load_config_file(filename):
        ...
        for match in re.findall(r"\$\{([a-zA-Z]\w*)\}", contents):
            console.print(
    ...
    def _upload(self, from_url, endpoint, project_id):
        if os.path.isdir(from_url):
            self._upload_directory(from_url, endpoint, project_id)
        else:
            self._upload_file(from_url, endpoint, project_id)

    def _upload_file(self, from_url, endpoint, project_id):
    ...
    def _upload_directory(self, from_url, endpoint, project_id):
        filenames = [
            filename
