...
def __get_argparser() -> argparse.ArgumentParser:
    ...
    """
    env_args.add_argument("--staging", nargs='?', action=UniqueStore, const=Util.DEFAULT_STAGING_ENV,
                          help=argparse.SUPPRESS)
    env_args.add_argument("--cloud", nargs='?', action=UniqueStore, const=Util.DEFAULT_CLOUD_ENV,
                          help=argparse.SUPPRESS)
...
def get_args(args_list: Optional[List[str]] = None) -> Tuple[argparse.Namespace, Dict[str, Any]]:
    ...
    Util.reset_certora_internal_dir(args.build_dir)
    Util.safe_create_dir(Util.get_certora_internal_dir())
    ...
    if args.mode == Util.Mode.REPLAY:
        prepare_replay_mode(args)

    if args.mode == Util.Mode.CONF:
...
def setup_cache(args: argparse.Namespace) -> None:
    ...
    if not args.disable_auto_cache_key_gen and not os.environ.get("CERTORA_DISABLE_AUTO_CACHE") is not None:
        if args.mode == Util.Mode.VERIFY or args.mode == Util.Mode.ASSERT:
