...
def top1gating(
    ...
    #     me = torch.mean(gates, dim=0)
    #     ce = torch.mean(mask1.to(gates.dtype), dim=0)
    #     ####### Code for unused penalty #######
    #     # l_aux2 = 0
    #     # UNUSED_PENALTY_WEIGHT = 0
    #     ####### Code for unused penalty #######
    # else:
    #     me = torch.mean(gates_for_l_aux, dim=0)
    #     ce = torch.mean(mask1.to(gates_for_l_aux.dtype), dim=0)
    #     # ce = ce - (1.0 / float(num_experts))
    #     ####### Code for unused penalty #######
    #     # aux2_diff = ce - 1.0 / num_experts
    #     # aux2_mask = ce - 0.5 / num_experts < 0
    #     # l_aux2 = torch.mean(me * aux2_diff * aux2_mask.to(me.dtype))
    #     # UNUSED_PENALTY_WEIGHT = 100.0
    #     ####### Code for unused penalty #######

    l_aux = torch.mean(me * ce)
    ####### Code for unused penalty #######
    # l_aux = UNUSED_PENALTY_WEIGHT * l_aux2 + l_aux
    ####### Code for unused penalty #######
...
class Top1Gate(torch.nn.Module):
    ...
    def __init__(
        ...
        if not use_xmoe:
            ...
            # self.wg = torch.nn.Linear(16, num_experts, bias=False)
            ...
            # scaling rule: 0.3 / ln(#experts / 32)
            # self.gating_t = torch.nn.Parameter(torch.FloatTensor(1).fill_(np.log(1.0 / 0.2)))
            # self.gating_t = torch.nn.Parameter(torch.FloatTensor(1).fill_(np.log(1.0 / 0.07)))
