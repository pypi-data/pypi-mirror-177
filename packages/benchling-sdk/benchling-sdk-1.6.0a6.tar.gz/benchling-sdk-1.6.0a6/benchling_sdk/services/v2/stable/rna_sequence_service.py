...
class RnaSequenceService(BaseService):
    ...
    @api_method
    def get_by_id(self, rna_sequence_id: str) -> RnaSequence:
        """
        Get a RNA sequence.

        See https://benchling.com/api/reference#/RNA%20Sequences/getRNASequence
        """
        response = get_rna_sequence.sync_detailed(client=self.client, rna_sequence_id=rna_sequence_id)
    ...
    @api_method
    def bulk_get(self, rna_sequence_ids: Iterable[str]) -> Optional[List[RnaSequence]]:
        """
        ...
        response = bulk_get_rna_sequences.sync_detailed(
            client=self.client, rna_sequence_ids=rna_sequence_id_string
        )
        rna_sequences_results = model_from_detailed(response)
        return rna_sequences_results.rna_sequences

    @api_method
    def bulk_create(self, rna_sequences: Iterable[RnaSequenceBulkCreate]) -> AsyncTaskLink:
        """
    ...
    @api_method
    def bulk_update(self, rna_sequences: Iterable[RnaSequenceBulkUpdate]) -> AsyncTaskLink:
        """
    ...
    @api_method
    def autofill_parts(self, rna_sequence_ids: Iterable[str]) -> AsyncTaskLink:
        """
    ...
    @api_method
    def autofill_translations(self, rna_sequence_ids: Iterable[str]) -> AsyncTaskLink:
        """
    ...
    @api_method
    def auto_annotate(self, auto_annotate: AutoAnnotateRnaSequences) -> AsyncTaskLink:
        """
