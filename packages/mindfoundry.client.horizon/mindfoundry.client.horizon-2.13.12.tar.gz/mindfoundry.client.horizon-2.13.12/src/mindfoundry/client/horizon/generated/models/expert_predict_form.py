from io import BytesIO
from typing import Any, Dict, List, Type, TypeVar, Union

import attr

from ..models.regressor_type import RegressorType
from ..types import UNSET, File, FileJsonType, Unset

T = TypeVar("T", bound="ExpertPredictForm")


@attr.s(auto_attribs=True)
class ExpertPredictForm:
    """  """

    horizon: int
    regressor: RegressorType
    file: Union[Unset, File] = UNSET
    options: Union[Unset, str] = "{}"
    filter_target_names: Union[Unset, None, str] = UNSET
    additional_properties: Dict[str, Any] = attr.ib(init=False, factory=dict)

    def to_dict(self) -> Dict[str, Any]:
        horizon = self.horizon
        regressor = self.regressor.value

        file: Union[Unset, FileJsonType] = UNSET
        if not isinstance(self.file, Unset):
            file = self.file.to_tuple()

        options = self.options
        filter_target_names = self.filter_target_names

        field_dict: Dict[str, Any] = {}
        field_dict.update(self.additional_properties)
        field_dict.update(
            {
                "horizon": horizon,
                "regressor": regressor,
            }
        )
        if file is not UNSET:
            field_dict["file"] = file
        if options is not UNSET:
            field_dict["options"] = options
        if filter_target_names is not UNSET:
            field_dict["filterTargetNames"] = filter_target_names

        return field_dict

    def to_multipart(self) -> Dict[str, Any]:
        horizon = (
            self.horizon if self.horizon is UNSET else (None, str(self.horizon), "text/plain")
        )
        regressor = (None, str(self.regressor.value), "text/plain")

        file: Union[Unset, FileJsonType] = UNSET
        if not isinstance(self.file, Unset):
            file = self.file.to_tuple()

        options = (
            self.options if self.options is UNSET else (None, str(self.options), "text/plain")
        )
        filter_target_names = (
            self.filter_target_names
            if self.filter_target_names is UNSET
            else (None, str(self.filter_target_names), "text/plain")
        )

        field_dict: Dict[str, Any] = {}
        field_dict.update(
            {
                key: (None, str(value), "text/plain")
                for key, value in self.additional_properties.items()
            }
        )
        field_dict.update(
            {
                "horizon": horizon,
                "regressor": regressor,
            }
        )
        if file is not UNSET:
            field_dict["file"] = file
        if options is not UNSET:
            field_dict["options"] = options
        if filter_target_names is not UNSET:
            field_dict["filterTargetNames"] = filter_target_names

        return field_dict

    @classmethod
    def from_dict(cls: Type[T], src_dict: Dict[str, Any]) -> T:
        d = src_dict.copy()
        horizon = d.pop("horizon")

        regressor = RegressorType(d.pop("regressor"))

        _file = d.pop("file", UNSET)
        file: Union[Unset, File]
        if isinstance(_file, Unset):
            file = UNSET
        else:
            file = File(payload=BytesIO(_file))

        options = d.pop("options", UNSET)

        filter_target_names = d.pop("filterTargetNames", UNSET)

        expert_predict_form = cls(
            horizon=horizon,
            regressor=regressor,
            file=file,
            options=options,
            filter_target_names=filter_target_names,
        )

        expert_predict_form.additional_properties = d
        return expert_predict_form

    @property
    def additional_keys(self) -> List[str]:
        return list(self.additional_properties.keys())

    def __getitem__(self, key: str) -> Any:
        return self.additional_properties[key]

    def __setitem__(self, key: str, value: Any) -> None:
        self.additional_properties[key] = value

    def __delitem__(self, key: str) -> None:
        del self.additional_properties[key]

    def __contains__(self, key: str) -> bool:
        return key in self.additional_properties
