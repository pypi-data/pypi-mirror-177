...
def get_new_trajactory_len(buffers, env_id, ep_len, relabelling_strategy, q_net):
    ...
    hindsight_goal_id = original_trajectory.obs_next.hindsight_goal_id
    goal_id = original_trajectory.obs_next.goal_id
    assert len(np.unique(goal_id)) == 1
    hindsight_goal_id = np.unique(hindsight_goal_id[hindsight_goal_id >= 0])
    ...
    if relabelling_strategy == 'cher':
        len_hind = len(hindsight_goal_id)
        elem1 = goal_id[0]
        new_hindsight_goal_id = []
        for elem0 in hindsight_goal_id:
            if ((elem0, elem1) in q_net.adjacent_goal_id_pairs) or ((elem1, elem0) in q_net.adjacent_goal_id_pairs):
                new_hindsight_goal_id.append(elem0)
        hindsight_goal_id = copy.deepcopy(new_hindsight_goal_id)

        if len(hindsight_goal_id) == 0:
            return None, None

        # print(len(hindsight_goal_id), len_hind)

        # with torch.no_grad():
        #     z_hg, *_ = q_net.encode_goal_w_id(hindsight_goal_id)
        #     z_g, *_ = q_net.encode_goal_w_id(goal_id)
        #     d = torch.cdist(z_g, z_hg)
        #     indices = torch.topk(d, min(len(hindsight_goal_id), 3)).indices
        #     indices = indices.cpu().numpy()
        #     if len(indices)<len(hindsight_goal_id):
        #         print(len(indices), len(hindsight_goal_id))
        #     hindsight_goal_id = hindsight_goal_id[indices].reshape(-1,1)


    if relabelling_strategy == 'advantage':
        s0_id = original_trajectory.obs.state_id[0]
        achieved_goal_id_set_one_hot = original_trajectory.obs.achieved_goal_id_set_one_hot[0]
        s0_id = [s0_id] * len(hindsight_goal_id)
        achieved_goal_id_set_one_hot = [achieved_goal_id_set_one_hot] * len(hindsight_goal_id)
        obs = Batch({'state_id': s0_id, 'goal_id': hindsight_goal_id, 'achieved_goal_id_set_one_hot': achieved_goal_id_set_one_hot})
        with torch.no_grad():
            q, _ = q_net(obs, state=None, info=None)
            q = torch.amax(q, dim=-1).cpu().numpy()
    ...
    for index, hindsight_goal_id_i in enumerate(hindsight_goal_id):
        length_candidate = np.where(original_trajectory.obs_next.hindsight_goal_id == hindsight_goal_id_i)[0]
        if relabelling_strategy in ['min_length', 'cher']:
            length = np.amin(length_candidate)
        elif relabelling_strategy == 'random':
            length = random.choice(length_candidate)
        elif relabelling_strategy == 'advantage':
