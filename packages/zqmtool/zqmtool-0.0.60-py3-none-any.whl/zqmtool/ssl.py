...
def update_beta(method, beta, loss_infomin=None, acc=None, min_val=None):
    ...
    if method == 'infogoal':
        return add_beta(beta) if acc > 0.99 else reduce_beta(beta)
    elif method == 'infobot':
        return 0.1
    else:
        return beta
...
def draw_scatter_emb(net, save_path, output_for_goal_representation_learning, times, device,env, show_goal_id_set=None, enlarge_goal_id=None):
