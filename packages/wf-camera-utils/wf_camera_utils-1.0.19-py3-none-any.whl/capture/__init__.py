...
TELEGRAF_HOST = os.environ.get("TELEGRAF_HOST", 'localhost')


def emit(name, values, tags=None):
    client = TelegrafClient(host=TELEGRAF_HOST, port=8092)
...
def capture_loop():
    ...
    try:
        with picamera.PiCamera() as camera:
            ...
            emit("camera_capture", {"video_start": 1, "origin": 1, "split": 0})
            ...
            while True:
                ...
                emit("camera_capture", {"video_start": 1, "origin": 0, "split": 1})
...
def upload_loop():
    ...
    while True:
        ...
        if name:
            ...
            emit("camera_move", {"ffmpeg_time_seconds": dur, "moved_to_control": moved})
        else:
            time.sleep(2)
