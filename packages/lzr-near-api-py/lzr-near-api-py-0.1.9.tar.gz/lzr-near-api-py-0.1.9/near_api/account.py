...
class Account(object):
    ...
    def _sign_and_submit_tx_once(self, receiver_id: str, actions: list['transactions.Action'], new_nonce=None):
        if not new_nonce:
            self._access_key['nonce'] += 13
        else:
            self._access_key['nonce'] = new_nonce
    ...
    def _sign_and_submit_tx(self, receiver_id: str, actions: list['transactions.Action']) -> dict:
        attempt = 0
        new_nonce = self._access_key['nonce'] + 1
        while True:
            try:
                return self._sign_and_submit_tx_once(receiver_id, actions, new_nonce)
                ...
                if err.is_invalid_nonce_tx_error():
                    ...
                    self.fetch_state()
                    if self._access_key['nonce'] - new_nonce > 1:
                        new_nonce += 13
