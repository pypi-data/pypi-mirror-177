...
def searches_av():
    dirname = os.path.dirname(__file__)
    filename = os.path.join(dirname, 'searches_av.py')
    with open(filename) as hw:
        print(hw.read())

def theorytext():
    dirname = os.path.dirname(__file__)
    filename = os.path.join(dirname, 'AI_notes_text.txt')
    with open(filename) as hw:
        print(hw.read())

def knowledgeknights():
    dirname = os.path.dirname(__file__)
    filename = os.path.join(dirname, 'knowledgeknights.py')
    with open(filename,encoding="utf-8") as hw:
        print(hw.read())
