...
class JobClient:
    ...
    async def create(self, input: JobInput) -> ProcessingJob:
        """Create a new job.
    ...
    async def process(self, input: JobInput, *, max_time: float = float("inf")) -> JobWithResults:
        ...
        async def inner():
            fetched_job = await self.get(job["uuid"])
    ...
    async def get(self, uuid: str) -> Union[JobWithResults, ProcessingJob, DeletedJob]:
