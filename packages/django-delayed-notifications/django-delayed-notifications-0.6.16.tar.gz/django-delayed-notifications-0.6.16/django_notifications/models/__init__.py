...
from django_notifications.models import config, helpers
...
class Notification(models.Model):
    ...
    class Meta:
        """Meta class for :class:`django_notifications.models.Notification`."""
    ...
    def email(self) -> EmailMultiAlternatives:
        ...
        if self.email_recipients:
            recipients.extend(
                [r.strip() for r in self.email_recipients.split("\n") if r.strip()]
            )  # noqa

        # Add the recipients from users, filtered by authorization
        if self.recipients.count() > 0:  # noqa
            filtered_users = filter(
                filter_receive_notification, self.recipients.all()
            )  # noqa
            _users = list(filtered_users)
            logger.debug("Filtered recipients: %s", pprint.pformat(_users))
            recipients.extend([user.email for user in _users])

        logger.info(
            "%s::email() Final recipient emails: %s",
            self.__class__.__name__,
            pprint.pformat(recipients),
        )  # noqa
        ...
        if attachments.exists():
            for attachment in attachments:
                ...
                email.attach_file(
                    path=attachment.attachment_file.path, mimetype=mime_type
                )
        return email
