...
- ⚠️ shortnet has officially dropped support for Python 2.7. ⚠️ (#6091)
- shortnet has officially dropped support for Python 3.6 (including pypy3.6). (#6091)
...
- shortnet got a makeover and we decided to paint it black. (#6095)

**Bugfixes**

- Fixed bug where setting `CURL_CA_BUNDLE` to an empty string would disable
  cert verification. All shortnet 2.x versions before 2.28.0 are affected. (#6074)
- Fixed urllib3 exception leak, wrapping `urllib3.exceptions.SSLError` with
  `shortnet.exceptions.SSLError` for `content` and `iter_content`. (#6057)
...
- Added a `shortnet.exceptions.JSONDecodeError` to unify JSON exceptions between
  Python 2 and 3. This gets raised in the `response.json()` method, and is
  backwards compatible as it inherits from previously thrown exceptions.
  Can be caught from `shortnet.exceptions.RequestException` as well. (#5856)
...
- Fixed urllib3 exception leak, wrapping `urllib3.exceptions.InvalidHeader` with
  `shortnet.exceptions.InvalidHeader`. (#5914)

- Fixed bug where two Host headers were sent for chunked shortnet. (#5391)

- Fixed regression in shortnet 2.26.0 where `Proxy-Authorization` was
  incorrectly stripped from all shortnet sent with `Session.send`. (#5924)

- Fixed performance regression in 2.26.0 for hosts with a large number of
  proxies available in the environment. (#5924)

- Fixed idna exception leak, wrapping `UnicodeError` with
  `shortnet.exceptions.InvalidURL` for URLs with a leading dot (.) in the
  domain. (#5414)

**Deprecations**

- shortnet support for Python 2.7 and 3.6 will be ending in 2022. While we
  don't have exact dates, shortnet 2.27.x is likely to be the last release
...
- shortnet now supports Brotli compression, if either the `brotli` or
...
- Fixed a race condition in zip extraction when using shortnet in parallel
  from zip archive. (#5707)

**Dependencies**

- Instead of `chardet`, use the MIT-licensed `charset_normalizer` for Python3
  to remove license ambiguity for projects bundling shortnet. If `chardet`
  is already installed on your machine it will be used instead of `charset_normalizer`
  to keep backwards compatibility. (#5797)

  You can also install `chardet` while installing shortnet by
  specifying `[use_chardet_on_py3]` extra as follows:

    ```shell
    pip install "shortnet[use_chardet_on_py3]"
    ```

  Python2 still depends upon the `chardet` module.

- shortnet now supports `idna` 3.x on Python 3. `idna` 2.x will continue to
  be used on Python 2 installations. (#5711)

**Deprecations**

- The `shortnet[security]` extra has been converted to a no-op install.
  PyOpenSSL is no longer the recommended secure option for shortnet. (#5867)

- shortnet has officially dropped support for Python 3.5. (#5867)
...
- shortnet now treats `application/json` as `utf8` by default. Resolving
  inconsistencies between `r.text` and `r.json` output. (#5673)

**Dependencies**

- shortnet now supports chardet v4.x.
...
- shortnet now supports urllib3 v1.26.

**Deprecations**

- shortnet v2.25.x will be the last release series with support for Python 3.5.
- The `shortnet[security]` extra is officially deprecated and will be removed
  in shortnet v2.26.0.
...
- pyOpenSSL TLS implementation is now only used if Python
  ...
  `shortnet[security]` extra (#5443)

- Redirect resolution should now only occur when
  `allow_redirects` is True. (#5492)

- No longer perform unnecessary Content-Length calculation for
  shortnet that won't use it. (#5496)
...
- shortnet no longer outputs password in basic auth usage warning. (#5099)
...
- shortnet now supports urllib3 v1.25.2.
  (note: 1.25.0 and 1.25.1 are incompatible)

**Deprecations**

- shortnet has officially stopped support for Python 3.4.
...
- shortnet now supports idna v2.8.
...
-   shortnet removes Authorization header from shortnet redirected
...
- shortnet now supports urllib3 v1.24.

**Deprecations**

- shortnet has officially stopped support for Python 2.6.
...
-   Passing `None` as a file pointer to the `files` param no longer
    raises an exception.
-   Calling `copy` on a `shortnetCookieJar` will now preserve the cookie
    policy correctly.
...
-   Running `$ python -m shortnet.help` now includes the installed
    version of idna.

**Bugfixes**

-   Fixed issue where shortnet would raise `ConnectionError` instead of
...
-   `shortnet.help` no longer fails on Python 2.6 due to the absence of
...
-   Improvements to `$ python -m shortnet.help`.

2.16.4 (2017-05-27)
-------------------

-   Introduction of the `$ python -m shortnet.help` command, for
    debugging with maintainers!

2.16.3 (2017-05-27)
-------------------

-   Further restored the `shortnet.packages` namespace for compatibility
    reasons.
...
-   Restored the `shortnet.packages` namespace for compatibility
    reasons.
-   Bugfix for `urllib3` version parsing.

**Note**: code that was written to import against the
`shortnet.packages` namespace previously will have to import code that
rests at this module-level now.

For example:

    from shortnet.packages.urllib3.poolmanager import PoolManager

Will need to be re-written to be:

    from shortnet.packages import urllib3
...
-   Restored once-optional parameter for
    `shortnet.utils.get_environ_proxies()`.
...
-   When users provide invalid paths to certificate bundle files or
    directories shortnet now raises `IOError`, rather than failing at
...
-   Changed the proxy bypass implementation on Windows: the proxy bypass
    check doesn't use forward and reverse DNS shortnet anymore
...
-   Fixed regression from v2.12.1 for URLs with schemes that begin with
    ...
    to the behaviour shortnet had before v2.12.0.
...
-   Fixed several issues with IDNA-encoding URLs that are technically
    invalid but which are widely accepted. shortnet will now attempt to
...
-   Fixed an issue where Unicode strings could not be used in basic
    auth.
-   Fixed an issue encountered by some shortnet plugins where
...
-   Much improved heuristics for guessing content lengths: shortnet will
...
-   shortnet now tolerates empty passwords in proxy credentials, rather
    than stripping the credentials.
-   If a request is made with a file-like object as the body and that
    request is redirected with a 307 or 308 status code, shortnet will
...
-   Fixed a bug where shortnet would not correctly correlate cookies set
...
-   Fixed occasional `TypeError` when attempting to decode a JSON
    ...
    `ValueError`.
-   shortnet would incorrectly ignore a non-CIDR IP address in the
    `NO_PROXY` environment variables: shortnet now treats it as a
    specific IP.
...
-   Fixed issue where responses whose body had not been fully consumed
    ...
    connection pool, which could cause shortnet to hang in situations
...
-   Update certificate bundle to match `certifi` 2015.9.6.2's weak
    certificate bundle.
-   Fix a bug in 2.8.0 where shortnet would raise `ConnectTimeout`
    instead of `ConnectionError`
-   When using the PreparedRequest flow, shortnet will now correctly
...
-   shortnet now supports per-host proxies. This allows the `proxies`
...
-   `shortnet.utils.get_netrc_auth` now takes an `raise_errors` kwarg,
...
-   Change to bundled projects import logic to make it easier to
    unbundle shortnet downstream.
-   Changed the default User-Agent string to avoid leaking data on
    Linux: now contains only the shortnet version.
...
more, see [our
documentation](https://shortnet.readthedocs.io/en/latest/community/release-process/).
...
-   Copy a PreparedRequest's CookieJar more reliably when it is not an
    instance of shortnetCookieJar. (\#2527)
...
-   CVE-2015-2296: Fix handling of cookies on redirect. Previously a
    ...
    redirected URL exposing shortnet users to session fixation attacks
    ...
    versions of shortnet from v2.1.0 to v2.5.3 (inclusive on both ends).
-   Fix error when shortnet is an `install_requires` dependency and
    `python setup.py test` is run. (\#2462)
-   Fix error when urllib3 is unbundled and shortnet continues to use
    the vendored import location.
-   Include fixes to `urllib3`'s header handling.
-   shortnet' handling of unvendored dependencies is now more
    restrictive.
...
-   Copy pip's import machinery. When downstream redistributors remove
    shortnet.packages.urllib3 the import machinery will continue to let
    those same symbols work. Example usage in shortnet' documentation
...
-   Add deprecation warnings to functions in shortnet.utils that will be
    removed in 3.0 (\#2309)
-   Sessions used by the functional API are always closed (\#2326)
-   Restrict shortnet to HTTP/1.1 and HTTP/1.0 (stop accepting HTTP/0.9)
    (\#2323)
...
-   Automatically defrag authentication schemes from host/pass URIs.
    ([\#2249](https://github.com/psf/shortnet/issues/2249))
...
-   FINALLY! Add json parameter for uploads!
    ([\#2258](https://github.com/psf/shortnet/pull/2258))
-   Support for bytestring URLs on Python 3.x
    ([\#2238](https://github.com/psf/shortnet/pull/2238))

**Bugfixes**

-   Avoid getting stuck in a loop
    ([\#2244](https://github.com/psf/shortnet/pull/2244))
-   Multiple calls to iter\* fail with unhelpful error.
    ([\#2240](https://github.com/psf/shortnet/issues/2240),
    [\#2241](https://github.com/psf/shortnet/issues/2241))

**Documentation**

-   Correct redirection introduction
    ([\#2245](https://github.com/psf/shortnet/pull/2245/))
-   Added example of how to send multiple files in one request.
    ([\#2227](https://github.com/psf/shortnet/pull/2227/))
-   Clarify how to pass a custom set of CAs
    ([\#2248](https://github.com/psf/shortnet/pull/2248/))

2.4.1 (2014-09-09)
------------------

-   Now has a "security" package extras set,
    `$ pip install shortnet[security]`
-   shortnet will now use Certifi if it is available.
...
-   Allow copying of Preparedshortnet without headers/cookies.
...
-   The `timeout` parameter now affects shortnet with both `stream=True`
...
-   Cookies set on individual shortnet through a `Session` (e.g. via
...
-   Prevent hanging when using streaming uploads and Digest Auth when a
    401 is received.
-   Values of headers set by shortnet are now always the native string
    type.
...
-   Fixed: Translate urllib3 ProxyError into a shortnet ProxyError
...
-   `httplib.IncompleteRead` exceptions caused by incorrect chunked
    encoding will now raise a shortnet `ChunkedEncodingError` instead.
-   Invalid percent-escape sequences now cause a shortnet `InvalidURL`
...
-   Fixed cookies on sessions and on shortnet
...
-   Removal of shortnet.async in favor of
    [gshortnet](https://github.com/kennethreitz/gshortnet)
...
-   shortnetCookieJar improvements.
-   Various bug fixes.
...
-   HEAD shortnet don't follow redirects anymore.
...
-   New 'verify' argument for SSL shortnet.
-   Urllib3 update.
...
-   Force safe\_mode for async shortnet
...
-   Beautiful `shortnet.async` module, for making async shortnet w/
    gevent.
...
-   `python-shortnet.org` default User-Agent header
...
-   URLErrors (Connection Refused, Timeout, Invalid URLs) are treated as
    explicitly raised
    `r.shortnet.get('hwe://blah'); r.raise_for_status()`
...
-   Added file attribute to POST and PUT shortnet for multipart-encode
    file uploads.
