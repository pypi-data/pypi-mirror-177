import decimal
import sys
from typing import TypedDict, Callable, Union, Optional, Sequence, Any, TypeVar, Type
...
def get_or_error(db: Session, model: Type[T], primary_keys, options: Optional[Sequence[Any]] = None) -> T:
    instance: Optional[T] = db.get(model, primary_keys, options=options)
    if instance is None:
...
def load_from_model_query(model, filters: list[FieldFilter], limit: Union[int, None], offset: Union[int, None],
                          query_modifier: Optional[Callable] = None, init_query=None):
