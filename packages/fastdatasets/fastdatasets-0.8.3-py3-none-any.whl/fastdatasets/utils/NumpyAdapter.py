...
class NumpyWriterAdapter:
    def __init__(self,outfile:str,
        ...
        if self._backend == E_file_backend.record:
            ...
            self._f_writer = record_writer.NumpyWriter(outfile, options=options)
            ...
            self._f_writer = leveldb_writer.NumpyWriter(outfile, options=options)
            ...
            if options is None:
                options = LMDB.LmdbOptions(env_open_flag=0,
                                 ...
                                 dbi_flag=0,
                                 put_flag=0)
            self._f_writer = lmdb_writer.NumpyWriter(outfile, options=options,
            ...
            self._f_writer = memory_writer.NumpyWriter(outfile, options=options)
        elif self._backend == E_file_backend.memory_raw:
            self._kv_flag = False
            self._batch_buffer_num = 100000
            if options is None:
                options = MEMORY.MemoryOptions()
            self._f_writer = memory_writer.WriterObject(outfile, options=options)
        else:
    ...
    '''
    def get_output(self):
        if not self._f_writer:
            return None
        if self.backend == E_file_backend.memory or self.backend == E_file_backend.memory_raw:
           return self._f_writer.file_writer.data()
...
class NumpyReaderAdapter:
    @staticmethod
    def load(files: typing.Union[typing.List[str], str,typing.List[typing.Any]],
             backend: typing.Union[E_file_backend,str],
             options: typing.Union[RECORD.TFRecordOptions, LEVELDB.LeveldbOptions, LMDB.LmdbOptions,MEMORY.MemoryOptions] = None,
        ...
        if data_backend == E_file_backend.record:
            if options is None:
                options = RECORD.TFRecordOptions(compression_type='GZIP')
            dataset = record_loader.IterableDataset(files,cycle_length=cycle_length,block_length=block_length,options=options)
        elif data_backend == E_file_backend.leveldb:
            if options is None:
                options = LEVELDB.LeveldbOptions(create_if_missing=True,error_if_exists=False)
            dataset = leveldb_loader.RandomDataset(files,
            ...
            if options is None:
                options = LMDB.LmdbOptions(env_open_flag=LMDB.LmdbFlag.MDB_RDONLY,
                                                                         ...
                                                                         dbi_flag=0,
                                                                         put_flag=0)
            dataset = lmdb_loader.RandomDataset(files,
                                                data_key_prefix_list=data_key_prefix_list,
                                                num_key=num_key,
                                                options=options)
            ...
            if options is None:
                options = MEMORY.MemoryOptions()
            dataset = memory_loader.RandomDataset(files,options=options)
...
class ParallelNumpyWriter(ParallelStruct,metaclass=Final):
    def __init__(self,*args,**kwargs):
        # super(ParallelNumpyWriter, self).__init__(*args, **kwargs)
        ParallelStruct.__init__(self,*args, **kwargs)
        self.batch_keys = []
        self.batch_values = []
        self.total_num = 0
        self.numpy_writer = None
        self.input_hook_fn= None
        self.fn_args = None
        self.final_data = None

    def initailize_input_hook(self, input_hook_fn: typing.Callable,fn_args:typing.Tuple):
        self.input_hook_fn = input_hook_fn
        self.fn_args = fn_args

    def initialize_writer(self,outfile:str,
                 backend: typing.Union[E_file_backend,str],
                 options: typing.Union[RECORD.TFRecordOptions,LEVELDB.LeveldbOptions,LMDB.LmdbOptions,MEMORY.MemoryOptions]=None,
                 leveldb_write_buffer_size=1024 * 1024 * 512,
                 lmdb_map_size=1024 * 1024 * 1024 * 150):
    ...
    def parallel_apply(self,data,write_batch_size=None):
    ...
    def flush(self):
        if not self.is_kv_writer:
            if self.backend == E_file_backend.memory_raw:
                ret = self.numpy_writer.writer.write_batch([copy.deepcopy(d) for d in self.batch_values])
            else:
                ret = self.numpy_writer.writer.write_batch(self.batch_values)

        else:
            ret = self.numpy_writer.writer.put_batch(self.batch_keys,self.batch_values)
