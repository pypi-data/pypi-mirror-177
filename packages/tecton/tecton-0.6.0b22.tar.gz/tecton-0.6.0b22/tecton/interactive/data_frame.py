...
class TectonDataFrame(DataframeWrapper):
    ...
    @sdk_public_method
    def explain(self, extended: bool = False, show_node_ids: bool = True, show_columns: bool = False):
        """Prints the query plan.

        Args:
            extended: If True, the plan will be printed in a verbose fashion.
            show_node_ids: If True, the unique id associated with each node will be printed. Should only be set to True
                if this TectonDataFrame is backed by a query tree.
            show_columns: If True, the columns of each node will be printed. Should only be set to True if this
                TectonDataFrame s backed by a query tree.
        """
        if self._spark_df:
            self._spark_df.explain(extended=extended)
        elif self._querytree:
            print(self._querytree.pretty_str(verbose=extended, show_node_ids=show_node_ids, show_columns=show_columns))
    ...
    def _to_sql(self, create_temp_views: bool = False):
        if self._querytree is not None:
            if create_temp_views:
                recurse_query_tree(
                    self._querytree,
                    lambda node: node.data._register_temp_view() if isinstance(node, DataNode) else None,
                )
    ...
    def from_node_id(self, node_id: int) -> "TectonDataFrame":
