...
class Conv2d(NamedTuple):
    ...
    def fout_shape(self):
        """输出形状"""
        return self.batch, self.out_filter, self.fout_height, self.fout_width

    def gen_data(self, data_dtype="float32",
                 kernel_dtype="float32"):
        """模拟卷积的参数"""
        data_np = np.random.normal(size=(self.batch, self.in_filter,
                                   self.height, self.width)).astype(data_dtype)
        kernel_np = np.random.normal(size=(self.out_filter, self.in_filter,
