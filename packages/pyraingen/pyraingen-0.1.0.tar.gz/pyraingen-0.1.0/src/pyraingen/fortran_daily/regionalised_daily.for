...
c	supply values of some fixed parameters here
      ...
      Read(1,*)
      Read(1,*)
      read(1,*)idxt,tlat,tlon,tele,tdcoast,tanrf
      read(1,*)
      read(1,'(a)')fpath
      if(ival.gt.0)then
      read(1,*)
...
c	find nearest 'nstn' no of stations from the target location
      call station(kk,idr,tlat,tlon,tele,tdcoast,tanrf,idx,w,nstn,
     1             nsmax,nstnmax)
...
c	endif
 ...
 300  format(i6,a6,f7.3,2i6,2f8.2)
      ...
      if(cc(i:i).ne.' ')then
      ii=ii+1
      file_rf_out(ii:ii)=cc(i:i)
      endif
      enddo
      file_rf_out(ii+1:ii+1)='.'
      file_rf_out(ii+2:ii+2)='o'
      file_rf_out(ii+3:ii+3)='u'
      file_rf_out(ii+4:ii+4)='t'
...
c	read nearby station coordinates
      subroutine station(kk,idr,tlat,tlon,tele,tdcoast,tanrf,idx,w,
     1               nstn,nsmax,nstnmax)
      real*4 lat(nsmax),lon(nsmax),ele(nsmax),dcoast(nsmax),
     1       avrf(nsmax),w(nstnmax),aa(50),air(nsmax),dlatlon(nsmax)
      integer indx(nsmax),ir(nsmax),ilat(nsmax),ilon(nsmax),
     1     iele(nsmax),idcoast(nsmax),iavrf(nsmax),idx(nstnmax),
     2     idlatlon(nsmax),idr(nsmax)
      ...
      ii=0
      do i=1,50000
      read(1,10,end=11)i1,iy,id,tt,alt,aln,aele,adcoast,
     1    (aa(j),j=1,16),anrf
 10	  format(2i5,i10,a50,2f10.4,19f10.2)
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      subroutine rank(xc,xp,ixn,nn,nstn,nearmax,nstnmax)
...
c	if(nn(nk).gt.nearmax1)write(*,*)' increase dimension nearmax1'
 ...
 11	  continue
      ...
      call rank(xc,xp,ixn,nn,nstn,nearmax1,nstnmax)
      do nk=1,nstn
      do ii=1,nn(nk)
