    ...
    def set_possible_cns_domain(self):
        ...
        for event in self.result['events']:
            ...
            if event['tag'] in [TransactionType.ContractUpdated.name, 'Updated']:
                ...
                seller_, winner_, price_, seller_share, royalty_length_, royalties = self.cns_domain.finalizeEvent(event["events"][0])
                    else:
                        tag_, contract_index, contract_subindex, token_id_, \
                seller_, winner_, price_, seller_share, royalty_length_, royalties = self.cns_domain.finalizeEvent(event["message"])
                    self.cns_domain.tokenId = token_id_
                    self.cns_domain.amount = amount_  
                ...
                if event["receiveName"] == 'BictoryCnsNft.getTokenExpiry':
                    ...
                    if self.cns_domain.tokenId in self.concordium_node.cns_cache_by_token_id:
                        ...
                        console.log(f"Need to perform get_cns_domain_name for {self.cns_domain.tokenId}")
                        self.cns_domain.get_cns_domain_name(self.concordium_node, self.cns_domain.tokenId)
            ...
            if t.result['outcome'] == 'success':
                ...
                for event in events:
                    ...
                    if event['tag'] == 'DelegationSetDelegationTarget':
                        ...
                        message += f"Stake increased to: {(int(event['newStake'])/1_000_000):,.0f} CCD. "
                        request_target = True
                    
                    elif event['tag'] == 'DelegationStakeDecreased':
                        message += f"Stake decreased to: {(int(event['newStake'])/1_000_000):,.0f} CCD. "
                        request_target = True
