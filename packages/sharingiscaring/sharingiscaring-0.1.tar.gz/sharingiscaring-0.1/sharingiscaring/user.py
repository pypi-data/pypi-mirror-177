...
class User:
    ...
    def perform_subscription_logic(self, 
        ...
        explorer_ccd_transactions, _ = ccdscan.ql_get_all_transactions_for_exchange_graphs_for_account(None, SubscriptionDetails.EXPLORER_CCD)
            ...
            if concordium_tx.memo:
                if payment_memo in concordium_tx.memo:
                    ...
                    if not SUBSCRIPTION_ONE_TIME_FEE_set:
                        self.subscription.subscription_active = paid_amount >= SubscriptionDetails.SUBSCRIPTION_ONE_TIME_FEE
                        
                        if self.subscription.subscription_active:
                            SUBSCRIPTION_ONE_TIME_FEE_set = True
                            self.subscription.subscription_start_date = concordium_tx.block['blockSlotTime']

                    # Has the user paid enough for unlimited? If so, record the unlimited end date
                    self.subscription.unlimited = paid_amount >= SubscriptionDetails.SUBSCRIPTION_UNLIMITED
        ...
        if self.subscription.subscription_active:
            if (self.subscription.paid_amount - SubscriptionDetails.SUBSCRIPTION_ONE_TIME_FEE - self.subscription.count_messages) > 0:
