...
async def apply(
    hub, ctx, resource, *, old_value: int, new_value: int, comments: List[str]
) -> bool:
    """
    ...
    """
    result = True
    # The instance must be stopped to modify its partition number
    was_running = False
    ...
    # Modify the partition number
    ret = await hub.exec.boto3.client.ec2.modify_instance_placement(
        ctx,
        InstanceId=resource.id,
        PartitionNumber=new_value,
        Affinity=resource.placement["Affinity"],
    )
    if ret.comment:
        comments.append(ret.comment)
    result &= ret.result
