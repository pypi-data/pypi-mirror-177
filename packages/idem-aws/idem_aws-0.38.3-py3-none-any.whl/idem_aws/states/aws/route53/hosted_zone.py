...
async def search(
    ...
    This function has been deprecated. Please use exec.run with aws.route53.hosted_zone.get instead.
    ...
    Args:
        ...
        resource_id(str, Optional):
            AWS hosted_zone id to filter the resource.If resource_id is mentioned then we directly return the resource
            without checking other filters
    ...
    if resource_id:
        ...
        ret = await hub.exec.aws.route53.hosted_zone.get(
            ctx, name=name, resource_id=resource_id
        )
        if not ret["result"]:
            result["result"] = ret["result"]
        ...
        describe_ret = (
            await hub.tool.aws.route53.hosted_zone_utils.get_all_hosted_zones(ctx)
        )
        ...
        ret = hub.tool.aws.route53.hosted_zone_utils.get_hosted_zones_with_filters(
        ...
        # For backward compatibility since search returns a single object
        hosted_zone = ret["ret"][0]
