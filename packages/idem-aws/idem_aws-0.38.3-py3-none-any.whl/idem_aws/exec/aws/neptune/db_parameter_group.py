...
async def get(
    ...
    result[
        "ret"
    ] = await hub.tool.aws.neptune.conversion_utils.convert_raw_db_parameter_group_to_present(
...
async def list_(hub, ctx, name) -> Dict:
    ...
    ret = await hub.tool.aws.neptune.db_parameter_group.search_raw(ctx=ctx, name=name)
    if not ret["result"]:
        result["comment"] += list(ret["comment"])
        result["result"] = False
        return result
