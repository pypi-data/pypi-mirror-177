...
from stratosphere.store.db.database import (
    Database,
    next_ulid,
    pandas_query,
    sanitize_table_name,
...
class Experiment:
    ...
    def __init__(
        ...
        self.table_name = sanitize_table_name(
            f"{config.db.experiment_table_prefix}{self.name}"
        )
    ...
    def query(self, q: str) -> pd.DataFrame:
        ...
        Args:
            q (str): SQL query. "{e}" occurrences are substituted with the table name, if present.

        Returns:
            pd.DataFrame: Result of the query.
        """

        return self.db.pandas(q.format(e=self.table_name))
