...
class Experiment:
    ...
    def query(self, query: str, verbose: bool = False) -> pd.DataFrame:
        ...
        Args:
            query (str): SQL query. "{e}" occurrences are substituted with the table name
            (prefixed with "e_" by default) and "{ee}" occurrences are substituted with
            the experiments table ("experiments" by default).
        ...
        return self.db.pandas(query.format(e=self.table_name, ee=config.db.experiments_table), verbose=verbose)
    ...
    def load(cls, db: Database, name: str):
        ...
        with db.session() as session:
            ...
            try:
                ...
                logging.warning("Unpicking failed, only the SQL interface is available.")
                logging.warning("Persisting this object will overwrite the existing one.")
                experiment = Experiment(db=db, name=name)
                return experiment
