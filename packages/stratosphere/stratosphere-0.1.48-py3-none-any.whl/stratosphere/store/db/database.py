    ...
    def pandas_to_sql(self, df: pd.DataFrame, name: str, if_exists: IfExists):
        ...
        with self.session() as session:
            if config.tqdm.enable:
                ...
                df.to_sql(f"experiment.{name}", session.bind, if_exists=if_exists, index=False)
...
def pandas_query(
    ...
    if use_tqdm:
        ...
        df_chunks = pd.read_sql(query, session.bind, chunksize=config.db.query_read_chunk_size)
