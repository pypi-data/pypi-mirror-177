...
class FileUpload:
    ...
    def split_object_key_list(self, sub_size):
        sub_list = [self.key_name_array[i:i + sub_size] for i in range(0, len(self.key_name_array), sub_size)]
        return sub_list
    ...
    def file_upload_initiate(self, path, collection_type, collection_name, meta_data_object, override):
        ...
        sub_list = self.split_object_key_list(SUB_FILE_LENGTH)
        ...
        if len(self.fail_upload_array) != 0:
            ...
            for file in self.fail_upload_array:
                print(f'{file}')
