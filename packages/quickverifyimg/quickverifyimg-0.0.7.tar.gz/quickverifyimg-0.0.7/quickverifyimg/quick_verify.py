...
class QuickVerify():
    ...
    def verify_img(self, img_list, verify_img_index, verify_type=1,compare_threshold=0.99):
        ...
        for img_path in img_list:
            ...
            for index, verify_img in enumerate(verify_images):
                ...
                if is_similiar:
                    logger.debug(f"{img_path} match {verify_images_name[index]}, similiar: {similiar} , engine_type :{verify_type}")
    ...
    def mutliple_engine_verify(self, img_dir, verify_engine_list, match_rate_threshold):
        ...
        logger.info(f"匹配算法和阈值列表：{verify_engine_list}")
        ...
        for t in thread_list:
            ...
            if verify_result['result']:
                ...
                logger.info(f"算法线程{index}结果：{verify_result}")
                # 取非背景图交集
        ...
        final_rate = (len(available_screenshot_set) - len(verify_fail_screenshots_set))/len(available_screenshot_set)
        ret = {
