...
from AutoWSGR.game.game_operation import QuickRepair, GetShip
from AutoWSGR.game.get_game_info import DetectShipStatus
...
class _Logit():
    ...
    def __init__(self, status: DecisiveStatus, level1: list, level2: list, flagship_priority: list):
        pass
...
class Logic(_Logit):
    ...
    def __init__(self, timer, status: DecisiveStatus, level1: list, level2: list, flagship_priority: list):
        self.timer = timer
    ...
    def _choose_ship(self, must=False):
        ...
        if (self.status.fleet.count() <= 1):
            choose = self.level1
        elif (self.status.fleet.count() < 6):
        ...
        for target in choose:
            if (target in self.status.selections.keys()):
                cost = self.status.selections[target][0]
                if (score >= cost and cost <= lim):
                    score -= cost
                    result.append(target)
        if not result and must:
            result.append(list(self.status.selections.keys())[0])
        return result

    def _use_skill(self):
        return 3 if (self.status.node == 'A') else 0

    def need_repair(self):
        return bool((1 in self.status.ship_status or 2 in self.status.ship_status))

    def _up_level(self):
        return bool((self.status.need - self.status.exp <= 5 and self.status.score >= 5))
...
class DecisiveBattle():
    ...
    def __init__(self, timer: Timer, chapter=6, map=1, node='A', version=3,
        ...
        self.status = DecisiveStatus(timer, chapter, map, node, version)
        if logic is None:
            self.logic = Logic(self.timer, self.status, level1, level2, flagship_priority)
        self.__dict__.update(kwargs)
    ...
    def next(self):
        res = self.status.next()
        if res in ['next', 'quit']:
            self.timer.ConfirmOperation()
            self.timer.ConfirmOperation()
            GetShip(self.timer, 5)
        return res
    ...
    def choose(self, refreshed=False):
        ...
        # 应该能保证数字读取成功...
        self.status.score = int(recognize_number(screen[25:55, 1162:1245], min_size=5, text_threshold=.05, low_text=.02)[0][1])
        costs = recognize_number(screen[550:585, 245:1031], 'x')
        _costs, ships, real_position = [], [], []
        for i, cost in enumerate(costs):
            if (int(cost[1][1:]) > self.status.score):
                continue
        ...
        # ==================做出决策===================
        self.status.selections = selections
        choose = self.logic._choose_ship(must=(self.status.map == 1 and self.status.node == 'A' and refreshed == True))
        ...
        for target in choose:
            ...
            self.status.score -= cost
            self.timer.Android.click(*p)
            if (is_ship(target)):
                self.status.ships.add(target)
    ...
    def move_chapter(self):
        if (self.get_chapter() < self.status.chapter):
            self.timer.Android.click(900, 507)
        elif (self.get_chapter() > self.status.chapter):
    ...
    def get_exp(self):
        ...
        try:
            i1 = src.index('(')
            i2 = src.index('/')
            self.status.exp = int(src[i1 + 1:i2])
            self.status.need = int(src[i2 + 1:-1])
        except:
            pass
    ...
    def before_fight(self):
        ...
        if (self.status.fleet.empty() and not self.status.is_begin()):
            self.check_fleet()
        ...
        if (self.logic._leave()):
            self.leave()
            return 'leave'
        if (self.status.fleet != _fleet):
            self.change_fleet(_fleet)
            self.status.ship_status = DetectShipStatus(self.timer)
        if (self.logic.need_repair()):
            self.repair()

    def after_fight(self):
        self.status.ship_status = self.timer.ship_status
        print(self.status.ship_status)

    def check_fleet(self):
        self.go_fleet_page()
        self.status.fleet.detect()
        for ship in self.status.fleet.ships:
            self.status.ships.add(ship)
        self.status.ship_status = DetectShipStatus(self.timer)

    def during_fight(self):
        formation = get_formation(self.status.fleet, self.status.enemy_now)
        night = self.status.node in self.status.key_points[self.status.chapter][self.status.map]
        DecisiveBattlePlan(self.timer, decision_block=DB(formation=formation, night=night)).run()

    def change_fleet(self, fleet):
        self.go_fleet_page()
        self.status.fleet.set_ship(fleet, order=True, search_method=None)

    def fight(self):
        try:
            res = self.before_fight()
        except (Exception, BaseException) as e:
            if self.status.map == 1 and self.status.node == 'A':
