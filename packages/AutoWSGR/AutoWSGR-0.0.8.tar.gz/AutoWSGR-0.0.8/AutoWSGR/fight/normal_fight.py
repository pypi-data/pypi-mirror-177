...
from AutoWSGR.game.get_game_info import DetectShipStatus, GetEnemyCondition
...
class NormalFightInfo(FightInfo):
    ...
    def _after_match(self):
        ...
        if self.state == "spot_enemy_success":
            ...
            DetectShipStatus(self.timer, 'sumup')
    ...
    def _move_chapter(self, target, chapter_now=None):
        ...
        try:
            ...
            if not self.timer.wait_image(IMG.chapter_image[chapter_now]):
                raise ImageNotFoundErr("after move_chapter operation but the chapter do not move")
            ...
            if target > NowNode:
                for i in range(1, target - NowNode + 1):
                    ...
                    if (not self.verify_node(NowNode + i, chapter, timeout=4)):
                        raise ImageNotFoundErr("after move_node operation but the node does not move")
                    time.sleep(0.15)
            else:
