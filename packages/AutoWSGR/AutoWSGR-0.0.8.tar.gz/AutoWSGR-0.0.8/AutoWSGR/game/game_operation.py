...
from AutoWSGR.constants.positions import BLOOD_LIST_POSITION
from AutoWSGR.controller.run_timer import Timer
from AutoWSGR.utils.api_image import relative_to_absolute
# from AutoWSGR.utils.logger import logit

from .get_game_info import CheckSupportStatus, DetectShipStatus
...
class Expedition:
    ...
    def update(self, force=False):
        ...
        if (isinstance(self.timer.now_page, str) and 'unknown' in self.timer.now_page)\
...
# @logit(level=INFO2)
def GetShip(timer: Timer, max_times=1):
    times = 0
    timeout = 5
...
def DestroyShip(timer: Timer):
    ...
    timer.wait_image(IMG.symbol_image[5])
    timer.Android.click(301, 25)  # 进入解装页面

    # timer.wait_image(IMG.symbol_image[6], after_get_delay=.33)
    timer.Android.click(90, 206, delay=3)  # 点添加

    # timer.wait_image(IMG.symbol_image[11], confidence=0.75)
    x, y = relative_to_absolute((0.4, -0.2))
    timer.Android.click(x, y)  # 快速选择
    timer.Android.click(860, 480)  # 确认

    # if (timer.image_exist(IMG.game_ui[8])):
    timer.Android.click(807, 346)  # 卸下装备

    timer.Android.click(870, 480)  # 解装
    timer.Android.click(364, 304)  # 确认


def VerifyTeam(timer: Timer):
    """检验目前是哪一个队伍(1~4)
    含网络状况处理
    Args:
...
def QuickRepair(timer: Timer, repair_mode=2, *args, **kwargs):
    ...
    Args:
        ...
        repair_mode:
            ...
            2: 快修，修大破
    """
    ShipStatus = DetectShipStatus(timer)
    ...
    if any(need_repair) or timer.image_exist(IMG.repair_image[1]):
        ...
        for i in range(1, 7):
            if need_repair[i-1]:
                ...
                timer.Android.click(BLOOD_LIST_POSITION[0][i][0], BLOOD_LIST_POSITION[0][i][1], delay=1.5)
...
# @logit(level=INFO2)
def Supply(timer: Timer, List=None, try_times=0):
    """补给指定舰船
...
# @logit(level=INFO2)
def ChangeShip(timer: Timer, team, pos=None, name=None, pre=None, detect_ship_status=True):
    """切换舰船(不支持第一舰队)
    ...
    # 切换单船
    # 懒得做 OCR 所以默认第一个
    if (detect_ship_status):
        DetectShipStatus(timer)
...
# @logit(level=INFO3)
def ChangeShips(timer: Timer, team, ship_list):
    """更换编队舰船
    ...
    for i in range(1, 7):
        if (timer.ship_status[i] != -1):
            ChangeShip(timer, team, 1, None, detect_ship_status=False)
    ship_list = ship_list + [None] * 6
    time.sleep(.3)
    DetectShipStatus(timer)
    for i in range(1, 7):
        ChangeShip(timer, team, i, ship_list[i], detect_ship_status=False)


def GetNewThings(timer: Timer, lock=0):
    pass
