...
from AutoWSGR.constants.positions import BLOOD_LIST_POSITION, TYPE_SCAN_AREA
...
# @logit(level=INFO1)
def DetectShipStatus(timer: Timer, type='prepare'):
    ...
    for i in range(1, 7):
        if type == 'prepare':
            pixel=timer.get_pixel(*BLOOD_LIST_POSITION[0][i])
            ...
            if timer.ship_status[i] == -1:
                result[i]=-1
                continue
            pixel=timer.get_pixel(*BLOOD_LIST_POSITION[1][i])
...
#@logit(level=INFO1)
def CheckSupportStatus(timer: Timer):
