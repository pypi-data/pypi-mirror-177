...
from AutoWSGR.game.game_operation import GetShip
...
class FightResult():
    ...
    def detect_result(self):
        ...
        self.mvp = get_nearest((mvp_pos[0], mvp_pos[1] + 20), BLOOD_LIST_POSITION[1])
...
class FightPlan(ABC):
    ...
    def run(self, same_work=False):
        ...
        # 战斗前逻辑
        enter_ret = self._enter_fight(same_work)
        if enter_ret == "success":
            # 战斗中逻辑
            return self._fight()
        elif enter_ret in ["dock is full", "out of times"]:
            return enter_ret
        else:
            self.logger.error(f"未知状态: {enter_ret}  屏幕状态已记录")
            self.timer.log_screen()
