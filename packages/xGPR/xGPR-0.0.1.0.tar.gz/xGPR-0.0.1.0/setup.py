...
def get_conjugate_grad_extensions(setup_fpath):
    ...
    gradient implementations. Note that this
    function may not necessarily be used -- Scipy's
    CG implementation is pretty good so it MAY not
    necessary that we have to use our own. We
    built our own in case we needed to do something
    harder to do with Scipy (e.g. simultaneously
    approximating a logdet using SLQ, multiple
    regression objectives, etc.)"""
    cpu_cg_path = os.path.join(setup_fpath, "xGPR",
                "cg_toolkit", "cpu_cg.pyx")
    gpu_cg_path = os.path.join(setup_fpath, "xGPR",
                "cg_toolkit", "gpu_cg.pyx")
    cpu_cg_ext = Extension("cpu_cg",
                sources=[cpu_cg_path],
                language="c",
                include_dirs = [numpy.get_include()])
    gpu_cg_ext = Extension("gpu_cg",
                sources=[gpu_cg_path],
                language="c",
                include_dirs = [numpy.get_include()])
    return [cpu_cg_ext, gpu_cg_ext]
...
def setup_cuda_fast_hadamard_extensions(setup_fpath, CUDA_PATH, NO_CUDA = False):
    ...
    if "libarray_operations.a" not in os.listdir():
        ...
        cuda_hadamard_basic_ext = Extension("cuda_basic_hadamard_operations",
                ...
                language="c++",
                libraries=["array_operations", "cudart", "cuda"],
...
def main():
    ...
    cg_ext = get_conjugate_grad_extensions(setup_fpath)
    kernel_tools_ext = get_kernel_tools_extensions(setup_fpath)
    #######
    #sgd_ext = get_sgd_grad_extensions(setup_fpath)
    #######
    cpu_fht_ext, cpu_fht_files = setup_cpu_fast_hadamard_extensions(setup_fpath)
    fht_cuda_ext, cuda_build_failure, gpu_fht_files = setup_cuda_fast_hadamard_extensions(setup_fpath,
                                            CUDA_PATH, NO_CUDA)

    ext_modules = cg_ext + cpu_fht_ext + fht_cuda_ext + kernel_tools_ext# + sgd_ext
    ...
    setup(
        ...
        install_requires=["numpy>=1.21", "scipy>=1.7.3"],
    ...
    if "cpu_cg.c" in os.listdir():
        os.remove("cpu_cg.c")
    if "gpu_cg.c" in os.listdir():
        os.remove("gpu_cg.c")
