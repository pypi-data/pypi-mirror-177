...
class FastConv1d:
    ...
    def __init__(self, seq_width, device = "cpu",
            kernel_type = "mm_maxpool", random_seed = 123,
        ...
        Args:
            ...
            kernel_type (str): Must be one of ['mm_maxpool', 'fht_maxpool'].
                The former uses matrix multiplication based convolution
                with random values from a standard normal, whereas the
                latter uses orthogonal random features.
            ...
            mode (str): One of 'maxpool', 'maxpool_loc', 'twolayer'.
        ...
        if kernel_type == "mm_maxpool":
            self.conv_kernel = [MMConv1dFeatureExtractor(seq_width,
                            self.f_per_kernel, random_seed, device = device,
                            conv_width = c, mode = mode) for c in conv_width]

        elif kernel_type == "fht_maxpool":
            self.conv_kernel = [FHTMaxpoolConv1dFeatureExtractor(seq_width,
                            self.f_per_kernel, random_seed, device = device,
                            conv_width = c, mode = mode) for c in conv_width]
        else:
            raise ValueError("Unrecognized kernel supplied.")
        self.device = device
