...
* (SORF) operation on an input 3d array of doubles on GPU. The input array should
* already live on GPU.
*/
...
extern "C" const char *doubleCudaSORF3d_(double *cArray, int8_t *radem,
    ...
    //First H-transform.
    doubleCudaHTransform3d_(cArray, dim0, dim1, dim2);
...
//
//Note that all of these arrays are already expected to "live" on GPU.
extern "C" const char *doubleCudaSRHT2d_(double *cArray, int8_t *radem,
                int dim0, int dim1){
    ...
    //First H-transform.
    doubleCudaHTransform2d_(cArray, dim0, dim1);
...
//be a power of 2 (caller must verify).
void doubleCudaHTransform2d_(double *cArray, int dim0, int dim1){
