...
cdef extern from "poly_fht_operations.h" nogil:
...
def floatCpuConv1dTransform(np.ndarray[np.float32_t, ndim=3] reshapedX,
                ...
                str mode):
    """Uses the wrapped floatConv1dPrep_ and numpy operations to perform a
    ...
    Note that floatConv1dPrep should ONLY be accessed through this
    ...
    if mode == "conv_gradient":
        gradient = np.zeros((Z.shape[0], Z.shape[1]))
...
@cython.boundscheck(False)
@cython.wraparound(False)
def floatCpuGraphConv1dTransform(np.ndarray[np.float32_t, ndim=3] reshapedX,
                np.ndarray[np.int8_t, ndim=3] radem,
                np.ndarray[np.float64_t, ndim=2] Z,
                np.ndarray[np.float32_t, ndim=1] S,
                int numThreads,
    ...
    of a graph. Has the capability to use different lengthscales for
    different 'segments' of each node vector (up to 3).
    ...
    Returns:
        gradient (np.ndarray); An array of shape output.shape[0] x output.shape[1] x sigmaValues.shape[0].
    ...
    scalingTerm = np.sqrt(1 / <float>radem.shape[2])
    if mode == "gradient":
        gradient = np.zeros((Z.shape[0], Z.shape[1], sigmaValues.shape[0]))
    ...
    for i in range(num_repeats):
        ...
        if mode == "gradient":
            if splitPoints.shape[0] == 2:
                gradient[:,startPosition:cutoff,j] = (-np.sin(reshapedXCopy) * reshapedXCopy /
                                            sigmaValues[0]).sum(axis=1)
                gradient[:,startPos2:cutoff2,j] = (np.cos(reshapedXCopy) * reshapedXCopy /
                                        sigmaValues[0]).sum(axis=1)
            else:
                for j in range(0, splitPoints.shape[0] - 1):
                    featureMod[:] = 0
                    ...
                    errCode = floatConv1dPrep_(&radem[0,0,0], &featureMod[0,0,0],
                        numThreads, reshapedX.shape[0], reshapedX.shape[1], 
                        reshapedX.shape[2], i * reshapedX.shape[2], radem.shape[2])
                    if errCode.decode("UTF-8") != "no_error":
                        raise Exception("Fatal error encountered while performing graph convolution.")
                    featureMod *= S[None,None,(i * reshapedX.shape[2]):((i+1) * reshapedX.shape[2])]
                    gradient[:,startPosition:cutoff,j] = (-np.sin(reshapedXCopy) * featureMod).sum(axis=1)
                    gradient[:,startPos2:cutoff2,j] = (np.cos(reshapedXCopy) * featureMod).sum(axis=1)
...
def floatCpuGraphPolyFHT(np.ndarray[np.float32_t, ndim=3] reshapedX,
    ...
    for i in range(num_repeats):
        ...
        errCode = floatPolyFHTPrep_(&radem[0,0,0], &preSumFeats[0,0,0],
                    numThreads, reshapedX.shape[0], reshapedX.shape[1], 
        ...
        for j in range(1, polydegree):
            reshapedXCopy[:] = reshapedX
            errCode = floatPolyFHTPrep_(&radem[0,0,0], &reshapedXCopy[0,0,0],
                    numThreads, reshapedX.shape[0], reshapedX.shape[1], 
