        ...
        if mode == "gradient":
            if splitPoints.shape[0] == 2:
                ...
                for j in range(0, splitPoints.shape[0] - 1):
                    ...
                    errCode = doubleConv1dPrep_(&radem[0,0,0], &featureMod[0,0,0],
...
def doubleCpuGraphPolyFHT(np.ndarray[np.float64_t, ndim=3] reshapedX,
    ...
    for i in range(num_repeats):
        ...
        errCode = doublePolyFHTPrep_(&radem[0,0,0], &preSumFeats[0,0,0],
                    numThreads, reshapedX.shape[0], reshapedX.shape[1], 
        ...
        for j in range(1, polydegree):
            reshapedXCopy[:] = reshapedX
            errCode = doublePolyFHTPrep_(&radem[0,0,0], &reshapedXCopy[0,0,0],
                    numThreads, reshapedX.shape[0], reshapedX.shape[1], 
