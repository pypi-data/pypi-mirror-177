...
def gpu_cho_calcs(z_trans_z, z_trans_y, lambda_):
    ...
    Returns:
        ...
        inner_deriv: A cupy array containing (dz_dsigma^T z +
    ...
    inner_deriv = gpu_cho_solver(z_trans_z_chol, z_trans_z)
    chol_inv = cpx.scipy.linalg.solve_triangular(z_trans_z_chol,
                    cp.eye(z_trans_z_chol.shape[0]), lower=True)
    return weights, z_trans_z_chol, inner_deriv, chol_inv
...
def cpu_cho_calcs(z_trans_z, z_trans_y, lambda_):
    ...
    Returns:
        ...
        z_trans_z_chol: A numpy array storing the cholesky decomposition
            of z^T z. Shape is (self.num_rffs, self.num_rffs).
        inner_deriv: A numpy array containing (dz_dsigma^T z +
            z^T dz_dsigma). Shape is (self.num_rffs, self.num_rffs, M)
            for M kernel-specific hyperparameters.
    ...
    z_trans_z.flat[::z_trans_z.shape[0]+1] -= lambda_**2
    inner_deriv = cho_solve((z_trans_z_chol, True), z_trans_z)
