...
class GraphPolySum(KernelBaseclass):
    ...
    Attributes:
        hyperparams (np.ndarray): A length two array of two
    ...
    def __init__(self, xdim, num_rffs, random_seed = 123,
        ...
        bounds = np.asarray([[1e-3,1e1], [0.2, 5]])
        ...
        self.radem_diag = rng.choice(radem_array, size=(3 * self.polydegree,
                                ...
                                replace=True)
        self.chi_arr = chi.rvs(df=self.padded_dims, size=(self.polydegree,
                            self.init_calc_freqsize),
    ...
    def pretransform_x(self, input_x):
        ...
        random feature generation, here called 'pretransform'.
        ...
        return output_x[:,:self.num_rffs]


    def finish_transform(self, input_x, multiply_by_beta = True):
        """For most kernels, random feature generation has two steps
        (pretransform and finish_transform). For the GraphPoly kernel,
        we merely need to multiply by the beta hyperparameter --
        everything else is done as part of pretransformation.
        """
        xtrans = input_x
        if multiply_by_beta:
            xtrans = self.hyperparams[1] * xtrans
        return xtrans
    ...
    def kernel_specific_gradient(self, input_x):
        ...
        xtrans = self.finish_transform(xtrans)
