...
class MMConv1dFeatureExtractor():
    ...
    extractor below, which uses an FHT-based procedure.)
    ...
    def __init__(self, seqwidth, num_rffs, random_seed = 123, device = "cpu",
        ...
        """
        if mode not in ["maxpool_loc", "maxpool", "max_std", "twolayer"]:
        ...
        if self.mode not in ["max_std", "twolayer"]:
            self.num_freqs = num_rffs
        else:
            if num_rffs % 2 != 0:
                raise ValueError("For the max_std feature extraction mode, "
                        "the number of requested features must be a multiple of 2.")
