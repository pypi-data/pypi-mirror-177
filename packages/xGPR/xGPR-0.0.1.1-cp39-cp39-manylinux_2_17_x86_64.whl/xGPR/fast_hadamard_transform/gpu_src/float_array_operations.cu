...
extern "C" const char *floatCudaSORF3d_(float *cArray, int8_t *radem,
    ...
    //First H-transform.
    floatCudaHTransform3d_(cArray, dim0, dim1, dim2);
...
//
//Note that all of these arrays are already expected to "live" on GPU.
extern "C" const char *floatCudaSRHT2d_(float *cArray, int8_t *radem,
                int dim0, int dim1){
    ...
    //First H-transform.
    floatCudaHTransform2d_(cArray, dim0, dim1);
