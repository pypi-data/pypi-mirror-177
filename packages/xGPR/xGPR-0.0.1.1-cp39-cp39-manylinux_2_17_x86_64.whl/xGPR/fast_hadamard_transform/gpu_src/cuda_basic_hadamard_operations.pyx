...
cdef extern from "float_array_operations.h" nogil:
    const char *floatCudaSORF3d_(float *npArray, np.int8_t *radem, 
                    int dim0, int dim1, int dim2)
    const char *floatCudaSRHT2d_(float *npArray, 
                    int8_t *radem, int dim0, int dim1);
cdef extern from "double_array_operations.h" nogil:
    const char *doubleCudaSORF3d_(double *npArray, np.int8_t *radem, 
                    int dim0, int dim1, int dim2)
    const char *doubleCudaSRHT2d_(double *npArray, 
                    int8_t *radem, int dim0, int dim1);
...
def floatCudaPySORFTransform(cpArray, radem, int numThreads):
    ...
    by wrapping floatCudaSORF3d_, when the input array is an array
    of floats.
    Note that floatCudaSORF3d_ should ONLY be accessed through this wrapper
    ...
    errCode = floatCudaSORF3d_(cArray, radem_ptr,
                cpArray.shape[0], cpArray.shape[1],
                cpArray.shape[2])
    if errCode.decode("UTF-8") != "no_error":
        raise Exception("Fatal error encountered in floatCudaSORF3d_.")
...
def doubleCudaPySORFTransform(cpArray, radem, int numThreads):
    """This function performs the calculation of the structured
    orthogonal features or SORF approach to random Fourier features
    by wrapping doubleCudaSORF3d_, when the input array is an array of
    doubles.
    Note that doubleCudaSORF3d_ should ONLY be accessed through this wrapper
    since this wrapper performs key checks (the shape of the input
    arrays, are they C-contiguous etc.) that should not be bypassed.
    ...
    errCode = doubleCudaSORF3d_(cArray, radem_ptr,
                cpArray.shape[0], cpArray.shape[1],
                cpArray.shape[2])
    if errCode.decode("UTF-8") != "no_error":
        raise Exception("Fatal error encountered in doubleCudaSORF3d_.")
...
def doubleCudaSRHT(Z, radem,
    ...
    errCode = doubleCudaSRHT2d_(ZArray, radem_ptr, zDim0, zDim1)
    if errCode.decode("UTF-8") != "no_error":
...
def floatCudaSRHT(Z, radem,
    ...
    errCode = floatCudaSRHT2d_(ZArray, radem_ptr, zDim0, zDim1)
    if errCode.decode("UTF-8") != "no_error":
        raise Exception("Fatal error encountered.")
