...
and where we may have 2-3 different 'kinds' of features per node
that need separate lengthscales."""
...
class GraphFHTConv1d(KernelBaseclass):
    ...
    This allows a number of simplifications (but also enables us
    to use separate lengthscales for up to 2-3 different 'kinds'
    of features). This class inherits from KernelBaseclass.
    Only attributes unique to this child are described in this docstring.

    Attributes:
        hyperparams (np.ndarray): An array of three or more
            hyperparameters: lambda_ (noise), beta_ (amplitude)
            and sigma_1 - sigma_n (inverse mismatch tolerance for
    ...
    def __init__(self, xdim, num_rffs, random_seed = 123, device = "cpu",
        ...
        Args:
            ...
            kernel_spec_parms (dict): A dictionary of additional kernel-specific
                attributes. In this case, should contain 'split_points'.
        ...
        if "split_points" not in kernel_spec_parms:
            raise ValueError("For the GraphFHTConv1d kernel, 'kernel_specific_params' "
                    "must contain a list called 'split_points'. This list "
                    "may be empty, in which case only one lengthscale is "
                    "used. If it is not empty, it should contain up to"
                    "2 'split points' at which to split the feature "
                    "representation of each graph node and use a different "
                    "lengthscale.")
        ...
        self.change_split_points(kernel_spec_parms["split_points"], xdim)

        self.conv_func = None
        self.device = device


    def change_split_points(self, new_split_points, xdim):
        """This function is unique to this and other ARD kernels.
        It changes the split points to the inputs. This is called
        when the kernel is first constructed.

        Args:
            new_split_points (list): A list of split points.
            xdim (tuple): A tuple indicating the size of the expected input data.

        Raises:
            ValueError: A ValueError is raised if the inputs are
                not valid.
        """
        if not isinstance(new_split_points, list):
            raise ValueError("For the GraphFHTConv1d kernel, 'split_points' must "
                    "be a list.")

        self.split_points = np.sort([0] + new_split_points + [xdim[2]])
        if self.split_points.min() < 0 or self.split_points.max() > xdim[2]:
            raise ValueError("Split points must be inside the expected dimensions "
                        "of the input.")
        if np.diff(self.split_points).min() == 0:
            raise ValueError("Duplicate split points supplied to GraphFHTConv1d!")
        hparams = np.empty((self.split_points.shape[0] + 1))
        hparams[:2] = self.hyperparams[:2]
        hparams[2:] = self.hyperparams[2]
        self.hyperparams = hparams
        bounds = [[1e-3,1e1]] + [[0.2,5]] + [[1e-3, 1e2] for i in
                range(self.split_points.shape[0] - 1)]
    ...
    def pretransform_x(self, input_x):
        """Generating pre-sorf data requires much more memory for the conv1d
        kernel than for 'basic' kernels, so that pre-transformation for
        hyperparameter tuning is not really viable (pre-transformation
        for fitting however is still a useful strategy). We nonetheless
        keep the pretransform_x function so that Conv1d shares a common
        interface with 'basic' kernels. This function merely returns
        a reference to the input numpy or cupy array and ensures the
        data type is correct."""
        return input_x.astype(self.dtype)


    def finish_transform(self, input_x, multiply_by_beta = True):
        """For this, unlike basic kernels, the full feature generation
        process takes place in the finish_transform function.
        ...
        self.conv_func(reshaped_x, self.radem_diag, xtrans, self.chi_arr,
                2, self.hyperparams[2:], self.split_points)
        if multiply_by_beta:
            xtrans *= self.hyperparams[1]
    ...
    def kernel_specific_gradient(self, input_x):
        ...
        dz_dsigma = self.conv_func(reshaped_x, self.radem_diag,
                ...
                self.split_points, mode = "gradient")
