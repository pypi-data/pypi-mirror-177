...
class Linear(KernelBaseclass):
    ...
    def pretransform_x(self, input_x):
        ...
        (pretransform and finish_transform). For the LinearKernel,
        we merely return the input. This function is only provided
        for consistency with other kernel classes."""
        return input_x
    ...
    def finish_transform(self, input_x, multiply_by_beta = True):
        """For most kernels, random feature generation has two steps
        (pretransform and finish_transform). For the LinearKernel,
        we merely return the input, multiplying by amplitude if
        requested.
        """
    ...
    def kernel_specific_gradient(self, input_x):
        ...
        """
        xtrans = self.finish_transform(input_x)
