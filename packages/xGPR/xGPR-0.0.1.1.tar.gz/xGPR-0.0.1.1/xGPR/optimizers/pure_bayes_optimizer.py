...
def pure_bayes_tuning(cost_function, tuning_dataset,
            bounds, random_seed, max_iter, starting_hyperparams = None,
            verbose = True, tol = 1e-2):
    """Bayesian optimization for hyperparameters without the efficient
    strategy used in bayes_grid. Compared with bayes_grid,
    this routine is somewhat cheaper per iteration and
    scalable to a larger number of RFFs but worse at navigating
    large hyperparameter spaces. It is therefore best used to
    search a small bounding box identified by an earlier search.
    ...
    Args:
        ...
        starting_hyperparams (np.ndarray): The starting hyperparameters. If None,
            randomly selected points are used.
    ...
    if starting_hyperparams is not None:
        hparam_vals.append(starting_hyperparams)
    for i, hparam_val in enumerate(hparam_vals):
        score = cost_function(hparam_val, tuning_dataset, suppress_warnings=True)
        scores.append(score)
        if verbose:
    ...
    for iternum in range(len(hparam_vals), max_iter):
        ...
        score = cost_function(new_hparam, tuning_dataset, suppress_warnings=True)
