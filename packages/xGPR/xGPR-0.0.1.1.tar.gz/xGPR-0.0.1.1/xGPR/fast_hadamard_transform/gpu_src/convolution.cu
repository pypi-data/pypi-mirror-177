...
__global__ void floatConv1dMultiplyByDiagonalMat(float *cArray, int8_t *rademArray,
...
//-- done by the wrapper -- and that only S elements of the appropriate row of
//the [3, 1, P x S] array are used.
__global__ void doubleConv1dMultiplyByDiagonalMat(double *cArray, int8_t *rademArray,
...
const char *floatConv1dPrep_(int8_t *radem, float *reshapedX, int reshapedDim0, 
    ...
    floatConv1dMultiplyByDiagonalMat<<<blocksPerGrid, DEFAULT_THREADS_PER_BLOCK>>>(reshapedX, 
...
//
//Note that all of these arrays are already expected to "live" on GPU.
const char *doubleConv1dPrep_(int8_t *radem, double *reshapedX, int reshapedDim0, 
                int reshapedDim1, int reshapedDim2, int startPosition,
                int numFreqs){
    ...
    //Multiply by first row of radem.
    doubleConv1dMultiplyByDiagonalMat<<<blocksPerGrid, DEFAULT_THREADS_PER_BLOCK>>>(reshapedX, 
                        radem, reshapedDim2, startPosition, numElements,
                        normConstant);
    //First H-transform.
