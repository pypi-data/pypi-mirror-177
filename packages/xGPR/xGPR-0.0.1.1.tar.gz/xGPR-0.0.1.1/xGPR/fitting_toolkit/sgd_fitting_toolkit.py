...
class sgdModelFit:
    ...
    def fit_model(self, dataset, kernel, tol = 1e-6, max_epochs = 40,
        ...
        for self.n_epoch in range(max_epochs):
            ...
            while not end_epoch:
                ...
                xbatch = kernel.transform_x(xbatch, pretransformed = dataset.pretransformed)
                ...
                if preconditioner is not None:
                    wvec -= step_size * preconditioner.matvec(current_grad)
                else:
    ...
    def initialize(self, dataset, kernel):
        ...
        for xdata, ydata in dataset.get_chunked_data():
            xdata = kernel.transform_x(xdata, pretransformed = dataset.pretransformed)
