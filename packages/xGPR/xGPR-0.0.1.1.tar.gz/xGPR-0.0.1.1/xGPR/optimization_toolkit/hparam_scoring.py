...
def build_preconditioner(xgp_model, fitting_dset, preset_hparams,
        random_state = 123):
    """Tries to build a preconditioner using initially rank 1024.
    If this gives a high ratio, the rank is increased up to 3k. If an
    If that was insufficient, we switch to L-BFGS.
    The goal is to make sure there is no scenario that 'breaks' the
    optimization.
    ...
    preconditioner, ratio = xgp_model.build_preconditioner(fitting_dset,
                    ...
                    random_state = random_state, method = "srht")
    ...
    if ratio > 100:
        improved_precond, _ = xgp_model.build_preconditioner(fitting_dset,
                    max_rank = 3000, preset_hyperparams = preset_hparams,
                    random_state = random_state, method = "srht")
        preconditioner = improved_precond
