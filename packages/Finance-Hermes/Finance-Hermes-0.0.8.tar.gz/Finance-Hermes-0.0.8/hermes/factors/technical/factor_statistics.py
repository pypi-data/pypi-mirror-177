...
class FactorStatistics(FactorBase, LongCallMixin, ShortMixin):
    ...
    def MAD(self,
            ...
            **kwargs):
        length = int(length) if length and length > 0 else 30
        result = mad(data['close'], length=length, offset=offset, **kwargs)
    ...
    def MEDIAN(self,
               ...
               **kwargs):
        length = int(length) if length and length > 0 else 30
        result = median(data['close'], length=length, offset=offset, **kwargs)
    ...
    def QUANTILE(self,
        ...
        result = median(data['close'],
                        length=length,
                        offset=offset,
                        q=q,
                        **kwargs)
        return self._format(result, f"QTL_{length}_{q}")
    ...
    def SKEW(self,
        ...
        length = int(length) if length and length > 0 else 30
        result = skew(data['close'], length=length, offset=offset, **kwargs)
