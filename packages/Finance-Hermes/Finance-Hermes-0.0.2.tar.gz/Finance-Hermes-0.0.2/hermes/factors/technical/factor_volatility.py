...
class FactorVolatility(FactorBase, LongCallMixin, ShortMixin):
    ...
    def ABERRATION(self,
        ...
        zg, sg, xg, atr_ = aberration(data['high'],
                                      data['low'],
                                      data['close'],
                                      length=length,
    ...
    def ACCBANDS(self,
        ...
        lower, mid, upper = accbands(data['high'],
                                     data['low'],
                                     data['close'],
                                     length=length,
                                     c=c,
    ...
    def BBANDS(self,
        ...
        lower, mid, upper, bandwidth, percent = bbands(data['close'],
                                                       length=length,
                                                       std=std,
                                                       ddof=ddof,
                                                       offset=offset,
                                                       **kwargs)
        lower = self._format(lower, f"BBL_{length}_{std}")
        mid = self._format(mid, f"BBM_{length}_{std}")
        upper = self._format(upper, f"BBU_{length}_{std}")
        bandwidth = self._format(bandwidth, f"BBB_{length}_{std}")
        percent = self._format(percent, f"BBP_{length}_{std}")
    ...
    def DONCHINAN(self,
        ...
        lower, mid, upper = donchian(data['high'],
                                     data['low'],
    ...
    def KC(self,
        ...
        lower, basis, upper = kc(data['high'],
                                 data['low'],
                                 data['close'],
                                 length=length,
                                 scalar=scalar,
    ...
    def THERMO(self,
        ...
        result, thermo_ma, thermo_long, thermo_short = thermo(data['high'],
                                                              data['low'],
                                                              length=length,
                                                              long=long,
                                                              short=short,
        ...
        result = self._format(result, f"THERMO_{length}_{long}_{short}")
        thermo_ma = self._format(thermo_ma,
                                 f"THERMOma_{length}_{long}_{short}")
        thermo_long = self._format(thermo_long,
                                   f"THERMOl_{length}_{long}_{short}")
        thermo_short = self._format(thermo_short,
                                    f"THERMOs_{length}_{long}_{short}")
