...
def ac_mode_func(args):
    ...
    # Get file headers
    header_indices, header_names, headers_supported = ac.parse_headers(
    ...
    if any(val == -1 for val in header_indices.values()):
        ...
        for ind, name in zip(header_indices.values(), header_names.keys()):
            if ind == -1:
                ...
                message += '\n  Options for expected header are: '
                for i in headers_supported[name]:
                    message += '\n   {} '.format(i)
    ...
    if any(val == -1 for val in header_indices.values()):
        message = '\n***Error***:\n'
        message += 'For () header should be one of ():'
        for ind, name in zip(header_indices.values(), header_names.keys()):
            if ind == -1:
                message += '\n {}:'.format(name)
                for i in headers_supported[name]:
                    message += '( {}, )'.format(i)
        message += '\ncannot be found in the input file.'
