...
class SDKTrader:
            ...
            try:
                ...
                async for msg in ws:
                    if isinstance(msg, Ok):
                        ...
                        data = base64.b64decode(msg.result.value.data[0])
    ...
    def commit_batch(self, sdk: "SDKContext") -> List[SDKOrderSummary]:
    ...
    def place_order(
        ...
        if batch:
            self._pending_actions.append(_PendingNewOrder(product, side, size, price, self_trade_behavior, order_type))
        else:
            update_mark_prices_ix = self._update_mark_prices_ix(sdk)
    ...
    def cancel(
        ...
        self.cancel_underwater(sdk, product, order_id, self.account, batch)
    ...
    def cancel_underwater(
        ...
        if batch:
            self._pending_actions.append(self._cancel_ix(sdk, product, order_id, under_water_trg, no_err))
        else:
            update_mark_prices_ix = self._update_mark_prices_ix(sdk)
    ...
    def replace(
        ...
        if batch:
            self._pending_actions.append(cancel_ix)
            self._pending_actions.append(_PendingNewOrder(product, side, size, price, self_trade_behavior, order_type))
        else:
            update_mark_prices_ix = self._update_mark_prices_ix(sdk)
