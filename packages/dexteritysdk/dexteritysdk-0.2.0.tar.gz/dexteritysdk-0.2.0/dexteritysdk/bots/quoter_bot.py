...
async def main():
    ...
    keypair_bytes = base58.b58decode(os.environ["PAYER"])
    ...
    trader.replace(sdk, sdk.products[0], 12345, Side.BID, 0.001, 15000)

    for p in sdk.products:
        eq = EventQueue.from_bytes(base64.b64decode(explore(p.event_queue).data[0]))
        print(f"Product {p.index} {p.name} - events {len(list(eq.items()))}")

    def p_cb(p, t):
        print(f"Position {t} - {p}")

    await trader.subscribe(sdk, "wss://api.devnet.solana.com/", orders_callback=p_cb)
