...
save_datasets = json.load(args.save_datasets_json)
...
for dataset in save_datasets:
    file_data_set = Dataset.File.from_files(path=(datastore, output_jsons + '/' + dataset["pattern"]))
    file_data_set.register(
        workspace=workspace,
        name=dataset["name"],
