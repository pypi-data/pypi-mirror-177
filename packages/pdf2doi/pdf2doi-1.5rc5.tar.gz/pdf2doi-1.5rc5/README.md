...
#### Main changes
- The library ```textract``` has been removed from the required dependencies, due to the fact that it often creates problems during installation (due to conflicts between library versions)
...
#### Added
- https://github.com/MicheleCotrufo/pdf2doi/issues/21
 When an arXiv ID is found a corresponding DOI is also returned, when available. This could be either the standard arXiv DOI (see also [here](https://blog.arxiv.org/2022/02/17/new-arxiv-articles-are-now-automatically-assigned-dois/)),
 ...
 - https://github.com/MicheleCotrufo/pdf2doi/issues/22
 The function ```get_pdf_text``` (finders.py) has been modified to make the library ```PyPDF2``` extract also the text of any annotations/comments present in the pdf file.

#### Fixed
- Potential titles of the papers were often not correctly found, because the function ```find_possible_titles()``` (finders.py) would mistakenly disregard all the results if just one of the methods (pdftitle, PyPDF2, filename) generated an error.
...
The library ```textract``` provides additional ways to analyze pdf files, but it comes with a large overhead of additional dependencies and sometimes it generates version conflicts. 
