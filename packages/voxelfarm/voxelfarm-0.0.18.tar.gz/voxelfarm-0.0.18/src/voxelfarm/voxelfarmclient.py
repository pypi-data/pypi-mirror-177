...
class rest:
        ...
        if project and project.ContainsKey('workflow_folder_' + product_id):
            ...
            if version > 0:
                ...
                if apiResult.success:
                    ...
                    for e in entities:
                        entity = entities[e]
                        if (entity['file_folder'] == product_folder_id) and (entity['type'] == 'FOLDER'):
                            if entity["name"] == version_name:
                                extended_prop = 'version_' + alias_id
                                if entity.ContainsKey(extended_prop):
                                    return entity[extended_prop]
            else:
                product_entity = self.get_entity(product_folder_id)
                if product_entity and product_entity.ContainsKey('version_active'):
                    active_version_id = product_entity['version_active']
                    active_version = self.get_entity(active_version_id)
                    extended_prop = 'version_' + alias_id
    ...
    def workflow_get_dataframe(self, project_id, product_id, alias_id, version = 0):
