...
:LastEditTime: 2022-11-21 11:32:49
...
class GoodsExchangeHandle(SevenBaseHandler):
    ...
    @filter_check_params(["user_id", "user_nick", "avator", "goods_id", "goods_count", "total_integral"])
    @filter_check_sign(config.get_value("sign_key"))
...
class ExchangeListHandle(SevenBaseHandler):
    ...
    def get_async(self, *args, **kwargs):
        ...
        # TODO 执行业务
        exchange_list,exchange_count = ExchangeModel().get_dict_page_list(field="nick_name,avator,add_time",page_index=page_index-1,page_size=page_size,where="goods_id = %s",order_by="add_time desc",params=goods_id)
