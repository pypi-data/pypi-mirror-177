...
:LastEditTime: 2022-11-22 18:16:30
...
class GoodsDetailHandler(SevenBaseHandler):
...
class GoodsExchangeHandler(SevenBaseHandler):

    @filter_check_params(["user_id", "goods_id", "goods_count", "total_integral"])
    @filter_check_sign(config.get_value("sign_key"))
    def post_async(self, *args, **kwargs):
        ...
        # 头像
        user_avatar = self.request_params["user_avatar"]
        # 商品id
        ...
        # TODO 执行业务
        goods_model = goods_conn.get_dict_by_id(goods_id,"goods_code,goods_inventory,goods_integral,goods_name,goods_icon")
        if not goods_model:
            return self.reponse_json_error("无法获取商品信息")

        goods_info = {
            "goods_code": goods_model["goods_code"],
            "goods_name": goods_model["goods_name"],
            "goods_icon": goods_model["goods_icon"],
        }
        ...
        is_inventory = config.get_value("is_inventory",0)
        if is_inventory:
            if self.goods_exchange(goods_info,self.request_params):
                # 添加兑换记录
                ...
                exchange_entity.user_avatar = user_avatar
            ...
            if self.user_deal(goods_info,order_id,self.request_params):
                return self.reponse_json_success("兑换成功")
            else:
    ...
    def goods_exchange(self,goods_info,request_params):
        """
    ...
    def user_deal(self,goods_info,exchange_order_id,request_params):
        """
        :description: 兑换用户所需处理，扣除积分等(必须重写)
        :param user_id 用户id
        :param exchange_order_id 兑换订单号
        :param total_integral 所需积分
        :return True 兑换成功 False 兑换失败
        :last_editors: Kangwenbin
        """
...
class ExchangeListHandler(SevenBaseHandler):
    ...
    def get_async(self, *args, **kwargs):
        ...
        # TODO 执行业务
        exchange_list,exchange_count = ExchangeModel().get_dict_page_list(field="nick_name,user_avatar,add_time,goods_count",page_index=page_index-1,page_size=page_size,where="goods_id = %s",order_by="add_time desc",params=goods_id)

        ret_model = {
            "goods_list": exchange_list,
            "has_next": True if math.ceil(exchange_count/page_size) > page_index else False
        }
