...
class Bank(Resource):
    ...
    def resource(self, resource_name):
        resource = self._resources[resource_name].set_bank_id(
            super().get_bank_id()).set_headers(self._nash.get_headers()).set_ipn_object(ipn_object=super().get_ipn_object)
    ...
    def ipn(self, operation=None, payload=None):
        ...
        if operation == super().IPN_REGISTER:
            ...
            if 'client_id' in super().response().keys():
                ...
                super().get_ipn_object.create(bank_id=payload.get('bank_id', ''), client_id=super().response().get('client_id', ''), client_secret=super().response().get(
        ...
        # If the operation performed is to update an IPN
            ...
            if 'client_id' in super().response().keys():
                ...
                data = super().get_ipn_object.filter(client_id=payload.get('client_id', ''),account_number=payload.get('account_number', ''))
        ...
        # If the operation performed is to update an IPN
            ...
            if 'client_id' in super().response().keys():
                ...
                data = super().get_ipn_object.filter(client_id=payload.get('client_id', ''))
        ...
        # If the operation performed is to delete an IPN
            ...
            if 'client_id' in super().response().keys():
                ...
                data = super().get_ipn_object.filter(client_id=payload.get(
                    'client_id', ''))
