...
class MaskRenderer:
    def __init__(self, name=DEFAULT_MP_NAME, root='/data/tianzun/model', insfa=None):
    ...
    def build_params(self, face_image):
        ...
        faces = self.insfa.get(face_image, max_num=1)
        # print('model_process',  time.time()- start_time)
        start_time = time.time()
        if len(faces)==0:
            return None
        ...
        # print('3d_process', time.time() - start_time)
        return [fitted_sp, fitted_ep, fitted_s, fitted_angles, fitted_t, landmark.reshape(-1,1)]
    ...
    def render_mask(self,face_image, mask_image, params, input_is_rgb=False, auto_blend = True, positions=[0.1, 0.33, 0.9, 0.7]):
        if isinstance(mask_image, str):
            ...
            uv_mask_image = ins_get_image(mask_image, to_rgb=to_rgb)
        # uv_mask_image = self.generate_mask_uv(mask_image, positions)
        if not uv_mask_image.shape[0] == self.uv_size[0]:
            uv_mask_image = cv2.resize(uv_mask_image,self.uv_size)

        # print('uv_mask_image',uv_mask_image.shape)
        # cv2.imshow("uv_mask_image",uv_mask_image)
        # cv2.waitKey()
    ...
    def decode_params(params):
        ...
        p5 = params[235:235+136]
        p5 = np.array(p5, dtype=np.float32).reshape( (-1, 2))
...
class MaskAugmentation(ImageOnlyTransform):
    ...
    def __init__(
            ...
            always_apply=False,
            p=1.0,
            name=DEFAULT_MP_NAME, root='/root/model'
            ):
        super(MaskAugmentation, self).__init__(always_apply, p)
        self.renderer = MaskRenderer(name=name,root=root)
    ...
    def apply(self, image, hlabel, mask_name, h_pos, **params):
        ...
        # print("h_pos",h_pos)
        valid_len = [237,235,238,237+136,235+136,238+136]
        assert len(hlabel) in valid_len, 'make sure the rec dataset includes mask params'
        if len(hlabel)==237 or len(hlabel)==237+136:
            if hlabel[1]<0.0:
                return image
            hlabel = hlabel[2:]
        if len(hlabel)==238 or len(hlabel)==238+136:
            if hlabel[1]<0.0:
                return image
            hlabel = hlabel[2:-1]
        # print(len(hlabel))
        mask_params, landmarks = self.renderer.decode_params(hlabel)
