...
class ArtifactEntry:
    ...
    def download(self, local_root_path: str, artifact_id: int):
        ...
        if platform.system() == "Windows":
            # OS가 윈도우인 경우 path seperator 를 "/"로 통일시켜줌.
            local_path = os.path.join(local_root_path, self.path.replace("/", os.path.sep))
        else:
    ...
    def upload(self, upload_type: str, root_path: str, run_id: int, artifact_id: int, last_file_yn: str, teamName: str, expName: str, artifact_name: str, artifact_type: str, artifact_digest: str, entry_list: List[grpc_ArtifactEntry]) -> bool:
        ...
        return interface.upload_file( upload_type, self.local_path, root_path, self.path, run_id, artifact_id, last_file_yn, teamName, expName, artifact_name, artifact_type, artifact_digest, self.digest, entry_list)
...
class Artifacts:
    ...
    def upload(self) -> bool:
        ...
        entry_dict = {entry.path.replace("\\", "/"): entry for entry in self.entry_list}
        ...
        for i, entry in enumerate(self.entry_list):
            ...
            succeeded = entry.upload(upload_type, root_path, self.run.run_id, self.id, last_file_yn,  self.run.team_name, self.run.exp_name, self.name, self.type, artifact_digest, entry_list)
            if not succeeded:
                return False
        return True
