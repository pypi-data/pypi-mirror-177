...
class GetComponents():
	...
	def excludeComponents(cls, frm, exclude=[]):
		...
		exclude = cls.convertReturnType(exclude, returnType=rtn, flatten=True) #assure both lists are of the same type for comparison.
...
class Component_utils_maya(GetComponents):
	...
	@classmethod
	def getBorderComponents(cls, x, rtnCompType='default', borderType='object', returnType='str', flatten=False):
