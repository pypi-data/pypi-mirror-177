...
class Connect:
    def __init__(self, ipaddr, username, passwd):
        ...
        try:
            self.version = self.get("provisioning/sysinfo")['SoftwareVersion'].replace('X', '')
    ...
    def rest(self, method, target_uri=None, params={}):
        if target_uri.find("api/") > -1:
            target_uri = "".join(target_uri.split("api/")[1:])

        target_uri = f"https://{self.ipaddr}/api/{target_uri}"
        ...
        try:
            ...
            if 200 <= response.status_code <= 300:
                ...
                try:
                    ...
                    print(f"EXP response parsing error: {target_uri} - {response.text}", file=sys.stderr)
                    return False
                else:
                    return result
