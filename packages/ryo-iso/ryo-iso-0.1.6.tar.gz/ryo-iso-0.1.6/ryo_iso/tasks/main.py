...
def task__base_image_download():
    ...
    def download_iso():
        ...
        if iso_path.is_file():
            ...
            cmd = "curl -J --silent --remote-name " + iso_url
...
def task__squashfs_apt_proxy():
    ...
    :actions:
      - ``echo 'Acquire::http { Proxy \"http://127.0.0.1:3142\"; };' | sudo tee squashfs-root/etc/apt/apt.conf.d/01proxy``
    :targets:
    ...
    if 'apt' in _ctx['config'].iso_cfg.keys():
        return {
            'actions': ["echo 'Acquire::http { Proxy \"http://127.0.0.1:3142\"; };'"
