...
class ReadCheque:
	...
	def read_cheque_details(self,file_name, templates, input_reader_module ):
		logger.log(f"cheque_details for file_name : {file_name}","0" )
		...
		#logger.log(f"mid_cheque_data : {mid_cheque_data}","0")
		logger.log(f"cheque_result: {str(result)}","0" )
		logger.log(f"cheque_no_details : |{cheque_no_details}","0")
		...
		logger.log(f"{result}","0")
		
		try:
			os.remove("mid_width_fo_acc_no.jpg")
			os.remove("bottom_part_cheque.jpg")
		except Exception as ex:
			logger.log(f"readCheque: Exception while removing cheque parts file {ex}","0")
