...
try:
    ...
    packagedir = path.dirname(__file__)
    sourcedir = path.join(packagedir, 'molalignlib')
    command = path.join(packagedir, 'compile.sh')
    with Popen([command, sourcedir, '-all', '-pic'], stdout=PIPE, stderr=STDOUT, bufsize=0) as p:
        for line in p.stdout:
            print(line.decode('utf-8').rstrip())
    command = path.join(packagedir, 'link_library_python.sh')
