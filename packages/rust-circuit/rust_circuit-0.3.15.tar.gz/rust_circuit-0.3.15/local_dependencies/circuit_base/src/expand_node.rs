...
pub fn expand_node(circuit: CircuitRc, inputs: &Vec<CircuitRc>) -> Result<CircuitRc> {
    ...
    match &**circuit {
        ...
        // TODO: symbolic shape case for below!!!
        ...
        Circuit::Rearrange(node) => {
            ...
            let new_input = SetSymbolicShape::some_set_and_symbolic_neq(
                ...
                iter::repeat(OpSize::NONE)
                    .take(batch_ranks[0])
                    .chain(node.spec.forced_input_shape(true))
                    .collect(),
                None,
            )
            .with_context(|| {
            ...
            Ok(Rearrange::nrc(new_input, new_spec, circuit.name_cloned()))
        }
        ...
        Circuit::Module(node) => {
            ...
            let spec = ModuleSpec::new(spec_circuit, arg_specs, false)
                .context("constructing new ModuleSpec failed in expand")?;
...
pub enum ExpandError {
    ...
    #[error("old_symbol={old_symbol:?} new_circuit={new_circuit:?}")]
    ...
    #[error("node_rank={node_rank} < symbol_rank={symbol_rank}, arg_spec={arg_spec:?} node_shape={node_shape:?} spec_circuit={spec_circuit:?}")]
    ModuleRankReduced {
        node_rank: usize,
        symbol_rank: usize,
    ...
    #[error("node_rank={node_rank} > symbol_rank={symbol_rank} (which indicates batching) and arg_spec={arg_spec:?} spec_circuit={spec_circuit:?}")]
    ...
    #[error("node_shape={node_shape:?} symbol_shape={symbol_shape:?} arg_spec={arg_spec:?} spec_circuit={spec_circuit:?}")]
