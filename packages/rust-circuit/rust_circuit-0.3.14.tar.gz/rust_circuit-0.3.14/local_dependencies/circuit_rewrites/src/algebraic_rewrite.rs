...
pub fn remove_axes(circuit: &CircuitRc, axes: &HashSet<usize>) -> Option<CircuitRc> {
    ...
    match &***circuit {
        ...
        Circuit::Rearrange(rearrange) => {
            ...
            let spec = rearrange
                .conform_to_input_shape_spec(false)
                .canonicalize(true);
...
pub fn rearrange_fuse(node: &Rearrange) -> Option<Rearrange> {
    node.node.as_rearrange().and_then(|inner| {
        RearrangeSpec::fuse(
            &inner.conform_to_input_shape_spec(false),
            &node.conform_to_input_shape_spec(false),
        )
        .map(|spec| {
            Rearrange::new(
                inner.node.clone(),
...
pub enum DistributeError {
    #[error("add={add:?} do_broadcasts={do_broadcasts}")]
    CouldntBroadcast { add: Add, do_broadcasts: bool },
    #[error("einsum={einsum:?} operand_idx={operand_idx}")]
