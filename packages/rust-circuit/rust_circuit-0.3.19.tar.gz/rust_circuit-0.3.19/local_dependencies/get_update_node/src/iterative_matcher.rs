...
pub enum IterativeMatcherData {
    ...
    FilterModuleSpec(IterativeMatcherRc),
...
impl IterativeMatcherData {
    fn uuid(&self) -> [u8; 16] {
        match self {
            ...
            Self::FilterModuleSpec(_) => uuid!("b728a8c3-0493-4ee5-96c2-71e4900fc0f8"),
    ...
    fn item_hash(&self, hasher: &mut blake3::Hasher) {
        match self {
            ...
            Self::SpecCircuit(matcher) | Self::FilterModuleSpec(matcher) => {
...
pub struct ChildrenMatcher {
    pub iterative_matcher: IterativeMatcherRc,
    pub child_numbers: HashSet<usize>,
}

impl ChildrenMatcher {
    pub fn new(
        iterative_matcher: IterativeMatcherRc,
        child_numbers: HashSet<usize>,
    ) -> Result<Self> {
        Ok(Self {
            iterative_matcher,
            child_numbers,
...
impl IterativeMatcher {
    ...
    pub fn validate_matched(&self, matched: &HashSet<CircuitRc>) -> Result<()> {
        match &self.data {
            ...
            IterativeMatcherData::FilterModuleSpec(m) => m.validate_matched(matched),
...
impl IterativeMatcher {
    ...
    pub fn match_iterate(&self, circuit: CircuitRc) -> Result<IterateMatchResults> {
        ...
        let res = match &self.data {
            ...
            IterativeMatcherData::FilterModuleSpec(matcher) => {
                let is_module = circuit.is_module();
                ...
                if is_module {
                    ...
                    let updated = per_child
                        ...
                        .map(|(i, x)| {
                            x.and_then(|x| {
                                (i != 0 && (i - 1) % 2 != 0)
                                    .then(|| IterativeMatcherData::FilterModuleSpec(x).into())
                            })
                            .into()
                        })
                        .collect::<Vec<Update>>()
                    ...
                    IterateMatchResults {
                        updated: map_updated(updated, |x| {
                            IterativeMatcherData::FilterModuleSpec(x).into()
                        }),
                        found,
                    }
                }
            ...
            IterativeMatcherData::Chains(chains) => {
                ...
                fn run_item(
                    chain: MaybeChain<'_>,
                    circuit: &CircuitRc,
                    new_items: &mut HashSet<ChainItemPerChild>,
                ) -> Result<bool> {
                    let IterateMatchResults { updated, found } =
                    ...
                    .any(|x| matches!(x, ChainItemPerChild::Many(_)))
                {
                    ...
                    let mut out = vec![HashSet::default(); num_children];
...
impl IterativeMatcher {
    ...
    pub fn children_matcher(&self, child_numbers: HashSet<usize>) -> IterativeMatcher {
    ...
    pub fn filter_module_spec(&self, enable: bool) -> IterativeMatcher {
        if enable {
            IterativeMatcherData::FilterModuleSpec(self_.clone().rc()).into()
        } else {
            self_.clone()
        }
    }
