...
class CustomCLIPWrapper(CLIPWrapper):
    def __init__(self,
        ...
        with torch.no_grad():
            ...
            self.image_encoder = timm.create_model('swin_base_patch4_window7_224',pretrained=True)
    ...
    def training_step(self, train_batch, idx):
        ...
        with torch.no_grad():
            ims = [F.normalize(self.image_encoder(im), dim=1) for im in image_mbs]
            txt = [F.normalize(self.model.encode_image(t), dim=1) for t in text_mbs]
            ...
            # calculate teacher
            teacher_ims = [F.normalize(self.image_encoder(im), dim=1) for im in image_mbs]
            teacher_txt = [F.normalize(self.model.encode_image(t), dim=1) for t in text_mbs]
        ...
        for j, mb in enumerate(text_mbs):
            ...
            text_tmp[self.global_rank][j*self.minibatch_size:(j+1)*self.minibatch_size] = F.normalize(self.model.encode_image(mb))
    ...
    def forward(self, images, text):
        logits = self.image_encoder(images) @ F.normalize(self.model.encode_text(text), dim=1).t() * self.model.logit_scale.exp()
