...
class AbstractComponentTest(unittest.TestCase):
    ...
    def test_remove_intent(self):
        ...
        df = pd.DataFrame({'A': list('12321'), 'B': list('12321'), 'C': list('12321')})
        inst.intent_model.auto_clean_header(df)
        self.assertEqual(['A', 'B', 'C'], inst.pm.get_intent().keys())
        inst.remove_intent('B')
        self.assertEqual(['A', 'C'], inst.pm.get_intent().keys())
        inst.remove_intent()
        self.assertEqual([], inst.pm.get_intent().keys())
