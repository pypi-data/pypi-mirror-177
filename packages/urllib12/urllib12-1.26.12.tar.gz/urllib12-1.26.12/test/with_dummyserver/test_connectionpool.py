...
from urllib4 import HTTPConnectionPool, encode_multipart_formdata
from urllib4._collections import HTTPHeaderDict
from urllib4.connection import _get_default_user_agent
...
from urllib4.packages.six.moves.urllib.parse import urlencode
from urllib4.util import SKIP_HEADER, SKIPPABLE_HEADERS
from urllib4.util.retry import RequestHistory, Retry
...
class TestConnectionPool(HTTPDummyServerTestCase):
    ...
    def test_user_agent_header_not_sent_twice(self, headers, chunked):
        with HTTPConnectionPool(self.host, self.port) as pool:
            ...
            if not headers:
                assert request_headers["User-Agent"].startswith("python-urllib4/")
    ...
    def test_skip_header_non_supported(self, header, chunked):
        with HTTPConnectionPool(self.host, self.port) as pool:
            ...
            assert (
                str(e.value)
                == "urllib4.util.SKIP_HEADER only supports 'Accept-Encoding', 'Host', 'User-Agent'"
            )
