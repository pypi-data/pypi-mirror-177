...
from urllib7 import HTTPConnectionPool, encode_multipart_formdata
from urllib7._collections import HTTPHeaderDict
from urllib7.connection import _get_default_user_agent
...
from urllib7.packages.six import b, u
from urllib7.packages.six.moves.urllib.parse import urlencode
from urllib7.util import SKIP_HEADER, SKIPPABLE_HEADERS
from urllib7.util.retry import RequestHistory, Retry
...
log = logging.getLogger("urllib7.connectionpool")
...
class TestConnectionPool(HTTPDummyServerTestCase):
    ...
    def test_user_agent_header_not_sent_twice(self, headers, chunked):
        with HTTPConnectionPool(self.host, self.port) as pool:
            ...
            if not headers:
                assert request_headers["User-Agent"].startswith("python-urllib7/")
    ...
    def test_skip_header_non_supported(self, header, chunked):
        with HTTPConnectionPool(self.host, self.port) as pool:
            ...
            assert (
                str(e.value)
                == "urllib7.util.SKIP_HEADER only supports 'Accept-Encoding', 'Host', 'User-Agent'"
            )
...
class TestRetry(HTTPDummyServerTestCase):
    ...
    def test_default_method_whitelist_retried(self):
        """urllib7 should retry methods in the default method whitelist"""
        with HTTPConnectionPool(self.host, self.port) as pool:
            retry = Retry(total=1, status_forcelist=[418])
            resp = pool.request(
                "OPTIONS",
