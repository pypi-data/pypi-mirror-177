...
from urllib7 import HTTPSConnectionPool
from urllib7.connection import RECENT_DATE, VerifiedHTTPSConnection
...
class TestHTTPS(HTTPSDummyServerTestCase):
    ...
    def test_verified_with_context(self):
        ctx = util.ssl_.create_urllib7_context(cert_reqs=ssl.CERT_REQUIRED)
    ...
    def test_ssl_wrong_system_time(self):
        with HTTPSConnectionPool(
            ...
            https_pool.cert_reqs = "CERT_REQUIRED"
            https_pool.ca_certs = DEFAULT_CA
            with mock.patch("urllib7.connection.datetime") as mock_date:
    ...
    def test_default_tls_version_deprecations(self):
        ...
        if self.tls_protocol_deprecated():
            ...
            assert str(w[0].message) == (
                ...
                "and will be disabled in urllib7 v2.0.0. Connecting to "
    ...
    def test_no_tls_version_deprecation_with_ssl_context(self):
        ...
        with HTTPSConnectionPool(
            ...
            self.port,
            ca_certs=DEFAULT_CA,
            ssl_context=util.ssl_.create_urllib7_context(),
        ) as https_pool:
            conn = https_pool._get_conn()
            try:
                with warnings.catch_warnings(record=True) as w:
...
class TestHTTPS_IPV4SAN:
    def test_can_validate_ip_san(self, ipv4_san_server):
        """Ensure that urllib7 can validate SANs with IP addresses in them."""
        with HTTPSConnectionPool(
...
class TestHTTPS_IPV6SAN:
    ...
    def test_can_validate_ipv6_san(self, ipv6_san_server, host):
        """Ensure that urllib7 can validate SANs with IPv6 addresses in them."""
        with HTTPSConnectionPool(
            host,
