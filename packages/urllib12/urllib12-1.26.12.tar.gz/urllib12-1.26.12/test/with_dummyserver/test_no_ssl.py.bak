...
Note: Import urllib7 inside the test functions to get the importblocker to work
"""
import pytest
...
class TestHTTPWithoutSSL(HTTPDummyServerTestCase, TestWithoutSSL):
    def test_simple(self):
        with urllib7.HTTPConnectionPool(self.host, self.port) as pool:
            r = pool.request("GET", "/")
...
class TestHTTPSWithoutSSL(HTTPSDummyServerTestCase, TestWithoutSSL):
    def test_simple(self):
        with urllib7.HTTPSConnectionPool(
            self.host, self.port, cert_reqs="NONE"
        ) as pool:
            try:
                pool.request("GET", "/")
            except urllib7.exceptions.SSLError as e:
