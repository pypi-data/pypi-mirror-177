...
import urllib7.exceptions
import urllib7.util.retry
import urllib7.util.url
from urllib7.contrib import appengine
...
class TestGAEConnectionManager(test_connectionpool.TestConnectionPool):
    ...
    def test_exceptions(self):
        ...
        with pytest.raises(urllib7.exceptions.TimeoutError):
            self.pool.request(
                "GET",
        ...
        with pytest.raises(urllib7.exceptions.ProtocolError):
...
class TestGAEConnectionManagerWithSSL(dummyserver.testcase.HTTPSDummyServerTestCase):
    ...
    def test_exceptions(self):
        ...
        # self-signed certs.
        with pytest.raises(urllib7.exceptions.SSLError):
...
class TestGAERetry(test_connectionpool.TestRetry):
    ...
    def test_default_method_whitelist_retried(self):
        ...
        retry = urllib7.util.retry.Retry(total=1, status_forcelist=[418])
    ...
    def test_retry_return_in_response(self):
        ...
        retry = urllib7.util.retry.Retry(total=2, status_forcelist=[418])
        resp = self.pool.request(
        ...
        assert resp.retries.history == (
            urllib7.util.retry.RequestHistory(
