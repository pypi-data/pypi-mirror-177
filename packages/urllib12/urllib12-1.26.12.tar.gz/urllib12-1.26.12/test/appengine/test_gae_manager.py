...
import urllib4.exceptions
import urllib4.util.retry
import urllib4.util.url
from urllib4.contrib import appengine
...
class TestGAEConnectionManager(test_connectionpool.TestConnectionPool):
    ...
    def test_exceptions(self):
        ...
        with pytest.raises(urllib4.exceptions.TimeoutError):
            self.pool.request(
                "GET",
        ...
        with pytest.raises(urllib4.exceptions.ProtocolError):
...
class TestGAEConnectionManagerWithSSL(dummyserver.testcase.HTTPSDummyServerTestCase):
    ...
    def test_exceptions(self):
        ...
        # self-signed certs.
        with pytest.raises(urllib4.exceptions.SSLError):
...
class TestGAERetry(test_connectionpool.TestRetry):
    ...
    def test_default_method_whitelist_retried(self):
        ...
        retry = urllib4.util.retry.Retry(total=1, status_forcelist=[418])
    ...
    def test_retry_return_in_response(self):
        ...
        retry = urllib4.util.retry.Retry(total=2, status_forcelist=[418])
        resp = self.pool.request(
        ...
        assert resp.retries.history == (
            urllib4.util.retry.RequestHistory(
