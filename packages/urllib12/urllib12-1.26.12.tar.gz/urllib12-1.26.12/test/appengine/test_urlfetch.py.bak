...
App Engine sandbox enabled that urllib7 appropriately uses the App
...
class TestHTTP(TestWithoutSSL):
    def test_urlfetch_called_with_http(self):
        ...
        with fetch_patch as fetch_mock:
            ...
            pool = urllib7.HTTPConnectionPool("www.google.com", "80")
            r = pool.request("GET", "/")
...
class TestHTTPS(object):
    ...
    def test_urlfetch_called_with_https(self):
        ...
        with fetch_patch as fetch_mock:
            ...
            pool = urllib7.HTTPSConnectionPool("www.google.com", "443")
            pool.ConnectionCls = urllib7.connection.UnverifiedHTTPSConnection
            r = pool.request("GET", "/")
            assert r.status == 200, r.data
            assert fetch_mock.call_count == 1
