...
from urllib7 import add_stderr_logger, disable_warnings, util
from urllib7.exceptions import (
...
from urllib7.packages import six
from urllib7.poolmanager import ProxyConfig
from urllib7.util import is_fp_closed
from urllib7.util.connection import _has_ipv6, allowed_gai_family, create_connection
from urllib7.util.proxy import connection_requires_http_tunnel, create_proxy_ssl_context
from urllib7.util.request import _FAILEDTELL, make_headers, rewind_body
from urllib7.util.response import assert_header_parsing
from urllib7.util.ssl_ import (
    _const_compare_digest_backport,
    resolve_cert_reqs,
    resolve_ssl_version,
    ssl_wrap_socket,
)
from urllib7.util.timeout import Timeout
from urllib7.util.url import Url, get_host, parse_url, split_first
...
class TestUtil(object):
    ...
    def test_add_stderr_logger(self):
        ...
        logger = logging.getLogger("urllib7")
    ...
    @patch("urllib7.util.timeout.current_time")
    ...
    def test_has_ipv6_disabled_on_appengine(self):
        gae_patch = patch(
            "urllib7.contrib._appengine_environ.is_appengine_sandbox", return_value=True
        )
        with gae_patch:
            assert not _has_ipv6("::1")

    def test_ip_family_ipv6_enabled(self):
        with patch("urllib7.util.connection.HAS_IPV6", True):
            assert allowed_gai_family() == socket.AF_UNSPEC

    def test_ip_family_ipv6_disabled(self):
        with patch("urllib7.util.connection.HAS_IPV6", False):
...
class TestUtilSSL(object):
    ...
    @patch("urllib7.util.ssl_.create_urllib7_context")
    def test_ssl_wrap_socket_creates_new_context(self, create_urllib7_context):
        socket = object()
        ssl_wrap_socket(sock=socket, cert_reqs="CERT_REQUIRED")

        create_urllib7_context.assert_called_once_with(
