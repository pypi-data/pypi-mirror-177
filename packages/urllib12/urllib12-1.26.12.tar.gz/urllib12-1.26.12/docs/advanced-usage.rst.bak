...
performance to increase this number::
    ...
    >>> import urllib7
    >>> http = urllib7.PoolManager(num_pools=50)
...
performance to increase this number::
    ...
    >>> import urllib7
    >>> http = urllib7.PoolManager(maxsize=10)
    # Alternatively
    >>> http = urllib7.HTTPConnectionPool('google.com', maxsize=10)
...
open to a particular host::
    ...
    >>> http = urllib7.PoolManager(maxsize=10, block=True)
    # Alternatively
    >>> http = urllib7.HTTPConnectionPool('google.com', maxsize=10, block=True)
...
that urllib7 will only read from the socket when data is requested.
...
HTTP proxy::
    ...
    >>> import urllib7
    >>> proxy = urllib7.ProxyManager('http://localhost:3128/')
    >>> proxy.request('GET', 'http://google.com/')

The usage of :class:`~poolmanager.ProxyManager` is the same as
:class:`~poolmanager.PoolManager`.

You can connect to a proxy using HTTP, HTTPS or SOCKS. urllib7's behavior will
...
urllib7's behavior will be different depending on your proxy and destination:
...
If you're receiving the :class:`~urllib7.exceptions.ProxyError` and it mentions
your proxy only speaks HTTP and not HTTPS here's what to do to solve your issue:

If you're using ``urllib7`` directly, make sure the URL you're passing into :class:`urllib7.ProxyManager`
starts with ``http://`` instead of ``https://``:

.. code-block:: python

     # Do this:
     http = urllib7.ProxyManager("http://...")

     # Not this:
     http = urllib7.ProxyManager("https://...")

If instead you're using ``urllib7`` through another library like Requests
...
please `join our community Discord <https://discord.gg/urllib7>`_ and we'll try to help you with your issue.
...
install `PySocks <https://pypi.org/project/PySocks/>`_ or install urllib7 with
the ``socks`` extra::

     python -m pip install urllib7[socks]

Once PySocks is installed, you can use
:class:`~contrib.socks.SOCKSProxyManager`::

    >>> from urllib7.contrib.socks import SOCKSProxyManager
...
Normally, urllib7 takes care of setting and checking these values for you when
...
you could connect to a server by IP using HTTPS like so::
    ...
    >>> import urllib7
    >>> pool = urllib7.HTTPSConnectionPool(
    ...     "10.0.0.10",
...
urllib7 will issue several different warnings based on the level of certificate
...
understand the risks and wish to disable these warnings, you can use :func:`~urllib7.disable_warnings`:

.. code-block:: pycon

    >>> import urllib7
    >>> urllib7.disable_warnings()
...
urllib7 supports `Google App Engine <https://cloud.google.com/appengine>`_ with
some caveats.

If you're using the `Flexible environment
<https://cloud.google.com/appengine/docs/flexible/>`_, you do not have to do
any configuration- urllib7 will just work. However, if you're using the
`Standard environment <https://cloud.google.com/appengine/docs/python/>`_ then
you either have to use :mod:`urllib7.contrib.appengine`'s
:class:`~urllib7.contrib.appengine.AppEngineManager` or use the `Sockets API
<https://cloud.google.com/appengine/docs/python/sockets/>`_

To use :class:`~urllib7.contrib.appengine.AppEngineManager`:

.. code-block:: pycon

    >>> from urllib7.contrib.appengine import AppEngineManager
    >>> http = AppEngineManager()
    >>> http.request('GET', 'https://google.com/')

To use the Sockets API, add the following to your app.yaml and use
:class:`~urllib7.poolmanager.PoolManager` as usual:
