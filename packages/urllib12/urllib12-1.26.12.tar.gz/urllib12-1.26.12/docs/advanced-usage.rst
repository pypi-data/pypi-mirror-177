...
performance to increase this number::
    ...
    >>> import urllib4
    >>> http = urllib4.PoolManager(num_pools=50)
...
performance to increase this number::
    ...
    >>> import urllib4
    >>> http = urllib4.PoolManager(maxsize=10)
    # Alternatively
    >>> http = urllib4.HTTPConnectionPool('google.com', maxsize=10)
...
open to a particular host::
    ...
    >>> http = urllib4.PoolManager(maxsize=10, block=True)
    # Alternatively
    >>> http = urllib4.HTTPConnectionPool('google.com', maxsize=10, block=True)
...
that urllib4 will only read from the socket when data is requested.
...
You can connect to a proxy using HTTP, HTTPS or SOCKS. urllib4's behavior will
...
urllib4's behavior will be different depending on your proxy and destination:
...
If you're receiving the :class:`~urllib4.exceptions.ProxyError` and it mentions
your proxy only speaks HTTP and not HTTPS here's what to do to solve your issue:

If you're using ``urllib4`` directly, make sure the URL you're passing into :class:`urllib4.ProxyManager`
starts with ``http://`` instead of ``https://``:

.. code-block:: python

     # Do this:
     http = urllib4.ProxyManager("http://...")

     # Not this:
     http = urllib4.ProxyManager("https://...")

If instead you're using ``urllib4`` through another library like Requests
...
please `join our community Discord <https://discord.gg/urllib4>`_ and we'll try to help you with your issue.
...
install `PySocks <https://pypi.org/project/PySocks/>`_ or install urllib4 with
the ``socks`` extra::

     python -m pip install urllib4[socks]

Once PySocks is installed, you can use
:class:`~contrib.socks.SOCKSProxyManager`::

    >>> from urllib4.contrib.socks import SOCKSProxyManager
...
Normally, urllib4 takes care of setting and checking these values for you when
...
you could connect to a server by IP using HTTPS like so::
    ...
    >>> import urllib4
    >>> pool = urllib4.HTTPSConnectionPool(
    ...     "10.0.0.10",
...
urllib4 will issue several different warnings based on the level of certificate
...
understand the risks and wish to disable these warnings, you can use :func:`~urllib4.disable_warnings`:
...
urllib4 supports `Google App Engine <https://cloud.google.com/appengine>`_ with
some caveats.

If you're using the `Flexible environment
<https://cloud.google.com/appengine/docs/flexible/>`_, you do not have to do
any configuration- urllib4 will just work. However, if you're using the
`Standard environment <https://cloud.google.com/appengine/docs/python/>`_ then
you either have to use :mod:`urllib4.contrib.appengine`'s
:class:`~urllib4.contrib.appengine.AppEngineManager` or use the `Sockets API
<https://cloud.google.com/appengine/docs/python/sockets/>`_

To use :class:`~urllib4.contrib.appengine.AppEngineManager`:

.. code-block:: pycon

    >>> from urllib4.contrib.appengine import AppEngineManager
    >>> http = AppEngineManager()
    >>> http.request('GET', 'https://google.com/')

To use the Sockets API, add the following to your app.yaml and use
:class:`~urllib4.poolmanager.PoolManager` as usual:
...
:mod:`urllib4.contrib.appengine`.

Brotli Encoding
---------------

Brotli is a compression algorithm created by Google with better compression
than gzip and deflate and is supported by urllib4 if the
`Brotli <https://pypi.org/Brotli>`_ package or
`brotlicffi <https://github.com/python-hyper/brotlicffi>`_ package is installed.
You may also request the package be installed via the ``urllib4[brotli]`` extra:

.. code-block:: bash

    $ python -m pip install urllib4[brotli]
