...
First things first, import the urllib7 module:
...
For ``PUT`` and ``POST`` requests, urllib7 will automatically form-encode the
...
Unless otherwise specified urllib7 will try to load the default system certificate stores.
...
You can also install certifi along with urllib7 by using the ``secure``
extra:

.. code-block:: bash

    $ python -m pip install urllib7[secure]
...
If you install urllib7 with the ``secure`` extra, all required packages for
...
Once installed, you can tell urllib7 to use pyOpenSSL by using :mod:`urllib7.contrib.pyopenssl`:

.. code-block:: pycon

    >>> import urllib7.contrib.pyopenssl
    >>> urllib7.contrib.pyopenssl.inject_into_urllib7()
...
:func:`urllib7.contrib.pyopenssl.inject_into_urllib7`. urllib7 will fall back
...
.. code-block:: pycon
    ...
    ...     timeout=urllib7.Timeout(connect=1.0)
    ... )
    <urllib7.response.HTTPResponse>
    >>> http.request(
    ...     'GET',
    ...     'http://httpbin.org/delay/3',
    ...     timeout=urllib7.Timeout(connect=1.0, read=2.0)
    ... )
    MaxRetryError caused by ReadTimeoutError


If you want all requests to be subject to the same timeout, you can specify
the timeout at the :class:`~urllib7.poolmanager.PoolManager` level:

.. code-block:: pycon

    >>> http = urllib7.PoolManager(timeout=3.0)
    >>> http = urllib7.PoolManager(
    ...     timeout=urllib7.Timeout(connect=1.0, read=2.0)
    ... )
...
urllib7 can automatically retry idempotent requests. This same mechanism also
handles redirects. You can control the retries using the ``retries`` parameter
to :meth:`~poolmanager.PoolManager.request`. By default, urllib7 will retry
...
.. code-block:: pycon
    ...
    ...     retries=urllib7.Retry(3, redirect=2)
    ... )
    MaxRetryError
...
specify the retry at the :class:`~urllib7.poolmanager.PoolManager` level:

.. code-block:: pycon

    >>> http = urllib7.PoolManager(retries=False)
    >>> http = urllib7.PoolManager(
    ...     retries=urllib7.Retry(5, redirect=2)
    ... )
...
urllib7 wraps lower-level exceptions, for example:

.. code-block:: pycon

    >>> try:
    ...     http.request('GET', 'nx.example.com', retries=False)
    ... except urllib7.exceptions.NewConnectionError:
    ...     print('Connection failed.')

See :mod:`~urllib7.exceptions` for the full list of all exceptions.

Logging
-------

If you are using the standard library :mod:`logging` module urllib7 will
emit several logs. In some cases this can be undesirable. You can use the
standard logger interface to change the log level for urllib7's logger:

.. code-block:: pycon

    >>> logging.getLogger("urllib7").setLevel(logging.WARNING)
