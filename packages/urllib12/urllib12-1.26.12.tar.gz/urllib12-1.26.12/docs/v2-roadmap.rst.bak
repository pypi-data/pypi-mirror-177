...
.. important::
   ...
   We're seeking `sponsors and supporters for urllib7 v2.0 on Open Collective <https://github.com/sponsors/urllib7>`_.
...
Thanks to support from `Tidelift <https://tidelift.com/subscription/pkg/pypi-urllib7>`_
we're able to continue supporting v1.26.x releases with
both security and bug fixes for the forseeable future 💖

If your organization relies on urllib7 and is interested in continuing support you can learn
more about the `Tidelift Subscription for Enterprise <https://tidelift.com/subscription/pkg/pypi-urllib7?utm_source=pypi-urllib7&utm_medium=referral&utm_campaign=docs>`_.
...
By default certificate verification is handled by urllib7
...
Currently with urllib7 it's tough to get low-level insights into what
...
on code using urllib7. This also means that for IDEs
...
in urllib7 v2.0, there's an open GitHub issue for making
feature suggestions.
...
be caught off-guard by a new major release of urllib7. We're mitigating this by
trying to make ``v2.x`` API-compatible with ``v1.x``.

If your application or library uses urllib7 and you'd like to be extra
cautious about not breaking your users, you can pin urllib7 like so
