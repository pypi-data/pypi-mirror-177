...
First things first, import the urllib4 module:
...
For ``PUT`` and ``POST`` requests, urllib4 will automatically form-encode the
...
Unless otherwise specified urllib4 will try to load the default system certificate stores.
...
You can also install certifi along with urllib4 by using the ``secure``
extra:

.. code-block:: bash

    $ python -m pip install urllib4[secure]
...
If you install urllib4 with the ``secure`` extra, all required packages for
...
Once installed, you can tell urllib4 to use pyOpenSSL by using :mod:`urllib4.contrib.pyopenssl`:
...
:func:`urllib4.contrib.pyopenssl.inject_into_urllib4`. urllib4 will fall back
...
.. code-block:: pycon
    ...
    ...     timeout=urllib4.Timeout(connect=1.0)
    ... )
    <urllib4.response.HTTPResponse>
    >>> http.request(
    ...     'GET',
    ...     'http://httpbin.org/delay/3',
    ...     timeout=urllib4.Timeout(connect=1.0, read=2.0)
    ... )
    MaxRetryError caused by ReadTimeoutError


If you want all requests to be subject to the same timeout, you can specify
the timeout at the :class:`~urllib4.poolmanager.PoolManager` level:

.. code-block:: pycon

    >>> http = urllib4.PoolManager(timeout=3.0)
    >>> http = urllib4.PoolManager(
    ...     timeout=urllib4.Timeout(connect=1.0, read=2.0)
    ... )
...
urllib4 can automatically retry idempotent requests. This same mechanism also
handles redirects. You can control the retries using the ``retries`` parameter
to :meth:`~poolmanager.PoolManager.request`. By default, urllib4 will retry
...
.. code-block:: pycon
    ...
    ...     retries=urllib4.Retry(3, redirect=2)
    ... )
    MaxRetryError
...
specify the retry at the :class:`~urllib4.poolmanager.PoolManager` level:

.. code-block:: pycon

    >>> http = urllib4.PoolManager(retries=False)
    >>> http = urllib4.PoolManager(
    ...     retries=urllib4.Retry(5, redirect=2)
    ... )
...
urllib4 wraps lower-level exceptions, for example:

.. code-block:: pycon

    >>> try:
    ...     http.request('GET', 'nx.example.com', retries=False)
    ... except urllib4.exceptions.NewConnectionError:
    ...     print('Connection failed.')

See :mod:`~urllib4.exceptions` for the full list of all exceptions.

Logging
-------

If you are using the standard library :mod:`logging` module urllib4 will
emit several logs. In some cases this can be undesirable. You can use the
standard logger interface to change the log level for urllib4's logger:

.. code-block:: pycon

    >>> logging.getLogger("urllib4").setLevel(logging.WARNING)
