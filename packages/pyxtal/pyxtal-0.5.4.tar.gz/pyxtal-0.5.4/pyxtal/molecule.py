...
def find_rotor_from_smile(smile):
    ...
    if smile in ["Cl-", "F-", "Br-", "I-", "Li+", "Na+"]:
        ...
        for t in tmp:
            ...
            if not b.IsInRing():
                reg_torsions.append(t)
            else:
                a = mol.GetBondBetweenAtoms(i, j)
                c = mol.GetBondBetweenAtoms(k, l)
                if b.GetIsAromatic() and a.IsInRing() and c.IsInRing():
                    if not a.GetIsAromatic() and not c.GetIsAromatic():
                        ring_torsions.append(t)
        #if len(torsions) > 6: torsions[1] = (4, 7, 10, 15)
        return reg_torsions, ring_torsions

def check_rings(smile):
    """
    check if a non-aromatic ring exists
    """


def generate_ring_conformer(smile):
