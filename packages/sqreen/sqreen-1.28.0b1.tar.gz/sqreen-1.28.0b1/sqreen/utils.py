...
def parse_form_params(qs):
    """Return a dict of form data after Basic HTTP form parsing."""
    qs = to_unicode_safe(qs).replace("+", " ")
    params = defaultdict(list)
    for item in qs.split("&"):
        k, equal, v = item.partition("=")
        if equal:
            params[to_unicode_safe(unquote_to_bytes(k))].append(to_unicode_safe(unquote_to_bytes(v)))
    return dict(params)
