...
def run_command(
    ...
    debug_print("exit_code", exit_code, verbose=verbose, long=False)
    debug_print("stdout", completed_subprocess.stdout, verbose=verbose, long=True)
    debug_print("stderr", completed_subprocess.stderr, verbose=verbose, long=True)
    return exit_code
...
def run_bx_job(
    ...
    timeout_s: int = 15 * 60,
    verbose: bool = True,
    acceptable_exit_codes: tuple[int, ...] | None = None,
):
    ...
    # raise error if acceptable exit codes are defined
    if acceptable_exit_codes is not None:
        if exit_code not in acceptable_exit_codes:
            ...
            Error message:
                {m}
            """.format(
                c=exit_code, a=acceptable_exit_codes, m=error_message
            )
