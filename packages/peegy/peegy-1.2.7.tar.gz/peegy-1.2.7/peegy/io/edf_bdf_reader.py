...
def read_bdf_channel(header,
    ...
    return data_channel * gain[0:-1] * unit_scaling * data_unit, event_channel * gain[-1], (unique_events, unique_codes)
...
def read_edf_channel(header,
    ...
    unit_scaling = np.array([u.Quantity(1, header['physical_dimension'][ch.idx]).to(data_unit).value
                             for ch in channels[0:-1]])
    return data_channel * unit_scaling * data_unit * gain[0:-1], event_channel, (unique_events, unique_codes)
