...
class RemoteAutoPipelines(BaseAutoPipelines):
    ...
    max_num_daub_ensembles: int, optional
        ...
        to use in pipeline composition. The default is None that means the true default value will be determined by the
        internal different algorithms, where only the highest ranked by model selection algorithm type is used.
