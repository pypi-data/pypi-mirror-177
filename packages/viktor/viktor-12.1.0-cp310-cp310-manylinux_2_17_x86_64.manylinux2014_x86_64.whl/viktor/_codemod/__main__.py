...
from typing import Iterable, Tuple, Type, List, Dict
...
def fix(paths: List[str], upgrades: List[int], apply: bool, keep_original: bool):
    """ Initiates the codemod for specified upgrade id.

    :param paths: Relevant paths of the application.
    ...
    key_paths = _get_key_paths(paths)
    ...
    for upgrade_id in upgrades:
        ...
        patched_files = []
        if upgrade_id == 46:
            from viktor._codemod.modifiers.U46 import specific_fix
            specific_fix(key_paths, apply, keep_original, patched_files)
        elif upgrade_id == 47:
            from viktor._codemod.modifiers.U47 import specific_fix
            specific_fix(key_paths, apply, keep_original, patched_files)
        else:
            _fix(key_paths, upgrade_id, apply, keep_original, patched_files)
...
def _fix(key_paths: Dict[str, Path], upgrade_id: int, apply: bool, keep_original: bool, patched_files: list):

    visitor, transformer = _retrieve_modifier(upgrade_id)

    for path_ in _python_file_paths(key_paths['app_dir']):
        rel_path = path_.relative_to(key_paths['root_dir'])
...
def _get_key_paths(paths: List[str]) -> Dict[str, Path]:
    """ Returns the key paths that build up the app directory, for example:

        {
            'root_dir':             Path('my_app/'),
            'app_dir':              Path('my_app/app/'),
            'requirements_file':    Path('my_app/requirements.txt'),
            'manifest_file':        Path('my_app/manifest/manifest.yml'),
        }

    """
    key_paths = {}

    # if provided path is a file, append parent directory instead
    directories: List[Path] = [Path(path).parent if os.path.isfile(path) else Path(path) for path in paths]

    if len(directories) > 1:
        root_path = Path(os.path.commonpath(directories))
    else:
        root_path = directories[0]

    key_paths['root_dir'] = root_path
    key_paths['app_dir'] = root_path / 'app'

    for dirpath, _, filenames in chain.from_iterable(os.walk(path) for path in directories):
        for filename in filenames:
            if filename == 'requirements.txt':
                key_paths['requirements_file'] = Path(dirpath) / filename
            if filename == 'manifest.yml':
                key_paths['manifest_file'] = Path(dirpath) / filename
...
if __name__ == '__main__':
    ...
    if args.upgrade is not None:
        _upgrades = list(args.upgrade)
        _paths = list(args.source_path)
        fix(_paths, _upgrades, args.apply, args.keep_original)
