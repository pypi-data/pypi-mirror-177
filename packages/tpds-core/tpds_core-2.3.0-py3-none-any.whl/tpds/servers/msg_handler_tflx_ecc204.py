    ...
    try:
        ...
        if cert_data.get("cert_type") == "custCert":
            ...
            if reply == QMessageBox.Yes:
                ...
                cert_dlg = QFileDialog(None, caption="Select your Signer CA Key", filter="*.key")
                ...
                if cert_file == QDialog.Accepted:
                    ...
                    cert_dlg = QFileDialog(
                        None, caption="Select your Device CA Key", filter="*.key"
                    )
                    cert_file = cert_dlg.exec()
                    if cert_file == QDialog.Accepted:
            ...
            if signer_ca_key is None:
                signer_ca_key = os.path.join(
                    str(QDir.homePath()), "Downloads", "TPDS_Downloads", "proto_signer_ca.key"
                )
                key = TPAsymmetricKey()
            ...
            if device_ca_key is None:
                device_ca_key = os.path.join(
                    str(QDir.homePath()), "Downloads", "TPDS_Downloads", "proto_device_ca.key"
                )
                key = TPAsymmetricKey()
