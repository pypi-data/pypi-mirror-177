...
def build_case_multiprocess(operation_name):
    ...
    if all(suffix not in operation_name for suffix in ignored_suffix_in_bpa_folder):
        ...
        pre_fault_dat_path = operation_path + '/' + operation_name + '.temp_dat'
        pre_fault_dat_lines = check_pre_fault_dat(pre_fault_dat_path, pre_cut_line_list)
        save_lines_to_file(pre_fault_dat_lines, path_dats + operation_name + '/', operation_name + '.temp_dat')
        ...
        for file in os.listdir(operation_path):
            if '.SWX' in file:  # x((x)-(x)).swi
                ...
                if len(swx_info) < 3:  # 有些swx直接不要了
                    continue
                # swx有两种命名格式
                if 'Bus' in swx_info[1]:  # 第一种： k((busa——busb)(x)), k:n_sample-k, x:morning/b短路端
                    ...
                    for i, branch in enumerate(line_text):
                        if (branch[1] == bus_a and branch[2] == bus_b) or (branch[2] == bus_a and branch[1] == bus_b):
                            post_cut_line = i
                            break
                else:  # 第二种： k(l-b), k:n_sample-k, l:fault_line, b:fault_bus
                ...
                save_lines_to_file(post_fault_dat_lines, path_dats + operation_name_post + '/',
                                   operation_name_post + '.temp_dat')

                # 3. 检查生成的文件是不是太小
                file_too_small = ''
                if os.path.getsize(path_dats + operation_name + '/' + operation_name + '.temp_dat') < 4859:
                    file_too_small += operation_name + ' '
                if os.path.getsize(path_dats + operation_name_post + '/' + operation_name_post + '.temp_dat') < 4859:
