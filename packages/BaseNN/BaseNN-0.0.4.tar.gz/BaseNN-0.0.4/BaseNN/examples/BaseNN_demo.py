...
def only_infer_demo():
    ...
    model = nn()
    checkpoint = 'iris_ckpt/basenn.pkl'
...
def iris_train_test():
    ...
    model.load_dataset(x, y)
    # model.print_model()
    model.train(lr=0.01, epochs=5000, save_fold='iris_ckpt')
...
def mnist_test(one_img):
    ...
    # 模型
    model = nn() # 声明模型
    checkpoint = 'mn_ckpt/basenn.pkl' # 载入模型
...
def visual_feature_demo():
    ...
    model.visual_feature(img, save_fold="/home/user/桌面/vis_res")
