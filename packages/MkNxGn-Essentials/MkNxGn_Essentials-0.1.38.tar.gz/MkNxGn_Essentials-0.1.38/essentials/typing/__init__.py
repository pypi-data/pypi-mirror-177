    ...
    if type(input) == list:
        ...
        for key in input:
            if type(input[key]) == dict:
                parsed, subbs = parse_dictList(f"{key}__dict", input[key])
                subs.update(subbs)
                subs.update(parsed)
                fields[key] = (f"{key}__dict", f"{key}__dict")
            elif type(input[key]) == list:
                parsed, subbs = parse_dictList(f"{key}__list", input[key])
                subs.update(subbs)
                subs.update(parsed)
                fields[f"{key}"] = (f"List[{key}__list]", f"{key}__list")
            else:
...
def create_class(name, data:dict, indent=0):
    ...
    for key in data:
        if type(data[key]) == list:
            line = f"{INDENTCHAR*indent*INDENTCHAR_COUNT}{INDENTCHAR*INDENTCHAR_COUNT*2}self.{key}:Union[{','.join(data[key])},None] = None"
            classData.append(line)
        elif type(data[key]) == tuple:
            if "List" in data[key][0]:
                line = f"{INDENTCHAR*indent*INDENTCHAR_COUNT}{INDENTCHAR*INDENTCHAR_COUNT*2}self.{key}:{data[key][0]} = [{data[key][1]}(x) for x in data.get('{key}', [])]"
            else:
                line = f"{INDENTCHAR*indent*INDENTCHAR_COUNT}{INDENTCHAR*INDENTCHAR_COUNT*2}self.{key}:{data[key][0]} = {data[key][1]}(data.get('{key}', {{}}))"
            lasts.append(line)
        else:
            line = f"{INDENTCHAR*indent*INDENTCHAR_COUNT}{INDENTCHAR*INDENTCHAR_COUNT*2}self.{key}:{data[key]} = {DEFAULTS[data[key]]}"
            classData.append(line)
...
def create_typing_file(name, data:dict, fp:str):
    """
