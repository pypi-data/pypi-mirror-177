...
def JsonObj(data):
    ...
    try:
        if sys.version >= "3.0":
            ...
            return compat_json(
                json.loads(str(data), object_hook=compat_json), ignore_dicts=True
            )
...
class Optional:
    ...
    def __bytes__(self):
        if not self.data:
            ...
            return (
                compat_bytes(Bool(1)) + compat_bytes(self.data)
                if compat_bytes(self.data)
                else compat_bytes(Bool(0))
            )
