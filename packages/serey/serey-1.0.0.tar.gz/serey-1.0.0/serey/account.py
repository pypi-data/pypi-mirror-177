...
from sereybase.exceptions import AccountDoesNotExistsException
...
class Account(dict):
    ...
    def refresh(self):
        account = self.sereyd.get_account(self.name)
        if not account:
    ...
    def converter(self):
        if not self._converter:
            self._converter = Converter(self.sereyd)
    ...
    def get_balances(self):
        ...
        totals = {
            "HIVE": sum([available["HIVE"], savings["HIVE"], rewards["HIVE"]]),
            "HBD": sum([available["HBD"], savings["HBD"], rewards["HBD"]]),
    ...
    def _get_followers(self, direction="follower", last_user=""):
        if direction == "follower":
            ...
            followers = self.sereyd.get_followers(self.name, last_user, "blog", 100)
        elif direction == "following":
            followers = self.sereyd.get_following(self.name, last_user, "blog", 100)
    ...
    def virtual_op_count(self):
        try:
            last_item = self.sereyd.get_account_history(self.name, -1, 1)[0][0]
        except IndexError:
            return 0
        else:
            return last_item
    ...
    def get_account_votes(self):
        return self.sereyd.get_account_votes(self.name)

    def get_withdraw_routes(self):
        return self.sereyd.get_withdraw_routes(self.name, "all")

    def get_conversion_requests(self):
        return self.sereyd.get_conversion_requests(self.name)
    ...
    def get_account_history(
        ...
        """A generator over sereyd.get_account_history.
        ...
        Args:
            ...
            raw_output (bool): (Defaults to False). If True, return history in
                sereyd format (unchanged).
        """
        history = self.sereyd.get_account_history(self.name, index, limit)
