...
class Converter(object):
    ...
    :param Sereyd blockchain_instance: Hived() instance to
    ...
    def hbd_median_price(self):
        ...
        return (
            Amount(
                self.sereyd.get_feed_history()["current_median_history"]["base"]
            ).amount
            / Amount(
                self.sereyd.get_feed_history()["current_median_history"]["quote"]
            ).amount
        )

    def hive_per_mvests(self):
        """Obtain HIVE/MVESTS ratio"""
        info = self.sereyd.get_dynamic_global_properties()
    ...
    def hp_to_rshares(self, hp, voting_power=10000, vote_pct=10000):
        ...
        # get props
        props = self.sereyd.get_dynamic_global_properties()

        # determine voting power used
        used_power = int((voting_power * vote_pct) / 10000)
        max_vote_denom = (
            props["vote_power_reserve_rate"] * (5 * 60 * 60 * 24) / (60 * 60 * 24)
        )
    ...
    def hbd_to_rshares(self, hbd_payout):
        ...
        reward_fund = self.sereyd.get_reward_fund()
