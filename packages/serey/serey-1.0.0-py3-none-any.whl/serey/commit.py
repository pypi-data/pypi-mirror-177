...
from sereybase import memo
from sereybase import operations
from sereybase.account import PrivateKey, PublicKey
from sereybase.exceptions import AccountExistsException, MissingKeyError
...
class Commit(object):
    ...
    :param Sereyd sereyd: Hived node to connect to*
    ...
    * **Force keys**: This more is for advanced users and
    ...
    def __init__(self, blockchain_instance=None, no_broadcast=False, **kwargs):
        ...
        self.wallet = Wallet(self.sereyd, **kwargs)
    ...
    def finalizeOp(self, ops, account, permission):
        ...
        tx = TransactionBuilder(
            None,
            blockchain_instance=self.sereyd,
    ...
    def create_account(
        ...
        delegation_fee_hive="0 SRY",
        creator=None,
    ):
        """Create new account in Hive

            The brainkey/password can be used to recover all generated keys
            (see `sereybase.account` for more details.
        ...
        try:
            Account(account_name, blockchain_instance=self.sereyd)
        ...
        from sereybase.account import PasswordKey, PublicKey
        ...
        if password:
            ...
            posting_pubkey = PublicKey(
                posting_key, prefix=self.sereyd.chain_params["prefix"]
            )
            active_pubkey = PublicKey(
                active_key, prefix=self.sereyd.chain_params["prefix"]
            )
            owner_pubkey = PublicKey(
                owner_key, prefix=self.sereyd.chain_params["prefix"]
            )
            memo_pubkey = PublicKey(memo_key, prefix=self.sereyd.chain_params["prefix"])
        else:
            raise ValueError(
                "Call incomplete! Provide either a password or public keys!"
            )

        owner = format(owner_pubkey, self.sereyd.chain_params["prefix"])
        active = format(active_pubkey, self.sereyd.chain_params["prefix"])
        posting = format(posting_pubkey, self.sereyd.chain_params["prefix"])
        memo = format(memo_pubkey, self.sereyd.chain_params["prefix"])
        ...
        props = self.sereyd.get_chain_properties()
        required_fee_hive = Amount(props["account_creation_fee"]).amount * 30

        required_fee_vests = 0
        delegation_fee_hive = Amount(delegation_fee_hive).amount
        if delegation_fee_hive:
            ...
            if delegation_fee_hive < 1:
                raise ValueError(
                    "When creating account with delegation, at least "
                    + "1 SRY in fee must be paid."
                )
        ...
        s = {
            "creator": creator,
            "fee": "%s SRY" % (delegation_fee_hive or required_fee_hive),
            ...
            "prefix": self.sereyd.chain_params["prefix"],
        }
    ...
    def transfer(self, to, amount, asset, memo="", account=None):
        """Transfer SRY to another account.
        ...
        if memo and memo[0] == "#":
            from sereybase import memo as Memo

            memo_wif = self.wallet.getMemoKeyForAccount(account)
            if not memo_wif:
                raise MissingKeyError("Memo key for %s missing!" % account)
            to_account = Account(to, blockchain_instance=self.sereyd)
            nonce = random.getrandbits(64)
            memo = Memo.encode_memo(
                PrivateKey(memo_wif),
                PublicKey(
                    to_account["memo_key"], prefix=self.sereyd.chain_params["prefix"]
                ),
                nonce,
                memo,
                prefix=self.sereyd.chain_params["prefix"],
    ...
    def transfer_to_vesting(self, amount, to=None, account=None):
        ...
        :param float amount: number of SRY to vest
    ...
    def convert(self, amount, account=None, request_id=None):
        """Convert HBD to SRY (takes 3.5 days to settle)
    ...
    def transfer_to_savings(self, amount, asset, memo, to=None, account=None):
        """Transfer SRY into a 'savings' account.

        :param float amount: SRY
        :param float asset: 'SRY'
    ...
    def transfer_from_savings(
        self, amount, asset, memo, request_id=None, to=None, account=None
    ):
        """Withdraw SRY from 'savings' account.

        :param float amount: SRY amount
        :param float asset: 'SRY'
        :param str memo: (optional) Memo
    ...
    def claim_reward_balance(
        ...
        Args:
            reward_hive (string): Amount of SRY you would like to claim.
        ...
        if none(
            ...
            a = Account(account, blockchain_instance=self.sereyd)
    ...
    def witness_set_properties(self, signing_key, props, account=None):
        ...
        props_list = [
            [
                ...
                repr(
                    PrivateKey(
                        signing_key, prefix=self.sereyd.chain_params["prefix"]
                    ).pubkey
                ),
            ]
        ]
        ...
        print({"owner": account, "props": props_list, "extensions": []})
    ...
    def interest(self, account):
        ...
        """
        account = Account(account, blockchain_instance=self.sereyd)
        last_payment = fmt_time_string(account["hbd_last_interest_payment"])
        next_payment = last_payment + timedelta(days=30)
        interest_rate = (
            self.sereyd.get_dynamic_global_properties()["hbd_interest_rate"] / 100
        )  # percent
        interest_amount = (
            (interest_rate / 100)
    ...
    def allow(
        ...
        try:
            ...
            try:
                foreign_account = Account(foreign, blockchain_instance=self.sereyd)
    ...
    def disallow(self, foreign, permission="posting", account=None, threshold=None):
        ...
        try:
            pubkey = PublicKey(foreign, prefix=self.sereyd.chain_params["prefix"])
            affected_items = list(
