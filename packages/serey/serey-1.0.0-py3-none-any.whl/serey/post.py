...
from sereybase.exceptions import (
    PostDoesNotExist,
    VotingInvalidOnArcsereydPost,
)
from sereybase.operations import CommentOptions
...
class Post(dict):
    ...
    Args:
        ...
        blockchain_instance (Sereyd): Hived node to connect to

    """

    def __init__(self, post, blockchain_instance=None):
        self.sereyd = blockchain_instance or shared_blockchain_instance()
        self.commit = Commit(blockchain_instance=self.sereyd)
        ...
        if isinstance(post, str):  # From identifier
            ...
            raise ValueError(
                "Post expects an identifier or a dict " "with author and permlink!"
            )
    ...
    def refresh(self):
        ...
        post = self.sereyd.get_content(post_author, post_permlink)
    ...
    def get_replies(self):
        ...
        post_author, post_permlink = resolve_identifier(self.identifier)
        replies = self.sereyd.get_content_replies(post_author, post_permlink)
    ...
    def reward(self):
        ...
        return Amount(self.get("total_payout_value", "0 HBD")) + Amount(
            self.get("pending_payout_value", "0 HBD")
        )
    ...
    def export(self):
        ...
        safe_dict = remove_from_dict(self, ["sereyd", "commit"])
    ...
    def vote(self, weight, voter=None):
        ...
        """
        # Test if post is arcsereyd, if so, voting is worthless but just
        # pollutes the blockchain and account history
        if self.get("net_rshares", None) == None:
            raise VotingInvalidOnArcsereydPost
        ...
        op = CommentOptions(
            **{
                ...
                "max_accepted_payout": options.get(
                    "max_accepted_payout", str(self["max_accepted_payout"])
                ),
                "percent_hive_dollars": int(
                    options.get(
                        "percent_hive_dollars", self["percent_hive_dollars"] / 100
                    )
                    * 100
                ),
