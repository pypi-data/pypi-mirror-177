...
from sereybase import bip38
from sereybase.account import PrivateKey
from sereybase.exceptions import InvalidWifError, WalletExists
...
class Wallet:
    ...
    :param Hive rpc: RPC connection to a Hive node
    ...
    * **Wallet Database**: Here, hivelibs loads the keys from the
    ...
    * **Force keys**: This more is for advanced users and
    ...
    def __init__(self, blockchain_instance=None, **kwargs):
        ...
        # Prefix
        if self.sereyd:
            self.prefix = self.sereyd.chain_params["prefix"]
        else:
        ...
        if "keys" in kwargs:
            ...
            keyStorage
            """
            from sereybase.storage import keyStorage, KeyEncryptionKey
    ...
    def encrypt_wif(self, wif):
        """Encrypt a wif key"""
        self.unlock()
        return format(bip38.encrypt(PrivateKey(wif), self.decryptedKEK), "encwif")
    ...
    def getOwnerKeyForAccount(self, name):
        ...
        if "owner" in Wallet.keyMap:
            ...
            account = self.sereyd.get_account(name)
            if not account:
                return
    ...
    def getAccountFromPublicKey(self, pub):
        ...
        names = self.sereyd.call("get_key_references", [pub], api="account_by_key_api")[
            0
        ]
        if not names:
            return None
    ...
    def getAccount(self, pub):
        ...
        if not name:
            ...
            return {"name": name, "account": account, "type": keyType, "pubkey": pub}
