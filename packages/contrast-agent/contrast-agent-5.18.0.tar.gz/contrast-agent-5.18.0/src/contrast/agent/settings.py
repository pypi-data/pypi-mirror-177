...
from contrast.agent.exclusions import Exclusions, ExclusionMapper
...
class ApplicationSettings:
    ...
    @cached_property
    def exclusions(self):
        return self._raw_settings.get("settings", {}).get("exceptions")
...
class Settings(Singleton, SettingsREPMixin):
    ...
    def load_exclusions_from_server(self, application_settings):
        if (
    ...
    def apply_ts_app_settings(self, response_json):
        ...
        if self.direct_ts_app_settings.exclusions:
            em = ExclusionMapper(self.direct_ts_app_settings.exclusions)

            self.load_exclusions_from_server(em)
    ...
    def process_application_settings(self, data):
        ...
        if not self.config.is_service_bypassed:
            self.load_exclusions_from_server(self.application_settings)
