...
@expose(Namespace.PULSES)
@no_graph_parameters(alternatives="`graph.pulses.square_pulse_pwc`")
def square_pulse(
    duration: float,
    ...
    :func:`.pulses.gaussian_pulse` : Create a `Pulse` object representing a Gaussian pulse.
    :func:`.pulses.sech_pulse` : Create a `Pulse` object representing a hyperbolic secant pulse.
...
@expose(Namespace.PULSES)
@no_graph_parameters(alternatives="`graph.pulses.cosine_pwc`")
def cosine_pulse(
    duration: float,
    amplitude: float | complex,
    drag: float = 0.0,
...
@expose(Namespace.PULSES)
@no_graph_parameters(
    alternatives="`graph.pulses.sinusoid_pwc` or `graph.pulses.sinusoid_stf`"
)
def sinusoid(
    duration: float,
    amplitude: float | complex,
...
@expose(Namespace.PULSES)
@no_graph_parameters(
    alternatives="`graph.pulses.hann_series_pwc` or `graph.pulses.hann_series_stf`"
)
def hann_series(duration: float, coefficients: np.ndarray) -> Pulse:
    r"""
    Create a `Pulse` object representing a sum of Hann window functions.
...
@expose(Namespace.PULSES)
@no_graph_parameters(
    alternatives="`graph.pulses.linear_ramp_pwc` or `graph.pulses.linear_ramp_stf`"
)
def linear_ramp(
    duration: float,
    ...
    r"""
    Create a `Pulse` object representing a linear ramp.
    ...
    See Also
    --------
    :func:`.pulses.linear_ramp_pwc` : Graph operation to create a `Pwc` representing a linear ramp.
    :func:`.pulses.linear_ramp_stf` : Graph operation to create a `Stf` representing a linear ramp.
    :func:`.pulses.tanh_ramp` : Create a `Pulse` object representing a hyperbolic tangent ramp.
    ...
    >>> pulse = qctrl.pulses.linear_ramp(
    ...     duration=4, end_value=2, start_time=1, end_time=3
    ... )
    >>> pulse.export_with_time_step(time_step=0.25)
    ...
    return Pulse(function=_linear_ramp, duration=duration)


@expose(Namespace.PULSES)
@no_graph_parameters(
    alternatives="`graph.pulses.tanh_ramp_pwc` or `graph.pulses.tanh_ramp_stf`"
)
def tanh_ramp(
    duration: float,
    end_value: float | complex,
    ...
    r"""
    Create a `Pulse` object representing a hyperbolic tangent ramp.
    ...
    See Also
    --------
    :func:`.pulses.linear_ramp` : Create a `Pulse` object representing a linear ramp.
    :func:`.pulses.tanh_ramp_pwc` :
        Graph operation to create a `Pwc` representing a hyperbolic tangent ramp.
    :func:`.pulses.tanh_ramp_stf` :
    ...
    >>> pulse = qctrl.pulses.tanh_ramp(
    ...     duration=4, end_value=2, start_value=1, ramp_duration=0.4, center_time=2.
    ... )
    >>> pulse.export_with_time_step(time_step=0.4)
    ...
    return Pulse(function=_tanh_ramp, duration=duration)


@expose(Namespace.PULSES)
@no_graph_parameters(
    alternatives="`graph.pulses.sech_pulse_pwc` or `graph.pulses.sech_pulse_stf`"
)
def sech_pulse(
    duration: float,
    amplitude: float | complex,
...
@expose(Namespace.PULSES)
@no_graph_parameters(
    alternatives="`graph.pulses.gaussian_pulse_pwc` or `graph.pulses.gaussian_pulse_stf`"
)
def gaussian_pulse(
    duration: float,
    amplitude: float | complex,
    ...
    return Pulse(function=_gaussian_pulse, duration=duration)
