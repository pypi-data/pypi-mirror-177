...
class SimpleXmlRpcServerEngine(serviceutils.ServerEngineBase):
...
class SimpleRpcApplication(DaemonApplication):
    ...
    def main(self):
        ...
        ServerEngineClass = magic_import.import_from_string(server_engine_class)
        if not ServerEngineClass:
            _logger.fatal("SimpleRpcServer.make_server failed: server engine class {server_engine_class} not found...".format(server_engine_class=server_engine_class))
            sys.exit(1)
