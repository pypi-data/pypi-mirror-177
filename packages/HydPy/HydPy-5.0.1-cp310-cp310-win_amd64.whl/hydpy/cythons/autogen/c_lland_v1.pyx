...
cdef class Model:
    ...
    cpdef inline void calc_et0_v1(self)  nogil:
        cdef int k
        for k in range(self.parameters.control.nhru):
            self.sequences.fluxes.et0[k] = self.parameters.control.ke[k] * (                ((8.64 * self.sequences.inputs.globalradiation + 93.0 * self.parameters.control.kf[k]) * (self.sequences.fluxes.tkor[k] + 22.0))                / (                    165.0                    * (self.sequences.fluxes.tkor[k] + 123.0)                    * (1.0 + 0.00019 * min(self.parameters.control.hnn[k], 600.0))                )            )
