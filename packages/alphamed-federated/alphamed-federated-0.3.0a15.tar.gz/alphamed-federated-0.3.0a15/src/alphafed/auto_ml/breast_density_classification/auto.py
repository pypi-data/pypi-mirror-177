...
class AutoInception(AutoModel):
    ...
    def _build_model(self):
        ...
        self._model: nn.Module = model_class(num_classes=pretrained_num_classes,
                                             init_weights=False)
...
class InceptionFedAvgScheduler(AutoFedAvgScheduler):
    ...
    def load_state_dict(self, state_dict: Dict[str, torch.Tensor]):
        self.auto_proxy.load_state_dict(state_dict)
...
class BreastDensityClassificationFamily(AutoModelFamily):
    ...
    _NAME_MAP = {
        (BREAST_DENSITY_CLASSIFICATION, 1): AutoInception,
        (BREAST_DENSITY_CLASSIFICATION_FED_AVG, 1): AutoInceptionFedAvg,
    }
