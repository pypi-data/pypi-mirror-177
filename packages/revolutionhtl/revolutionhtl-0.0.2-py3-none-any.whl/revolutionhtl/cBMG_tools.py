...
def LRT_from_cBMG(G, speciesAttr, geneAttr):
    ...
    if len(CCs)>1:
        CC0= CCs[0]
        are_equal= all((CC0==CCi for CCi in CCs[1:]))
        if not are_equal:
            raise ValueError('The connected components contain different color sets')
    # Verificar si es sink-free
    have_matches= all((x>0 for x in dict(G.out_degree()).values()))
    if not have_matches:
        raise ValueError('The graph should be sink free')
