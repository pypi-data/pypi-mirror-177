...
from tmp.jsl_pre_processor import JslPreprocessor
...
def execute_nb_as_nb(nb_path, out_file, work_dir=None):
    if not work_dir:
        work_dir = settings.testing_work_dir
    Path(settings.testing_work_dir, ).mkdir(exist_ok=True, parents=True)

    # TODO keep run on error?

    # Load  file and custom Preprocessor
    ...
    if exception_message:
        failed = True
    return dict(failed=failed, exception_message=str(failed),
                nb_path=nb_path, out_file=out_file, work_dir=work_dir)
