...
class ValidationManager(object):
    ...
    def make_data(self, status):
        ...
        for name, info in self.input_assets.items():
            if info['shape'] is not None:
                arr = np.random.rand(*info['shape'], dtype=info['dtype'])
                bts = arr.astype().tobytes()
    ...
    def test_response(self, resp, result, n_tests) -> int:
        ...
        dtype_state = "Passed"
        n_tests += 1
        if expected_asset['dtype'] is not None and resp.header.dtype != expected_asset['dtype']:
            result.append(
        ...
        n_tests += 1
        try:
            if expected_asset['shape'] is not None:
        ...
        shape_state = "Passed"
        n_tests += 1
        if expected_asset['shape'] is not None and test_arr.shape != tuple(expected_asset['shape']):
            result.append(
