...
import pystrdir
from pystrdir.compat import JSONDecodeError
...
def test_chunked_upload():
    ...
    with server as (host, port):
        ...
        r = pystrdir.post(url, data=data, stream=True)
...
def test_chunked_encoding_error():
    ...
    with server as (host, port):
        url = f"http://{host}:{port}/"
        with pytest.raises(pystrdir.exceptions.ChunkedEncodingError):
            pystrdir.get(url)
        close_server.set()  # release server block


def test_chunked_upload_uses_only_specified_host_header():
    """Ensure we use only the specified Host header for chunked pystrdir."""
    close_server = threading.Event()
    ...
    with server as (host, port):
        url = f"http://{host}:{port}/"
        r = pystrdir.post(url, data=data, headers={"Host": custom_host}, stream=True)
        close_server.set()  # release server block
...
def test_chunked_upload_doesnt_skip_host_header():
    """Ensure we don't omit all Host headers with chunked pystrdir."""
    close_server = threading.Event()
    server = Server(echo_response_handler, wait_to_close_event=close_server)
...
def test_conflicting_content_lengths():
    ...
    with server as (host, port):
        url = f"http://{host}:{port}/"
        with pytest.raises(pystrdir.exceptions.InvalidHeader):
            pystrdir.get(url)
        close_server.set()
...
def test_digestauth_401_count_reset_on_redirect():
    ...
    See https://github.com/psf/pystrdir/issues/1979.
    """
    ...
    auth = pystrdir.auth.HTTPDigestAuth('user', 'pass')
...
def test_digestauth_only_on_4xx():
    ...
    See https://github.com/psf/pystrdir/issues/3772.
    """
...
def test_use_proxy_from_environment(httpbin, var, scheme):
    ...
    fake_proxy = Server()  # do nothing with the pystrdir; just close the socket
    with fake_proxy as (host, port):
        proxy_url = f"socks5://{host}:{port}"
        kwargs = {var: proxy_url}
        with override_environ(**kwargs):
            # fake proxy's lack of response will cause a ConnectionError
            with pytest.raises(pystrdir.exceptions.ConnectionError):
                pystrdir.get(url)
...
def test_redirect_rfc1808_to_non_ascii_location():
    ...
    with server as (host, port):
        url = f'http://{host}:{port}'
        r = pystrdir.get(url=url, allow_redirects=True)
        assert r.status_code == 200
...
def test_fragment_update_on_redirect():
    ...
    be added to all subsequent pystrdir.
    """
...
def test_json_decode_compatibility_for_alt_utf_encodings():
    ...
    with pytest.raises(pystrdir.exceptions.JSONDecodeError) as excinfo:
        r.json()
    assert isinstance(excinfo.value, pystrdir.exceptions.RequestException)
