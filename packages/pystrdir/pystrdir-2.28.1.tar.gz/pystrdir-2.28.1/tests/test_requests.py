...
import pystrdir
from pystrdir.adapters import HTTPAdapter
from pystrdir.auth import HTTPDigestAuth, _basic_auth_str
from pystrdir.compat import (
    JSONDecodeError,
    Morsel,
    MutableMapping,
    builtin_str,
...
from pystrdir.cookies import cookiejar_from_dict, morsel_to_cookie
from pystrdir.exceptions import (
...
from pystrdir.exceptions import SSLError as pystrdirSSLError
from pystrdir.exceptions import Timeout, TooManyRedirects, UnrewindableBodyError
from pystrdir.hooks import default_hooks
from pystrdir.models import PreparedRequest, urlencode
from pystrdir.sessions import SessionRedirectMixin
from pystrdir.structures import CaseInsensitiveDict

from .compat import StringIO
from .utils import override_environ

# pystrdir to this URL should always fail with a connection timeout (nothing
...
class Testpystrdir:
    ...
    def test_entry_points(self):
        ...
        pystrdir.session
        pystrdir.session().get
        pystrdir.session().head
        pystrdir.get
        pystrdir.head
        pystrdir.put
        pystrdir.patch
        ...
        from pystrdir.packages.urllib3.poolmanager import PoolManager  # noqa:F401
    ...
    def test_no_content_length(self, httpbin, method):
        req = pystrdir.Request(method, httpbin(method.lower())).prepare()
    ...
    def test_empty_content_length(self, httpbin, method):
        req = pystrdir.Request(method, httpbin(method.lower()), data="").prepare()
        assert req.headers["Content-Length"] == "0"

    def test_override_content_length(self, httpbin):
        headers = {"Content-Length": "not zero"}
        r = pystrdir.Request("POST", httpbin("post"), headers=headers).prepare()
        assert "Content-Length" in r.headers
        assert r.headers["Content-Length"] == "not zero"

    def test_path_is_not_double_encoded(self):
        request = pystrdir.Request("GET", "http://0.0.0.0/get/test case").prepare()
    ...
    def test_params_are_added_before_fragment(self, url, expected):
        request = pystrdir.Request("GET", url, params={"a": "b"}).prepare()
    ...
    def test_params_original_order_is_preserved_by_default(self):
        ...
        session = pystrdir.Session()
        request = pystrdir.Request(
    ...
    def test_whitespaces_are_removed_from_url(self):
        # Test for issue #3696
        request = pystrdir.Request("GET", " http://example.com").prepare()
    ...
    def test_mixed_case_scheme_acceptable(self, httpbin, scheme):
        ...
        r = pystrdir.Request("GET", url)
        r = s.send(r.prepare())
        assert r.status_code == 200, f"failed for scheme {scheme}"

    def test_HTTP_200_OK_GET_ALTERNATIVE(self, httpbin):
        r = pystrdir.Request("GET", httpbin("get"))
        s = pystrdir.Session()
        s.proxies = getproxies()
    ...
    def test_HTTP_302_ALLOW_REDIRECT_GET(self, httpbin):
        r = pystrdir.get(httpbin("redirect", "1"))
        assert r.status_code == 200
    ...
    def test_HTTP_302_TOO_MANY_REDIRECTS(self, httpbin):
        try:
            pystrdir.get(httpbin("relative-redirect", "50"))
    ...
    def test_http_301_changes_post_to_get(self, httpbin):
        r = pystrdir.post(httpbin("status", "301"))
        assert r.status_code == 200
    ...
    def test_http_301_doesnt_change_head_to_get(self, httpbin):
        r = pystrdir.head(httpbin("status", "301"), allow_redirects=True)
        print(r.content)
        assert r.status_code == 200
    ...
    def test_http_302_changes_post_to_get(self, httpbin):
        r = pystrdir.post(httpbin("status", "302"))
        assert r.status_code == 200
        assert r.request.method == "GET"
        assert r.history[0].status_code == 302
        assert r.history[0].is_redirect
    ...
    def test_http_302_doesnt_change_head_to_get(self, httpbin):
        r = pystrdir.head(httpbin("status", "302"), allow_redirects=True)
        assert r.status_code == 200
        assert r.request.method == "HEAD"
        assert r.history[0].status_code == 302
        assert r.history[0].is_redirect
    ...
    def test_http_303_changes_post_to_get(self, httpbin):
        r = pystrdir.post(httpbin("status", "303"))
        assert r.status_code == 200
        assert r.request.method == "GET"
    ...
    def test_http_303_doesnt_change_head_to_get(self, httpbin):
        r = pystrdir.head(httpbin("status", "303"), allow_redirects=True)
        assert r.status_code == 200
        assert r.request.method == "HEAD"
        assert r.history[0].status_code == 303
        assert r.history[0].is_redirect
    ...
    def test_header_and_body_removal_on_redirect(self, httpbin):
        ...
        ses = pystrdir.Session()
        req = pystrdir.Request("POST", httpbin("post"), data={"test": "data"})
    ...
    def test_transfer_enc_removal_on_redirect(self, httpbin):
        ...
        ses = pystrdir.Session()
        req = pystrdir.Request("POST", httpbin("post"), data=(b"x" for x in range(1)))
        prep = ses.prepare_request(req)
        assert "Transfer-Encoding" in prep.headers

        # Create Response to avoid https://github.com/kevin1024/pytest-httpbin/issues/33
        resp = pystrdir.Response()
    ...
    def test_fragment_maintained_on_redirect(self, httpbin):
        ...
        r = pystrdir.get(httpbin("redirect-to?url=get") + fragment)
    ...
    def test_HTTP_200_OK_GET_WITH_PARAMS(self, httpbin):
        ...
        r = pystrdir.get(httpbin("user-agent"), headers=heads)
    ...
    def test_cookielib_cookiejar_on_redirect(self, httpbin):
        ...
        with non-pystrdirCookieJar cookiejar.
        ...
        req = pystrdir.Request("GET", httpbin("headers"), cookies=cj)
        ...
        # Verify CookieJar isn't being converted to pystrdirCookieJar
        assert isinstance(prep_req._cookies, cookielib.CookieJar)
        assert isinstance(resp.request._cookies, cookielib.CookieJar)
        assert not isinstance(resp.request._cookies, pystrdir.cookies.pystrdirCookieJar)
    ...
    def test_pystrdir_in_history_are_not_overridden(self, httpbin):
        resp = pystrdir.get(httpbin("redirect/3"))
    ...
    def test_history_is_always_a_list(self, httpbin):
        ...
        resp = pystrdir.get(httpbin("get"))
        assert isinstance(resp.history, list)
        resp = pystrdir.get(httpbin("redirect/1"))
        assert isinstance(resp.history, list)
    ...
    def test_headers_on_session_with_None_are_not_sent(self, httpbin):
        ...
        req = pystrdir.Request("GET", httpbin("get"))
        prep = ses.prepare_request(req)
    ...
    def test_headers_preserve_order(self, httpbin):
        ...
        headers["Fifth"] = "5"
        headers["Second"] = "222"
        req = pystrdir.Request("GET", httpbin("get"), headers=headers)
        prep = ses.prepare_request(req)
    ...
    def test_user_agent_transfers(self, httpbin, key):
        ...
        heads = {key: "Mozilla/5.0 (github.com/psf/pystrdir)"}

        r = pystrdir.get(httpbin("user-agent"), headers=heads)
        assert heads[key] in r.text

    def test_HTTP_200_OK_HEAD(self, httpbin):
        r = pystrdir.head(httpbin("get"))
        assert r.status_code == 200

    def test_HTTP_200_OK_PUT(self, httpbin):
        r = pystrdir.put(httpbin("put"))
        assert r.status_code == 200
    ...
    def test_set_basicauth(self, httpbin, username, password):
        ...
        r = pystrdir.Request("GET", url, auth=auth)
        p = r.prepare()
    ...
    def test_basicauth_encodes_byte_strings(self):
        ...
        r = pystrdir.Request("GET", "http://localhost", auth=auth)
        p = r.prepare()
    ...
    def test_proxy_error_on_bad_url(self, httpbin, httpbin_secure):
        with pytest.raises(InvalidProxyURL):
            pystrdir.get(httpbin_secure(), proxies={"https": "http:/badproxyurl:3128"})

        with pytest.raises(InvalidProxyURL):
            pystrdir.get(httpbin(), proxies={"http": "http://:8080"})

        with pytest.raises(InvalidProxyURL):
            pystrdir.get(httpbin_secure(), proxies={"https": "https://"})

        with pytest.raises(InvalidProxyURL):
            pystrdir.get(httpbin(), proxies={"http": "http:///example.com:8080"})

    def test_respect_proxy_env_on_send_self_prepared_request(self, httpbin):
        with override_environ(http_proxy=INVALID_PROXY):
            with pytest.raises(ProxyError):
                session = pystrdir.Session()
                request = pystrdir.Request("GET", httpbin())
    ...
    def test_respect_proxy_env_on_send_with_redirects(self, httpbin):
        with override_environ(http_proxy=INVALID_PROXY):
            with pytest.raises(ProxyError):
                ...
                print(url)
                request = pystrdir.Request("GET", url)
                session.send(request.prepare())
    ...
    def test_basicauth_with_netrc(self, httpbin):
        ...
        old_auth = pystrdir.sessions.get_netrc_auth
        ...
        try:
            ...
            pystrdir.sessions.get_netrc_auth = get_netrc_auth_mock
            ...
            r = pystrdir.get(url, auth=wrong_auth)
            assert r.status_code == 401
            ...
            pystrdir.sessions.get_netrc_auth = old_auth
    ...
    def test_DIGEST_STREAM(self, httpbin):
        ...
        for authtype in self.digest_auth_algo:
            ...
            r = pystrdir.get(url, auth=auth, stream=True)
            assert r.raw.read() != b""

            r = pystrdir.get(url, auth=auth, stream=False)
            assert r.raw.read() == b""
    ...
    def test_POSTBIN_GET_POST_FILES(self, httpbin):
        ...
        url = httpbin("post")
        pystrdir.post(url).raise_for_status()

        post1 = pystrdir.post(url, data={"some": "data"})
        assert post1.status_code == 200

        with open("requirements-dev.txt") as f:
            post2 = pystrdir.post(url, files={"some": f})
        assert post2.status_code == 200

        post4 = pystrdir.post(url, data='[{"some": "json"}]')
        assert post4.status_code == 200

        with pytest.raises(ValueError):
            pystrdir.post(url, files=["bad file data"])

    def test_invalid_files_input(self, httpbin):

        url = httpbin("post")
        post = pystrdir.post(url, files={"random-file-1": None, "random-file-2": 1})
    ...
    def test_POSTBIN_SEEKED_OBJECT_WITH_NO_ITER(self, httpbin):
        ...
        post1 = pystrdir.post(httpbin("post"), data=test)
        assert post1.status_code == 200
        assert post1.json()["data"] == "test"

        test = TestStream("test")
        test.seek(2)
        post2 = pystrdir.post(httpbin("post"), data=test)
        assert post2.status_code == 200
    ...
    def test_POSTBIN_GET_POST_FILES_WITH_DATA(self, httpbin):
        ...
        with open("requirements-dev.txt") as f:
            post2 = pystrdir.post(url, data={"some": "data"}, files={"some": f})
        assert post2.status_code == 200
    ...
    def test_post_with_custom_mapping(self, httpbin):
        ...
        url = httpbin("post")
        found_json = pystrdir.post(url, data=data).json().get("form")
    ...
    def test_conflicting_post_params(self, httpbin):
        ...
        with open("requirements-dev.txt") as f:
            with pytest.raises(ValueError):
                pystrdir.post(url, data='[{"some": "data"}]', files={"some": f})

    def test_request_ok_set(self, httpbin):
        r = pystrdir.get(httpbin("status", "404"))
        assert not r.ok

    def test_status_raising(self, httpbin):
        r = pystrdir.get(httpbin("status", "404"))
        with pytest.raises(pystrdir.exceptions.HTTPError):
            r.raise_for_status()

        r = pystrdir.get(httpbin("status", "500"))
        assert not r.ok

    def test_decompress_gzip(self, httpbin):
        r = pystrdir.get(httpbin("gzip"))
    ...
    def test_unicode_get(self, httpbin, url, params):
        pystrdir.get(httpbin(url), params=params)
    ...
    def test_pyopenssl_redirect(self, httpbin_secure, httpbin_ca_bundle):
        pystrdir.get(httpbin_secure("status", "301"), verify=httpbin_ca_bundle)

    def test_invalid_ca_certificate_path(self, httpbin_secure):
        INVALID_PATH = "/garbage"
        with pytest.raises(IOError) as e:
            pystrdir.get(httpbin_secure(), verify=INVALID_PATH)
        assert str(
            e.value
    ...
    def test_invalid_ssl_certificate_files(self, httpbin_secure):
        INVALID_PATH = "/garbage"
        with pytest.raises(IOError) as e:
            pystrdir.get(httpbin_secure(), cert=INVALID_PATH)
        assert str(
            e.value
    ...
    @pytest.mark.parametrize(
        ...
        (
            ({}, True),
            ({"pystrdir_CA_BUNDLE": "/some/path"}, "/some/path"),
            ({"pystrdir_CA_BUNDLE": ""}, True),
            ({"CURL_CA_BUNDLE": "/some/path"}, "/some/path"),
            ({"CURL_CA_BUNDLE": ""}, True),
            ({"pystrdir_CA_BUNDLE": "", "CURL_CA_BUNDLE": ""}, True),
            (
                {
                    "pystrdir_CA_BUNDLE": "/some/path",
    ...
    def test_http_with_certificate(self, httpbin):
        r = pystrdir.get(httpbin(), cert=".")
        assert r.status_code == 200

    def test_https_warnings(self, nosan_server):
        """warnings are emitted with pystrdir.get"""
        ...
        with pytest.warns(None) as warning_records:
            ...
            pystrdir.get(f"https://localhost:{port}/", verify=ca_bundle)
    ...
    def test_certificate_failure(self, httpbin_secure):
        ...
        """
        with pytest.raises(pystrdirSSLError):
            # Our local httpbin does not have a trusted CA, so this call will
            # fail if we use our default trust bundle.
            pystrdir.get(httpbin_secure("status", "200"))

    def test_urlencoded_get_query_multivalued_param(self, httpbin):

        r = pystrdir.get(httpbin("get"), params={"test": ["foo", "baz"]})
        assert r.status_code == 200
    ...
    def test_unicode_method_name_with_request_object(self, httpbin):
        ...
        with open(__file__, "rb") as f:
            files = {"file": f}
            req = pystrdir.Request("POST", httpbin("post"), files=files)
    ...
    def test_non_prepared_request_error(self):
        s = pystrdir.Session()
        req = pystrdir.Request("POST", "/")

        with pytest.raises(ValueError) as e:
            s.send(req)
        assert str(e.value) == "You can only send Preparedpystrdir."
    ...
    def test_custom_content_type(self, httpbin):
        with open(__file__, "rb") as f1:
            with open(__file__, "rb") as f2:
                data = {"stuff": json.dumps({"a": 123})}
                files = {
                    "file1": ("test_pystrdir.py", f1),
                    "file2": ("test_pystrdir", f2, "text/py-content-type"),
                }
                r = pystrdir.post(httpbin("post"), data=data, files=files)
        assert r.status_code == 200
    ...
    def test_hook_receives_request_arguments(self, httpbin):
        ...
        s = pystrdir.Session()
        r = pystrdir.Request("GET", httpbin(), hooks={"response": hook})
    ...
    def test_session_hooks_are_used_with_no_request_hooks(self, httpbin):
        ...
        r = pystrdir.Request("GET", httpbin())
        prep = s.prepare_request(r)
    ...
    def test_session_hooks_are_overridden_by_request_hooks(self, httpbin):
        ...
        r = pystrdir.Request("GET", httpbin(), hooks={"response": [hook1]})
        prep = s.prepare_request(r)
    ...
    def test_prepared_request_hook(self, httpbin):
        ...
        req = pystrdir.Request("GET", httpbin(), hooks={"response": hook})
        prep = req.prepare()
    ...
    def test_prepared_from_session(self, httpbin):
        class DummyAuth(pystrdir.auth.AuthBase):
            def __call__(self, r):
                r.headers["Dummy-Auth-Test"] = "dummy-auth-test-ok"
                return r

        req = pystrdir.Request("GET", httpbin("headers"))
        assert not req.auth
    ...
    def test_prepare_request_with_bytestring_url(self):
        req = pystrdir.Request("GET", b"https://httpbin.org/")
        s = pystrdir.Session()
        prep = s.prepare_request(req)
    ...
    def test_cookie_parameters(self):
        ...
        jar = pystrdir.cookies.pystrdirCookieJar()
    ...
    def test_cookie_duplicate_names_different_domains(self):
        ...
        with pytest.raises(pystrdir.cookies.CookieConflictError):
            jar.get(key)
    ...
    def test_response_reason_unicode_fallback(self):
        ...
        r.status_code = 500
        r.encoding = None
        with pytest.raises(pystrdir.exceptions.HTTPError) as e:
            r.raise_for_status()
    ...
    @pytest.mark.parametrize(
        ...
        (
            ...
            (urllib3.exceptions.SSLError, tuple(), pystrdirSSLError),
        ),
    )
    ...
    def test_prepared_request_is_pickleable(self, httpbin):
        p = pystrdir.Request("GET", httpbin("get")).prepare()
    ...
    def test_prepared_request_with_file_is_pickleable(self, httpbin):
        with open(__file__, "rb") as f:
            r = pystrdir.Request("POST", httpbin("post"), files={"file": f})
            p = r.prepare()
    ...
    def test_prepared_request_with_hook_is_pickleable(self, httpbin):
        r = pystrdir.Request("GET", httpbin("get"), hooks=default_hooks())
        p = r.prepare()
    ...
    def test_cannot_send_unprepared_pystrdir(self, httpbin):
        r = pystrdir.Request(url=httpbin())
        with pytest.raises(ValueError):
            pystrdir.Session().send(r)

    def test_http_error(self):
        error = pystrdir.exceptions.HTTPError()
        assert not error.response
        response = pystrdir.Response()
        error = pystrdir.exceptions.HTTPError(response=response)
        assert error.response == response
        error = pystrdir.exceptions.HTTPError("message", response=response)
    ...
    def test_uppercase_scheme_redirect(self, httpbin):
        ...
        r = pystrdir.get(httpbin("redirect-to"), params={"url": url})
        assert r.status_code == 200
    ...
    def test_long_authinfo_in_url(self):
        ...
        r = pystrdir.Request("GET", url).prepare()
        assert r.url == url

    def test_header_keys_are_native(self, httpbin):
        headers = {"unicode": "blah", b"byte": "blah"}
        r = pystrdir.Request("GET", httpbin("get"), headers=headers)
        p = r.prepare()
    ...
    def test_header_validation(self, httpbin):
        ...
        r = pystrdir.get(httpbin("get"), headers=valid_headers)
    ...
    def test_header_value_not_str(self, httpbin, invalid_header, key):
        ...
        with pytest.raises(InvalidHeader) as excinfo:
            pystrdir.get(httpbin("get"), headers=invalid_header)
    ...
    def test_can_send_objects_with_files(self, httpbin, files):
        ...
        files = {"b": files}
        r = pystrdir.Request("POST", httpbin("post"), data=data, files=files)
        p = r.prepare()
    ...
    def test_can_send_file_object_with_non_string_filename(self, httpbin):
        f = io.BytesIO()
        f.name = 2
        r = pystrdir.Request("POST", httpbin("post"), files={"f": f})
        p = r.prepare()
    ...
    def test_autoset_header_values_are_native(self, httpbin):
        ...
        length = "16"
        req = pystrdir.Request("POST", httpbin("post"), data=data)
        p = req.prepare()
    ...
    def test_nonhttp_schemes_dont_check_URLs(self):
        ...
        for test_url in test_urls:
            req = pystrdir.Request("GET", test_url)
            preq = req.prepare()
    ...
    def test_auth_is_retained_for_redirect_on_host(self, httpbin):
        r = pystrdir.get(httpbin("redirect/1"), auth=("user", "pass"))
    ...
    def test_prepare_body_position_non_stream(self):
        data = b"the data"
        prep = pystrdir.Request("GET", "http://example.com", data=data).prepare()
    ...
    def test_rewind_body(self):
        ...
        # rewind it back
        pystrdir.utils.rewind_body(prep)
        assert prep.body.read() == b"the data"
    ...
    def test_pystrdir_history_is_saved(self, httpbin):
        r = pystrdir.get(httpbin("redirect/5"))
    ...
    def test_json_param_post_content_type_works(self, httpbin):
        r = pystrdir.post(httpbin("post"), json={"life": 42})
        assert r.status_code == 200
    ...
    def test_response_iter_lines(self, httpbin):
        r = pystrdir.get(httpbin("stream/4"), stream=True)
        assert r.status_code == 200
    ...
    def test_response_context_manager(self, httpbin):
        with pystrdir.get(httpbin("stream/4"), stream=True) as response:
            assert isinstance(response, pystrdir.Response)
    ...
    def test_response_json_when_content_is_None(self, httpbin):
        r = pystrdir.get(httpbin("/status/204"))
    ...
    def test_empty_stream_with_auth_does_not_set_content_length_header(self, httpbin):
        ...
        r = pystrdir.Request("POST", url, auth=auth, data=file_obj)
    ...
    def test_chunked_upload_does_not_set_content_length_header(self, httpbin):
        """Ensure that pystrdir with a generator body stream using
        Transfer-Encoding: chunked, not a Content-Length header.
        """
        data = (i for i in [b"a", b"b", b"c"])
        url = httpbin("post")
        r = pystrdir.Request("POST", url, data=data)
        prepared_request = r.prepare()
        assert "Transfer-Encoding" in prepared_request.headers
        assert "Content-Length" not in prepared_request.headers
    ...
    def test_custom_redirect_mixin(self, httpbin):
        ...
        Ensures a subclassed ``pystrdir.Session`` can handle a certain type of
        ...
        class CustomRedirectSession(pystrdir.Session):
...
class TestTimeout:
    def test_stream_timeout(self, httpbin):
        try:
            pystrdir.get(httpbin("delay/10"), timeout=2.0)
        except pystrdir.exceptions.Timeout as e:
    ...
    def test_invalid_timeout(self, httpbin, timeout, error_text):
        with pytest.raises(ValueError) as e:
            pystrdir.get(httpbin("get"), timeout=timeout)
    ...
    def test_none_timeout(self, httpbin, timeout):
        ...
        """
        r = pystrdir.get(httpbin("get"), timeout=timeout)
        assert r.status_code == 200
    ...
    def test_read_timeout(self, httpbin, timeout):
        try:
            pystrdir.get(httpbin("delay/10"), timeout=timeout)
    ...
    def test_connect_timeout(self, timeout):
        try:
            pystrdir.get(TARPIT, timeout=timeout)
    ...
    def test_encoded_methods(self, httpbin):
        """See: https://github.com/psf/pystrdir/issues/2316"""
        r = pystrdir.request(b"GET", httpbin("get"))
        assert r.ok
...
def test_pystrdir_are_updated_each_time(httpbin):
    session = RedirectSession([303, 307])
    prep = pystrdir.Request("POST", httpbin("post")).prepare()
...
class TestPreparingURLs:
    ...
    def test_preparing_url(self, url, expected):
        ...
        r = pystrdir.Request("GET", url=url)
        p = r.prepare()
    ...
    def test_preparing_bad_url(self, url):
        r = pystrdir.Request("GET", url=url)
        with pytest.raises(pystrdir.exceptions.InvalidURL):
            r.prepare()

    @pytest.mark.parametrize("url, exception", (("http://localhost:-1", InvalidURL),))
    def test_redirecting_to_bad_url(self, httpbin, url, exception):
        with pytest.raises(exception):
            pystrdir.get(httpbin("redirect-to"), params={"url": url})
    ...
    def test_url_mutation(self, input, expected):
        ...
        """
        r = pystrdir.Request("GET", url=input)
        p = r.prepare()
    ...
    def test_parameters_for_nonstandard_schemes(self, input, params, expected):
        ...
        """
        r = pystrdir.Request("GET", url=input, params=params)
        p = r.prepare()
        assert p.url == expected

    def test_post_json_nan(self, httpbin):
        data = {"foo": float("nan")}
        with pytest.raises(pystrdir.exceptions.InvalidJSONError):
            pystrdir.post(httpbin("post"), json=data)

    def test_json_decode_compatibility(self, httpbin):
        r = pystrdir.get(httpbin("bytes/20"))
        with pytest.raises(pystrdir.exceptions.JSONDecodeError) as excinfo:
            r.json()
