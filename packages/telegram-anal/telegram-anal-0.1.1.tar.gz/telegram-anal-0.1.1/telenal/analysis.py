...
from telenal.teleplotter import plot_wordcloud, plot_bars_from_dict

load_dotenv()


class Chats:
    # an object that describes a json export and helps do actions with it
    ...
...
def json_generator(json_path: str, target: str | list[str]) -> Generator:
    try:
        with open(json_path, encoding="utf-8", mode="r") as json_file:
            json_data = json.load(json_file)
            if isinstance(json_data[target], Iterable):
                item: dict
                for item in json_data[target]:
                    yield item
            else:
                raise Exception(
                    f"Incorrect Target, {target} should route to an Iterable"
                )
    except ValueError as error:
        logger.error(error)


def who_breaks_silence_json(
    json_path: str, pause_in_hours=1, out="username", plot=True
) -> Counter:
    """A FUNction to detect, who breaks silence in a given chat after a {pause_in_hours} hour pause"""
    out = ("id", "from")[out == "username"]
    json_gen: Generator = json_generator(json_path, target="messages")
    json_gen.__next__()  # <-- removes first "group item" which is not a correct message, to be improved
    before_message = json_gen.__next__()
    assert before_message["type"] == "message", Exception(
        "Wrong Data, Got non-message object"
    )
    breaker_counter = Counter()
    for after_message in json_gen:
        after_date = datetime.datetime.strptime(
            after_message["date"], "%Y-%m-%dT%H:%M:%S"
        )
        before_date = datetime.datetime.strptime(
            before_message["date"], "%Y-%m-%dT%H:%M:%S"
        )
        timedelta = after_date - before_date

        if (timedelta.seconds / 3600) > pause_in_hours:
            print(
                f"({after_date}): {after_message['text'] or 'non-text'} is the first message",
                end="\r",
            )
            if after_message.get("from", False):
                breaker_counter.update([after_message[out]])
        before_message = after_message
    print(f"\n\nSilence breakers: {dict(breaker_counter)}")
    if plot:
        if len(dict(breaker_counter)) >= 100:
            labelsize = 3
        else:
            labelsize = 7
        plot_bars_from_dict(dict(breaker_counter), labelsize=labelsize)
    return breaker_counter
...
def wordcloud_json(
    ...
    language="ukrainian",
    plot=True,
    custom_stopwords: None | set = None,
    max_words=400,
) -> WordCloud:
    json_gen: Generator = json_generator(json_path, target="messages")
    json_gen.__next__()  # <-- removes first "group item" which is not a correct message, to be improved
    ...
    all_words = ""
    for message_ in json_gen:
        if isinstance(message_.get("text", False), str):
            text = re.sub(r"\W+", " ", message_["text"])  # deleting symbols
            text = text.replace("\n", "")  # deleting indents
            print(f"({message_['date']}) {text.lower().strip()}", end="\r")
