...
class Run():
    def __init__(self, name="", user_id="", lab_id="", org_id="", token="", flush_interval_seconds=5,
    ...
    def conclude(self, show_memoize=True, save_model=False, model_path="./saved_model", target=None):
        ...
        if save_model == True:
            ...
            endpoint_get_token = self.remote_path.replace(
                '/api/runs', path_artifact) + '&token=' + self.token
            oss_config = requests.get(endpoint_get_token)
