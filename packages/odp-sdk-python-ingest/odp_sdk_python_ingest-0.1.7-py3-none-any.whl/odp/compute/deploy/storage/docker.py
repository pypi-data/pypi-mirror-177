...
class Docker(AbstractStorage):
    ...
    def _create_dockerfile_object(self, dir: str) -> str:
        ...
        for flow_name, flow in self._flows.items():
            ...
            open(os.path.join(dir, f"{flow_name}.pickle"), "wb+").write(cloudpickle.dumps(flow))
            open(os.path.join(dir, f"{flow_name}.py"), "w+").write(
                textwrap.dedent(
                    f"""
                import cloudpickle
                import os
                ...
                {flow_name} = cloudpickle.loads(open("{flow_name}.pickle", "rb").read())
            """
                )
            )

            copy_flows = f"COPY {flow_name}.pickle {PREFECT_DIR}/flows/\n" f"COPY {flow_name}.py {PREFECT_DIR}/flows/\n"

        install_cwd = ""
        if self._install_cwd:
            shutil.copytree(src=os.getcwd(), dst=f"{dir}/src", symlinks=False, ignore=None)
            install_cwd = f"COPY src/ {PREFECT_DIR}/src\n" f"RUN pip install {PREFECT_DIR}/src\n"
