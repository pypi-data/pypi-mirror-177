...
def print_itau():
    ...
    for i in range(2):
        ...
        d.linha_digitavel = "34191120938984233293784294590009491950000056427"
        ...
        lista_dados_itau.append(d)

    # Caixa Formato normal - uma pagina por folha A4
    boleto = BoletoPDF('boleto-itau-formato-carne-teste.pdf', True)
    for i in range(0, len(lista_dados_itau), 2):
        boleto.drawBoletoCarneDuplo(
            lista_dados_itau[i],
            lista_dados_itau[i + 1]
        )
        boleto.nextPage()
    boleto.save()
    ...
    for i in range(len(lista_dados_itau)):
        boleto.drawBoleto(lista_dados_itau[i])
        boleto.nextPage()
    boleto.save()


def print_all():
    print("Pyboleto version: %s" % pyboletobr.__version__)
    print("----------------------------------")
