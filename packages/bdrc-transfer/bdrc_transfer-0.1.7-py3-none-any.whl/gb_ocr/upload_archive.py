...
def upload_archive_main():
    ...
    ap = lib.GbParserBase(
        description="Archives images and uploads to GB")
    ap.add_argument("work_path", help="Path to work. Last node is work RID", nargs='?',
    ...
    try:
        if parsed_args.input_file:
            for work_path in fileinput.input(files=parsed_args.input_file):
                process_work(work_path.strip(), zip_queue)
        else:
            process_work(parsed_args.work_path, zip_queue)
    finally:
        zip_queue.join()
...
def process_work(work_p: str, work_queue: queue.Queue):
    """
