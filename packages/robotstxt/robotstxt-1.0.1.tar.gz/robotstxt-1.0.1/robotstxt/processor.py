                    ...
                    if disallow_rule:
                        for token in user_agent_tokens:
                            if token in user_agent_blocks:
                                user_agent_blocks[token].append(['Disallow', -len(disallow_rule[1]), disallow_rule[1].strip(), disallow_rule[1].strip()])

                            else:
                                user_agent_blocks[token] = [['Disallow', -len(disallow_rule[1]), disallow_rule[1].strip(), disallow_rule[1].strip()]]
                        ...
                        if allow_rule:
                            for token in user_agent_tokens:
                                if token in user_agent_blocks:
                                    user_agent_blocks[token].append(['Allow', -len(allow_rule[1]), allow_rule[1], allow_rule[1]])

                                else:
                                    user_agent_blocks[token] = [['Allow', -len(allow_rule[1]), allow_rule[1], allow_rule[1]]]
    ...
    def test_url(self, url, token):
        ...
        if selected_block:
            for rule in self.rule_blocks[selected_block]:
                ...
                if rule[3] and pattern_match(unquote(rule[3]), url_path_to_test):
                    if rule[0] == 'Disallow':
