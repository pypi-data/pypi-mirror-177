...
class NoThreadPythonRepl(PythonRepl):
    ...
    - asyncio_gevent sets up a gevent backend for asyncio, as a result operations that
    ...
    asyncio_gevent for asyncio + monkey-patched Python so we can avoid threads
    completely.
    """
    ...
    def eval_greenlet(self, text):
        with self._lock:
            ...
            return asyncio_gevent.greenlet_to_future(self._current_eval_g)
