...
class ImageHandler(Api):
        ...
        for i in self.__to_path.split('/'):
            if i:
                ...
                if not os.path.exists(p):
                    os.mkdir(p)
                p += "/"
        os.mkdir(self.__to_path + ".nomedia")
    ...
    def click(self, img_name: Union[str, list[str]]):
        self.__isClick = False
        ...
        if res:
            ...
            if deviation and deviation_len == 2 or deviation_len == 4:
                ...
                height = res["height"]
                width = res["width"]
                y += height * 0.25 + random.randint(height * 0.25, height * 0.75)
                x += width * 0.25 + random.randint(width * 0.25, width * 0.75)
            self.api("/touch", {"x": int(x), "y": int(y)})
            self.__isClick = True
        return self
    ...
    def exists(self, img_name: Union[str, list[str]]) -> bool:
        res = self.__find_images(img_name)
        return bool(res)
