...
from hdiiofficial import HDev, filters
from hdiiofficial.config import settings, ENV
from pyrogram.types import InlineKeyboardMarkup, InlineKeyboardButton, Message
from pyrogram.errors import ChatAdminRequired, UserNotParticipant, ChatWriteForbidden


@HDev.cmd(private_only=True, extra_filters=~filters.edited & filters.incoming, group=-1)
async def must_join_channel(bot: HDev, msg: Message):
    if ENV.DATABASE_URL:
        from hdiiofficial.plugins.helpers import db
    ...
    try:
        ...
        chats = []
    if not chats:
        HDev.log("Addon must_join is loaded but no chat is listed in MUST_JOIN var of settings", "warn")
        return
    nope = []
    for chat in chats:
        try:
            ...
            HDev.log(f"I'm not admin in the {chat} Chat !", "warning")
    if not nope:
        return
    buttons = []
    for i in nope:
