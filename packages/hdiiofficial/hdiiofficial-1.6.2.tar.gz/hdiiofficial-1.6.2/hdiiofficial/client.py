...
from hdiiofficial.logger import logger
from pyrogram.types import BotCommand
from pyrogram import Client, idle, raw
from hdiiofficial.decorators import Mechanism
from hdiiofficial.config import ENV, settings
from hdiiofficial.constants import __version__
from inspect import getmembers, isfunction
from hdiiofficial.plugins.models.bans import Bans
from hdiiofficial.plugins.models.users import Users
from hdiiofficial.helpers.localization import l10n_setup, get_all_langs
from hdiiofficial.decorators.command import command_data
...
class HDev(Client, Mechanism):
    __support__ = "hdiiofficial"
    __updates__ = "hdiiofficial"
    __channel__ = "hdiiofficial"
    ...
    def __init__(self, **kwargs):
        global __printed__
        if not __printed__:
            print(f'Welcome to HDevpy [v{__version__}]')
            print('Copyright (C) 2021-2022 HDev <https://github.com/hdiiofficial> \n')
            __printed__ = True
        env = ENV()
        super().__init__(
            ":memory:",
    ...
    def activate(self):
        """Main method of `HDevpy` class which loads plugins and activates/runs your bot."""
        module = settings()
        try:
            ...
            if addons:
                ...
                for i in addons:
                    path = f"hdiiofficial/plugins/addons/{i}"
                    if not path.endswith(".py"):
                        path += ".py"
                    self.load_modules(path)
            if module.DATABASE_TABLES:
                self.log("Initializing Database")
                from hdiiofficial.database.sql import Database
            ...
            if getattr(module, "SET_BOT_MENU"):
                try:
                    ...
                    pass
            self.stop()
            logger.info("Bot has stopped working. For issues, visit <https://t.me/hdiiofficial>")

    def run(self):
        """Alias of `HDev.activate()`"""
        self.activate()
    ...
    def all_plugins(self) -> dict:  # not "plugins" to prevent clash with pyrogram's "plugins" attribute
        ...
        ```python
        {
            ...
            "must_join":"C:\\Users\\...\\hdiiofficial\\addons\\must_join.py"
        },
        ```
        """
