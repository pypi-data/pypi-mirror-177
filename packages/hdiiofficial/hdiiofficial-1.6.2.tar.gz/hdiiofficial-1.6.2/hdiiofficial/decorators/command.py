...
class Command(OnMessage):
    ...
    def command(
        ...
        Examples:
            ...
            from hdiiofficial import filters

            # Filter only media messages.
            @HDev.command(extra_filters=filters.media)

            # Filter only text messages.
            @HDev.command(extra_filters=filters.text)

            # Filter only messages sent by 'StarkProgrammer'.
            @HDev.command(extra_filters=filters.user('StarkProgrammer'))

            # Filter only messages sent in 'StarkBotsChat'
            @HDev.command(extra_filters=filters.chat('StarkBotsChat'))

            # Filter only messages with the word 'baby'.
            @HDev.command(extra_filters=filters.regex('baby'))

            # Filter all media messages sent by bots.
            @HDev.command(extra_filters=filters.bot & filters.media)

            # Filter all messages sent by bots except media messages.
            @HDev.command(extra_filters=filters.bot & ~filters.media)

            # Filter either media messages or text messages.
            @HDev.command(extra_filters=filters.text | filters.media)
            ```
        """
