...
class Callback(OnCallbackQuery):
    ...
    def callback(
        ...
        Examples:
            ...
            @HDev.callback(['first_button', 'second_button'])
            ...
            @HDev.callback('first_button', owner_only=True)
            ...
            # Others will be ignored.
            @HDev.callback('first_button', sudo_only=True)
            ...
            @HDev.callback(filters=filters.user(['StarkProgrammer', 'Designatory']))

            # Filter only queries done in 'StarkBotsChat'
            @HDev.callback(filters=filters.chat('StarkBotsChat'))

            # Filter only queries ending with the word 'baby'.
            @HDev.callback(filters=filters.regex(r'baby$'))

            # Filter all queries with the word 'hello' AND which are done in 'StarkBotsChat'.
            @HDev.callback(filters=filters.chat('StarkBotsChat') & filters.regex('hello'))
            # or
            @HDev.callback('hello', filters=filters.chat('StarkBotsChat'))
            ...
            @HDev.callback(filters=~filters.chat('StarkBotsChat') & filters.regex('hello'))
            # or
            @HDev.callback(filters=filters.regex('hello') & ~filters.chat('StarkBotsChat'))
            ```
        """
        # ToDo:
