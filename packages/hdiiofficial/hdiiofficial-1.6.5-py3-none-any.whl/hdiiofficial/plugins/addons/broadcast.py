...
from HDev import HDev, Message
from HDev.plugins.helpers import db


@HDev.cmd("broadcast", owner_only=True)
async def broadcast(bot: HDev, msg: Message):
    ...
    for user in users:
        try:
            ...
            HDev.log(f"Exception occurred while sending message to user with id {user}: {e}", "warning")
