...
from hdiiofficial import settings as s
from importlib import import_module
from hdiiofficial.plugins import hdev as h
...
def settings():
    ...
    if os.path.exists('settings.py'):
        ...
        mod = import_module('hdiiofficial.settings')
    for i in dir(s):
        if not i.isupper():
            continue
        if "HDEVBOTS" in mod.__dict__ and getattr(mod, "HDEVBOTS") and i == "ADDONS":
            continue
        try:
            getattr(mod, i)
        except AttributeError:
            setattr(mod, i, getattr(s, i))
    for i in ["HDEVBOTS", "FULL_MESSAGES", "CUSTOM_USERS_TABLE", "NO_BUTTONS", "REMOVE_ADDONS"]:  # my personalization (not in settings.py)
        try:
            getattr(mod, i)
        except AttributeError:
            setattr(mod, i, False)
    ...
    if getattr(mod, "HDEVBOTS") or os.getenv("HDEVBOTS", ""):
        # FULL_MESSAGES
        if "FULL_MESSAGES" not in mod.__dict__ or not getattr(mod, "FULL_MESSAGES"):
            ...
            setattr(
                ...
                .replace("{2}", (("\n\nSource Code : [Click Here](https://github.com/hdiiofficial/"+rep+")") if rep else ""))
            )
        # MUST_JOIN
        if not getattr(mod, "MUST_JOIN") and ("special" in os.getenv("MUST_JOIN", "") or os.getenv("HDEVBOTS", "")):
            setattr(mod, "MUST_JOIN", ["hdii_dn", "hdiiofficial"])

        # ADDONS
        if "ADDONS" not in mod.__dict__:
            from hdiiofficial.plugins import addons
...
class ENV:
    ...
    def __init__(self):
        ...
        if "special" in self.OWNER_ID or getattr(module, "HDEVBOTS"):
        ...
        if self.LOG_CHAT and ("special" in self.LOG_CHAT or os.getenv("HDEVBOTS", "")) and getattr(module, "HDEVBOTS"):
