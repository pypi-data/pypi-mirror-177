...
class PostProcessor(ABC):
    ...
    def __call__(self, encoding: Encoding, pair_encoding: Encoding,
...
class BertPostProcessor(PostProcessor):
    ...
    def __init__(self,
                 sep: Tuple[str, int] = ("[SEP]", 102),
                 cls: Tuple[str, int] = ("[CLS]", 101)):
...
class RobertaPostProcessor(PostProcessor):
    ...
    def __init__(self,
        ...
        self._postprocessor = C.postprocessors.RobertaPostProcessor(
...
class ByteLevelPostProcessor(PostProcessor):
    ...
    def __init__(self,
                 ...
                 trim_offsets: bool = True,
                 use_regex: bool = True):
        self._postprocessor = C.postprocessors.ByteLevelPostProcessor(
            add_prefix_space, trim_offsets, use_regex)


class TemplatePostProcessor(PostProcessor):

    def __init__(self, single: Union[str, List[str]],
                 pair: Union[str, List[str]], special_tokens: List[Tuple[str,
                                                                         int]]):
        self._postprocessor = C.postprocessors.TemplatePostProcessor(
