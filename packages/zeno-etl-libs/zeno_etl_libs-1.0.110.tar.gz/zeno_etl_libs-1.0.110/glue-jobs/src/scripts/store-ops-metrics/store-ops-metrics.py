...
pro = f"""
  select
	...
	count(distinct (pr."order-number" || pr."store-id" || pr."patient-id" || pr."created-at" || nvl(pr."bill-id", 0) )::text) as "pr_created_count",
	count(distinct case when (case when pr."order-type" = 'pickup' then pr."completed-at" else hdm."delivered-at" end)<= pr."turnaround-time" then (pr."order-number" || pr."store-id" || pr."patient-id" || pr."created-at" || nvl(pr."bill-id", 0)::text ) else null end) as "within_slot_delivered_count",
	count(distinct case when (case when pr."order-type" = 'pickup' then pr."completed-at" else hdm."delivered-at" end) is not null then (pr."order-number" || pr."store-id" || pr."patient-id" || pr."created-at" || nvl(pr."bill-id", 0) )::text end) as "total_delivered_pr_count",
    sum(case when pr."order-type"='delivery' then pr."selling-rate" end) as "pr_created_value_delivery",
	sum(case when pr."order-type"='delivery' and (case when pr."order-type" = 'pickup' then pr."completed-at" else hdm."delivered-at" end)<= pr."turnaround-time" then pr."selling-rate" else 0 end) as "within_slot_delivered_pr_value_delivery",
	sum(case when pr."order-type"='delivery' and (case when pr."order-type" = 'pickup' then pr."completed-at" else hdm."delivered-at" end) is not null then pr."selling-rate" end) as "total_delivered_pr_value_delivery",
	count(distinct case when pr."order-type"='delivery' then (pr."order-number" || pr."store-id" || pr."patient-id" || pr."created-at" || nvl(pr."bill-id", 0) )::text end) as "pr_created_count_delivery",
	count(distinct case when pr."order-type"='delivery' and (case when pr."order-type" = 'pickup' then pr."completed-at" else hdm."delivered-at" end)<= pr."turnaround-time" then (pr."order-number" || pr."store-id" || pr."patient-id" || pr."created-at" || nvl(pr."bill-id", 0)::text ) else null end) as "within_slot_delivered_count_delivery",
	count(distinct case when pr."order-type"='delivery' and (case when pr."order-type" = 'pickup' then pr."completed-at" else hdm."delivered-at" end) is not null then (pr."order-number" || pr."store-id" || pr."patient-id" || pr."created-at" || nvl(pr."bill-id", 0) )::text end) as "total_delivered_pr_count_delivery"
from
...
'store-id' ,'date' ,'pr-received-tat' ,'as-received-tat'
    ...
    , 'total-delivered-pr-count-delivery'
]]
