...
from .typing import CallbackError, CallbackHandler, GetResultsHandler
...
class AioPool:
    ...
    _executed: int
    _joined: set[Future]
    _waiting: dict[Future, Future]
    _active: dict[Future, Future]
    ...
    def _build_callback(
        ...
        ctx: Any = None,
    ) -> tuple[Coroutine[CallbackHandler, None, None] | None, Exception | None]:
        ...
        parameters = Signature.from_callable(cb).parameters
        ...
        if list(parameters.keys())[0] in ("self", "cls"):
    ...
    async def map(
        ...
        ctx: CallbackError | None = None,
        *,
        get_result: GetResultsHandler = getres.flat,
    ):
    ...
    async def itermap(
        ...
        *,
    ...
    ) -> AsyncGenerator[Any | list[Any], None]:
        ...
        """
        active = self.map_n(fn, iterable, cb, ctx)
        while active:
            done, _ = await asyncio.wait(
                active, timeout=timeout, return_when=yield_when
            )
            if flat:
                for fut in done:
    ...
    async def cancel(
        ...
        *futures: Future,
        ...
        if not futures:  # meaning cancel all
            _tasks.extend(iter(self._waiting.values()))
            _tasks.extend(iter(self._active.values()))
            _futures.extend(iter(self._waiting.keys()))
            _futures.extend(iter(self._active.keys()))
        else:
            for fut in futures:
        ...
        cancelled = 0
        if _tasks:
            cancelled = sum(1 for task in _tasks if task.cancel())
            await asyncio.wait(_tasks)  # let them actually cancel
