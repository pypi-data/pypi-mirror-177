...
def main():
    ...
    if len(argv) == 3:
        ...
        if argv[2] == "1":
            ...
            resume.apply_theme(loads(get_file("themes/default.json")))
        elif argv[2] == "2":
            resume.apply_theme(loads(get_file("themes/default2.json")))
        elif argv[2] == "3":
            resume.apply_theme(loads(get_file("themes/default3.json")))
        else:
            # todo: schema checks ones schema is complete (sprint 5?)
            resume.apply_theme(loads(get_file(argv[2])))
