...
def _check_cross_entropy_initializer(input_):
    ...
    _check_seed(initializer.get("rngSeed"))
...
def _check_cmaes_initializer(input_):
    ...
    initial_cov = initializer.get("initialCovariance")

    if initial_cov is not None:
        if len(initial_cov) != parameter_count:
            raise QctrlFieldError(
                message="The number of rows in the initial covariance must be the same as "
                "the number of parameters.",
                fields=["initialCovariance"],
            )
        for row in initial_cov:
            if len(row) != parameter_count:
                raise QctrlFieldError(
                    message="The length of each element in the initial covariance must be the "
...
def _check_seed(seed):
    if seed is not None and seed < 0:
        raise QctrlFieldError(
            message="Ensure seed value is greater than or equal to 0.",
            fields=["seed"],
        )
