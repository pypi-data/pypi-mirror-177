...
class ArawinzSdk:
    ...
    def __feeding_contract(self, owner_passphrase,receiver):
        owner = mnemonic.to_public_key(owner_passphrase)
        private_key = mnemonic.to_private_key(owner_passphrase)
        note = "Algos".encode()
    ...
    def __set_manager01(self,app_id,owner_passphrase,m_addr,name,email,designation,password,mobile):
        owner = mnemonic.to_public_key(owner_passphrase)
        private_key = mnemonic.to_private_key(owner_passphrase)
        params = algod_client.suggested_params()
    ...
    def createWallet(self, owner_passphrase,app_id,name, email,designation,password, mobile,manager):
        ...
        feed =  self.__feeding_contract(owner_passphrase,address)
        
        if feed:
            optin = self.__App_optin(app_id,mnemonic.from_private_key(private_key))
            if optin:
                if manager=='SC':
                    status = self.__set_manager01(app_id,owner_passphrase,address,name, email,designation,password, mobile)
                    if status:
