...
class mysql:
   ...
   def mysqlQueryRe(q,intentos=3,sleepStep=1):
      looper=0
      valid=False
      while looper<intentos:
          ...
          looper+=1
          try:
              df=mysqlQuerySilent(q)
              if isinstance(df, pd.DataFrame):
                  valid=True
                  break
          except Exception as e:
              falla='Err mysql;'+str(e)
              out().print(falla)
              nada=0
      ...
      if(valid):
        ...
        out().print('Err mysql; intentos:' + str(looper) + '; df.shape:' + str(df.shape) )
        return {'result':0,'data':pd.DataFrame()}


   def mysqlQuerySilent(query):
      df='err:'
      try:
         sqlEngine       = create_engine('mysql+pymysql://'+self.user+':'+self.password+'@'+selfserver)
