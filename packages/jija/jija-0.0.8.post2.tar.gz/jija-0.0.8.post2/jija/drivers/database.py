...
class JijaOrmDriver(DatabaseDriver):
    ...
    async def preflight(cls):
        ...
        class JijaORMAppFix(jija_orm_config.App):
            def check(self, base_path):
                for app_models in self._App__models_modules:
                    path = base_path.joinpath(self._App__path.replace('.', '/'), app_models.replace('.', '/') + '.py')
                    if not os.path.exists(path):
                        raise exceptions.AppConfigImportError(path)

        jija_orm_config.App = JijaORMAppFix
