...
class StringTool:
    ...
    @staticmethod
    def list_to_string(value: List, item_as_string: bool = False, separator: str = ", ", start: str = "", end: str = "") -> str:
        return start + separator.join(list(map(lambda item: f"'{item}'" if item_as_string else str(item), value))) + end
...
class FullNameTool:
    ...
    def to_given_name(full_name: Any, join_symbol: str = SPLIT_SYMBOL) -> str:
        if isinstance(full_name, FullName):
            return join_symbol.join(list(filter(lambda item: len(item) > 0, [full_name.first_name, full_name.middle_name])))
