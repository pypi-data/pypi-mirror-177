from stochasticx.constants.urls import DATASETS_URL, LocalRoutes, get_local_url
...
class Datasets:
    ...
    def get_local_dataset(dataset_id: str):
        ...
        Returns:
            Dataset: the dataset
        """
        url = get_local_url(LocalRoutes.DATASET_URL) + "/{}".format(dataset_id)
        ...
        if dataset_data is not None:
            ds = Dataset(
                name=dataset_data.get("name"),
                directory_path=dataset_data.get("path"),
                dataset_type=dataset_data.get("type")
            )
    ...
    @staticmethod
    def get_local_datasets(fmt="table"):
        """Gets all the datasets

        Returns:
            List[Dataset]: all the local datasets
        """
        datasets = []

        # auth_header = AuthUtils.get_auth_headers()
        auth_header = None
        response = requests.get(get_local_url(LocalRoutes.DATASET_URL), headers=auth_header)
        datasets_data = response.json().get("data")
