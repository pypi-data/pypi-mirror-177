from stochasticx.constants.urls import MODELS_URL, OPTIMIZED_MODELS_URL, LocalRoutes, get_local_url
...
class Models:
    ...
    def get_local_model(model_id: str):
        ...
        url = get_local_url(LocalRoutes.MODEL_URL) + "/{}".format(model_id)
    ...
    def get_local_models(fmt="table"):
        ...
        # auth_header = AuthUtils.get_auth_headers()
        auth_header = None
        response = requests.get(get_local_url(LocalRoutes.MODEL_URL), headers=auth_header)
        models_data = response.json().get("data")
