...
class Cohort(BaseCohort, extra=RESULT_DATACLASS_EXTRA):
    ...
    def run_code(self, run_code, **kwargs):
        ...
        Parameters
        ----------
        run_code: JSON that contains code to run example: {"code":"df['BMI'] = df.Weight / (df.Height ** 2)"}

        Returns
        -------
        Tuple: (output_cohorts, run_response)
            output_cohorts: List of  Cohort Dataclasses
            run_response: A dict containing the run outcome
        """
        ...
        param_dict = {
            ...
            "input_data_schema_uid": self.data_schema_uid,
            "output_data_schema_uid": None,
            "output_cohort_names_suffix": "containerless code",
            "timeout_seconds": 600,
        }
        param_dict.update(kwargs)
        create_model_params = AIModelCreateInput(
