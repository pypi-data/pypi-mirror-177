...
class AIModelCreateInput(RhinoBaseModel):
    ...
    """@autoapi True The first data schema this ai model expects input cohorts to adhere to"""
    output_data_schema_uid: Annotated[Optional[str], Field(alias="output_data_schema")]
    """@autoapi True The first data schema this ai model expects output cohorts to adhere to"""
    ...
    output_data_schema_uids: Optional[List[str]]
...
class AIModelMultiCohortInput(AIModelRunInputBase):
    """
    @autoapi True
    Input parameters for running generalized compute with multiple
    input and output cohorts per container
    """

    # TODO: Better documentation examples
    input_cohort_uids: List[List[str]]
    """@autoapi True A list of lists of the input cohort uids. [[first_cohort_for_first_run, second_cohort_for_first_run ...], [first_cohort_for_second_run, second_cohort_for_second_run ...], ...] for N runs"""
    output_cohort_naming_templates: List[str]
    """@autoapi True A list of naming templates for output cohorts indexed by the cohort. Supported templates: {{workgroup_name}} {{workgroup_uid}} {{model_name}} {{input_cohort_names.n}} where n is the input cohort (zero indexed)"""
