        ...
        for file_cnt, file_path in enumerate(files_found):
            ...
            for roi_type, roi_data in self.ROI_dict_lists.items():
                ...
                for roi_name in roi_data:
                    ...
                    elif (roi_type.lower() == 'circle') or (roi_type.lower() == 'circledf'):
                    ...
                    if roi_type.lower() == 'rectangle':
                        ...
                        self.results[roi_name] = data_df.apply(lambda x: self.__inside_circle(bp_x=x[body_part_col_names_x[0]], bp_y=x[body_part_col_names_y[0]], center_x=x['center_x'], center_y=row['center_y'], radius=x['radius']), axis=1)
                        self.found_rois.append(roi_name)
                    elif roi_type.lower() == 'polygon':
                        polygon_vertices = []
...
# clf_ROI_analyzer.perform_ROI_clf_analysis(behavior_list=['Attack', 'Sniffing'], ROI_dict_lists={'Rectangle': ['rec']}, body_part_list=['Nose_1'], measurements=['Total time by ROI (s)', 'Started bouts by ROI (count)', 'End bouts by ROI (count)'])
