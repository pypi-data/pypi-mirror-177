...
class SetHyperparameterPopUp(object):
    def __init__(self,
        ...
        self.n_estimators_entrybox = Entry_Box(hyperparameters_frm,'RF N Estimators','25', validation='numeric')
        self.max_features_entrybox = Entry_Box(hyperparameters_frm,'RF Max features','25')
        self.criterion_entrybox = Entry_Box(hyperparameters_frm,'RF Criterion','25')
        self.test_size_entrybox = Entry_Box(hyperparameters_frm,'Train Test Size','25')
        self.min_sample_leaf_entrybox = Entry_Box(hyperparameters_frm,'RF Min sample leaf','25', validation='numeric')
        ...
        self.learning_curve_k_splits_entry = Entry_Box(self.evaluation_settings_frm, 'LearningCurve shuffle K splits', '25',status=DISABLED, validation='numeric')
        self.learning_curve_data_splits_entry = Entry_Box(self.evaluation_settings_frm, 'LearningCurve shuffle Data splits', '25',status=DISABLED, validation='numeric')
        self.n_feature_importance_bars_entry = Entry_Box(self.evaluation_settings_frm, 'N feature importance bars', '25', status=DISABLED, validation='numeric')
        ...
        self.label_n_feature_importance_bars = Entry_Box(self.evaluation_settings_frm, 'N feature importance bars', '25', status=DISABLED, validation='numeric')
        ...
        create_example_decision_tree_cb = Checkbutton(self.evaluation_settings_frm, text='Generate Example Decision Tree (requires "graphviz")', variable=self.create_example_decision_tree_var)
        self.create_example_fancy_decision_tree_var = BooleanVar()
        create_example_fancy_decision_tree_cb = Checkbutton(self.evaluation_settings_frm, text='Generate Fancy Example Decision Tree ("dtreeviz")', variable=self.create_example_fancy_decision_tree_var)
        self.create_clf_report_var = BooleanVar()
        create_clf_report_cb = Checkbutton(self.evaluation_settings_frm, text='Generate Classification Report', variable=self.create_clf_report_var)
        self.create_feature_importance_bar_graph_var = BooleanVar()
        create_feature_importance_bar_graph_cb = Checkbutton(self.evaluation_settings_frm, text='Generate Features Importance Bar Graph', variable=self.create_feature_importance_bar_graph_var, command = lambda:activate(self.create_feature_importance_bar_graph_var, self.label_n_feature_importance_bars))
        ...
        compute_learning_curves_cb = Checkbutton(self.evaluation_settings_frm, text='Generate Sklearn Learning Curves (Note: CPU intensive)', variable=self.compute_learning_curves_var, command = lambda:activate(self.compute_learning_curves_var, self.learning_curve_data_splits_entry, self.learning_curve_k_splits_entry))
        self.compute_pr_curves_var = BooleanVar()
        compute_pr_curves_cb = Checkbutton(self.evaluation_settings_frm, text='Generate Precision Recall Curves', variable=self.compute_pr_curves_var)
        self.compute_shap_scores_var = BooleanVar()
        compute_shap_scores_cb = Checkbutton(self.evaluation_settings_frm,text='Calculate SHAP scores',variable=self.compute_shap_scores_var,command= lambda:activate(self.compute_shap_scores_var,self.shap_present,self.shap_absent))
        ...
        save_global_btn = Button(self.main_frm, text='Save settings into global environment', font=('Helvetica', 10, 'bold'),fg='blue', command=self.save_global_data)
        save_one_model_to_config = Button(self.main_frm, text='Save settings for specific model', font=('Helvetica', 10, 'bold'),fg='green' ,command=self.save_config_file)
        clear_cache_btn = Button(self.main_frm,text='Clear cache',font=('Helvetica', 10, 'bold'),fg='red',command = self.clear_cache)
        ...
        create_feature_importance_bar_graph_cb.grid(row=4, sticky=W)
        self.label_n_feature_importance_bars.grid(row=5, sticky=W)
        compute_feature_permutation_importances_cb.grid(row=6, sticky=W)
        compute_learning_curves_cb.grid(row=7, sticky=W)
        self.learning_curve_k_splits_entry.grid(row=8, sticky=W)
        self.learning_curve_data_splits_entry.grid(row=9, sticky=W)
        compute_pr_curves_cb.grid(row=10, sticky=W)
        compute_shap_scores_cb.grid(row=11, sticky=W)
        self.shap_present.grid(row=12, sticky=W)
        self.shap_absent.grid(row=13, sticky=W)

        save_global_btn.grid(row=6, pady=5)
        save_one_model_to_config.grid(row=7)
        clear_cache_btn.grid(row=8, pady=5)

    def clear_cache(self):
        config_files = glob.glob(os.path.join(self.project_path, 'configs') + '/*')
        for file_path in config_files:
            os.remove(file_path)
        print('SIMBA COMPLETE: {} cache model config files deleted from {} folder'.format(str(len(config_files)), os.path.join(self.project_path, 'configs')))

    def load_meta_data(self):
        check_file_exist_and_readable(file_path=self.selected_file.file_path)
        meta_dict = pd.read_csv(self.selected_file.file_path, index_col=False).to_dict()
        print(meta_dict)
    ...
    def save_global_data(self):
        ...
        self.config.set('create ensemble settings', 'RF_meta_data', self.create_meta_data_file_var.get())
        self.config.set('create ensemble settings', 'generate_example_decision_tree', self.create_example_decision_tree_var.get())
        self.config.set('create ensemble settings', 'generate_classification_report', self.create_clf_report_var.get())
        self.config.set('create ensemble settings', 'generate_features_importance_log', self.create_feature_importance_bar_graph_var.get())
        self.config.set('create ensemble settings', 'generate_features_importance_bar_graph', self.create_feature_importance_bar_graph_var.get())
        self.config.set('create ensemble settings', 'N_feature_importance_bars', self.create_feature_importance_bar_graph_var.get())
        self.config.set('create ensemble settings', 'compute_permutation_importance', self.compute_feature_permutation_importances_var.get())
        self.config.set('create ensemble settings', 'generate_learning_curve', self.compute_learning_curves_var.get())
        self.config.set('create ensemble settings', 'generate_precision_recall_curve', self.compute_pr_curves_var.get())
        ...
        self.config.set('create ensemble settings', 'generate_example_decision_tree_fancy', self.create_example_fancy_decision_tree_var.get())
        self.config.set('create ensemble settings', 'generate_shap_scores', self.compute_shap_scores_var.get())
    ...
    def global_unit_tests(self):
        ...
        check_str(name='Under sample setting', value=self.undersample_setting_entrybox.entry_get.lower(), options=('random undersample', ''))
        if self.undersample_setting_entrybox.entry_get.lower() == 'random undersample':
            check_float(name='Under sample ratio', value=self.undersample_ratio_entrybox.entry_get)
        check_str(name='Over sample setting', value=self.oversample_setting_entrybox.entry_get.lower(), options=('smote', 'smoteen', ''))
        if (self.oversample_setting_entrybox.entry_get.lower() == 'smote') or (self.oversample_setting_entrybox.entry_get.lower() == 'smoteen'):
    ...
    def save_config_file(self):
        self.global_unit_tests()
        existing_file_cnt = 0
        for f in os.listdir(os.path.join(os.path.dirname(self.config), 'configs')):
        ...
        meta_dict = {'Classifier_name': self.model_name_dropdown.getChoices(),
                     ...
                     'RF_criterion': self.criterion_entrybox.entry_get,
                     ...
                     'under_sample_setting': self.undersample_setting_entrybox,
                     'over_sample_ratio': self.undersample_ratio_entrybox.entry_get,
                     'over_sample_setting': self.undersample_ratio_entrybox.entry_get,
                     ...
                     'generate_features_importance_log': self.create_feature_importance_bar_graph_var.get(),
