...
class ConcatenatorPopUp(object):
    def __init__(self,
        ...
        self.videos_lst = []
        for file_path in glob.glob(self.in_folder_path):
            _, name, _ = get_fn_ext(filepath=file_path)
            self.videos_lst.append(name)
        self.main_frm = Toplevel()
        self.main_frm.minsize(500, 800)
        self.main_frm.wm_title('MERGE (CONCATENATE) VIDEOS')
        self.settings_frm = LabelFrame(self.main_frm, text='SETTINGS', pady=5, padx=5, font=("Helvetica", 12, 'bold'), fg='black')
        ...
        self.resolution_dropdown = DropDownMenu(self.settings_frm, 'Resolution height', ['640', '1280', '1920', '2560'], '15')
        self.video_dropdown = DropDownMenu(self.select_video_frm, 'Video: ', self.videos_lst, '15')
        ...
        self.heatmaps_clf_cb.grid(row=6, column=0, sticky=NW)
        self.path_cb.grid(row=7, column=0, sticky=NW)
        self.path_cb.grid(row=7, column=0, sticky=NW)
        self.resolution_dropdown.grid(row=8, column=0, sticky=NW)
        self.select_video_frm.grid(row=0, column=0, sticky=NW)
        self.settings_frm.grid(row=1, column=0, sticky=NW)
        run_btn.grid(row=2, column=0, sticky=NW)

    def run(self):
        settings_dict, video_name = {}, self.video_dropdown.getChoices()
        video_counter = 0
        for var, name in zip([self.classifications_sklearn_var, self.gantt_var, self.data_var, self.distance_var, self.probability_var, self.path_var, self.heatmaps_clf_var, self.heatmaps_loc_var], ['Classifications (sklearn)', 'Gantt plots', 'Data plots', 'Distance plots', 'Probability plots', 'Path plots', 'Heatmap (classifications)', 'Heatmap (location)']):
            settings_dict[name] = {}
            if var.get():
                settings_dict[name]['include'] = True
                settings_dict[name]['video path'] = find_video_of_file(self.data_dirs[name], filename=video_name)
                if settings_dict[name]['video path'] is None:
                    ...
                    raise FileNotFoundError('SIMBA ERROR: Could not locate a video representing {} inside the {} directory. Create this video before concatenating it to other videos.'.format(video_name, self.data_dirs[name]))
                else:
                    video_counter += 1
            else:
                settings_dict[name]['include'] = False
        if video_counter < 2:
