...
def zimbra_execute(config, commands, options):
    ...
    if commands == "":
        ...
        try:
            ...
            info(f"WARNING: unexpected timeout ({options.zimbra_timeout}) during Zimbra command")
        except Exception:
...
def update_lists(options=None):
    ...
    if options.personnel:
        ...
        if options.execute or options.output:
            ...
            if options.execute:
                zimbra_execute(config["zimbra"], zimbra_cmds, options)
            else:
...
def main():
    ...
    parser.add_argument(
        "--zimbra-timeout",
        type=int,
        help=f"Max wait time in seconds for completion of a Zimbra (zmprov) command"
