...
def zimbra_execute(config, commands, options):
    """
    ...
    :param options: script options passed by the user
    :return: 0 if success else number of failed commands
    """
    ...
    if commands == "":
        ...
        for action in commands:
            try:
                info(f"Executing {action} (timeout={options.zimbra_timeout} seconds)...")
                zimbra_cmd, _ = action.split(maxsplit=1)
                start_time = time.time()
                zimbra.sendline(action)
                zimbra.expect(ZMPROV_PROMPT, timeout=options.zimbra_timeout)
                ...
                for line in zimbra_output.splitlines():
                    for pattern in zimbra_expected_msgs:
                        ...
                        if line.strip():
                            info(f"{line}")
                info(f"Zimbra command completed in {time.time()-start_time:.2f} seconds")
            except pexpect.TIMEOUT:
                info(
                    "WARNING: unexpected timeout during Zimbra command"
                )
                failed_commands += 1
            except Exception:
...
def load_hito_csv(
    ...
    :param previous: True if the previous file must be read instead of the current one,
                     False otherwise
    :param options: program options passed by the user
    :param team_params: team-related parameters from the configuration file
    :param exclusion_list: lists of users that must not be added to any list as a dictionary
                           where the key is the user and the value is a boolean set in this
                           function to track if the user was found in the input file (for
                           the final report)
    :param optout_list: users who have opted out a list as a dictionary where the key is the list
                        and the value is the user list
...
def set_options(
    ...
    if zimbra_timeout:
        options.zimbra_timeout = zimbra_timeout
    elif "zimbra_timeout" not in options or not options.zimbra_timeout:
        options.zimbra_timeout = ZMPROV_CMD_TIMEOUT
...
def update_lists(options=None):
    ...
    if options.personnel:
        ...
        if options.execute or options.output:
            ...
            if options.execute:
                failed_commands = zimbra_execute(config["zimbra"], zimbra_cmds, options)
                if failed_commands != 0:
                    info(
                        f"WARNING: {failed_commands} Zimbra update commands failed"
...
def main():
    ...
    # Most of the option default values are implemented in set_options()
    # to maintain consistency between executing this scirpt as a command
    # or a module
    ...
    parser.add_argument(
        ...
        type=int,
        help=(
            f"Max wait time in seconds for completion of a Zimbra (zmprov) command"
            f" (D: {ZMPROV_CMD_TIMEOUT})",
        ),
    )
    parser.add_argument(
        "--verbose",
