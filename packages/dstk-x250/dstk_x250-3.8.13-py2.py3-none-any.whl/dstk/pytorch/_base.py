...
class BaseEnvironnement(BaseEstimator):
    ...
    def fit(
        ...
        *,
        ...
        eval_in_main_process: bool=True,
        ...
        eval_in_main_process: bool (=True)
            Exécute la phase d'évaluation dans le process principal dans le
            cadre multi-GPU/multi-node.
        ...
        **Attention pour les TPU** :
        ...
        # marqueur permettant d'éxécuter la partir éval dans le processus
        # principale ou partout (pour le cas multi-GPU/Node
        if eval_in_main_process:
            test_eval_prosses = is_main_process
        else:
            test_eval_prosses = lambda: True

        if not (
            eval_in_main_process
            or eval_dataset is None
            or hasattr(eval_dataset.sampler, 'set_epoch')
        ):
            raise AttributeError(
                "Vous souhaitez réaliser l'éxécution du dataset en parallèle "
                "mais il semblerait que le dataset ne soit pas compatible "
                "pour la parallélisation (voir multi-GPU/multi-Note)."
            )
        ...
        try:
            for n_epoch in range(nb_epoch):
                if is_dist_avail_and_initialized():
                    ...
                    if (
                        eval_dataset
                        and hasattr(eval_dataset.sampler, 'set_epoch')
                    ):
