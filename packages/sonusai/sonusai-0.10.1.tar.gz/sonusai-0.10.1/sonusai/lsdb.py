...
usage: lsdb [-hta] (-d MIXDB) [-i MIXID] [-c TID]
...
def lsdb(mixdb_loc: Location,
    ...
    if len(mixids) == 1:
        ...
        for m in mixids:
            with h5py.File(join(mixdb_loc, mixdb.mixtures[m].name), 'r') as f:
                if m == mixids[0]:
...
def main():
    ...
    logger.info(f'Analyzing {mixdb_loc}')

    lsdb(mixdb_loc=mixdb_loc,
         mixids=mixid,
