...
def _process_mixture(mixid: int) -> None:
    ...
    write_wav(name=mixture_basename + '_mixture.wav', data=mixture)
    if mp_mkwav.write_target:
        write_wav(name=mixture_basename + '_target.wav', data=target)
    if mp_mkwav.write_noise:
        write_wav(name=mixture_basename + '_noise.wav', data=noise)
