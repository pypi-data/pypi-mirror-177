...
The mixture database is written to a JSON file (mixdb.json) in a directory that inherits the base name of the config
file.
...
def genmixdb(file: str = None,
    ...
    from sonusai.mixture import get_mixture_filename
    ...
    fs = get_feature_stats(feature=config['feature'], num_classes=config['num_classes'])
                    ...
                    for augmented_target_index_for_mixup in augmented_target_indices_for_mixup:
                        ...
                        for snr in mixdb.snrs:
                            mixdb.mixtures.append(MRecord(
                                name=get_mixture_filename(len(mixdb.mixtures), mixid_padding),
    ...
    # Fill in the details
    mp_genmixdb.mixdb = mixdb
    mp_genmixdb.raw_target_audios = raw_target_audios
    mp_genmixdb.augmented_noise_audios = augmented_noise_audios
    mp_genmixdb.save_mix = save_mix
    mp_genmixdb.save_ft = save_ft
    mp_genmixdb.save_segsnr = save_segsnr
    mp_genmixdb.output_dir = output_dir
...
def _process_mixture(mixid: int) -> MRecord:
    ...
    mrecord = mp_genmixdb.mixdb.mixtures[mixid]
    ...
    raw_target_audios = [mp_genmixdb.raw_target_audios[mrecord.target_file_index[ti]] for ti in
                         range(len(mrecord.target_file_index))]
    noise_audio = mp_genmixdb.augmented_noise_audios[mrecord.noise_file_index][mrecord.noise_augmentation_index]

    mrecord, audio_data = process_mixture(mixdb=mp_genmixdb.mixdb,
                                          mrecord=mrecord,
                                          raw_target_audios=raw_target_audios,
                                          noise_audio=noise_audio)

    if mp_genmixdb.save_mix or mp_genmixdb.save_ft:
        if mp_genmixdb.output_dir is None:
            raise SonusAIError('must specify output_dir when saving mixture data')

        if not isdir(mp_genmixdb.output_dir):
            raise SonusAIError(f'output_dir, {mp_genmixdb.output_dir}, is not a directory')

        mixture_filename = join(mp_genmixdb.output_dir, mrecord.name)
        mixture_basename = splitext(mixture_filename)[0]

        if mp_genmixdb.save_mix:
            add_audio_data_to_h5(file=mixture_filename, data=audio_data)

        if mp_genmixdb.save_ft:
            feature_data = genft(mixdb=mp_genmixdb.mixdb,
                                 mixids=mixid,
                                 raw_target_audios=mp_genmixdb.raw_target_audios,
                                 augmented_noise_audios=mp_genmixdb.augmented_noise_audios,
                                 mixture=audio_data.mixture,
                                 truth_t=audio_data.truth_t,
                                 compute_segsnr=mp_genmixdb.save_segsnr)

            add_feature_data_to_h5(file=mixture_filename,
                                   data=feature_data,
                                   save_segsnr=mp_genmixdb.save_segsnr)

        with open(file=mixture_basename + '.txt', mode='w') as f:
            f.write(get_mixture_metadata(mp_genmixdb.mixdb, mixid))
...
def main():
    ...
    for config_file in args['CONFIG']:
        ...
        output_dir = get_output_from_config(config, config_file)
        ...
        create_file_handler(join(output_dir, 'genmixdb.log'))
        ...
        json_name = join(output_dir, 'mixdb.json')
