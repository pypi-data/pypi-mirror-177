...
def add_feature_data_to_h5(file: str, data: FeatureData, save_segsnr: bool = False) -> None:
    ...
    with h5py.File(file, 'a') as f:
        ...
        f.create_dataset(name='feature', data=data.feature, dtype=np.single)
        f.create_dataset(name='truth_f', data=data.truth_f, dtype=np.single)

        if save_segsnr:
            if 'segsnr' in f:
                del f['segsnr']
            f.create_dataset(name='segsnr', data=data.segsnr, dtype=np.single)
