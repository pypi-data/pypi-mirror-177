...
def one_hot(truth: np.ndarray,
    ...
    """ Calculates metrics from one-hot prediction and truth data (numpy float arrays) where
        ...
        1. Default = 0 (multiclass or binary) which infers:
           ...
           note multi-label metrics are disabled for predict_thr = 0, must set predict_thr > 0

        2. predict_thr > 0 (multilabel or binary) scalar or a vector num_class x 1 then use
           predict_thr as a binary decision threshold in each class:
           binary (nclass = 1)         use >= predict_thr[0] for pred and predict_thr[num_classes+1] for truth
                                       if it exists, else use >= 0.5 for truth
           multi-label                 use >= predict_thr for pred if scalar, or predict_thr[class_idx] if vector
        ...
        num_classes is inferred from 1D, 2D or 3D truth inputs by default (default timesteps = -1 or 0).
        Only set timesteps > 0 in case of ambiguous binary 2D case where input frames x timesteps,
        then it must set to the number of timesteps = TSTEPS (which will be > 0).
        It is safe to always set timesteps <= 0 for binary inputs, and if truth_shape[2] exists
