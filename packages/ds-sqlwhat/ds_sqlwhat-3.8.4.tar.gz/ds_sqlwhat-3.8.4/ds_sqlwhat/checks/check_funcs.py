...
from russian_protowhat.sct_syntax import link_to_state
from russian_protowhat.checks.check_simple import allow_errors
from ds_sqlwhat.checks import has_no_error, has_result, has_equal_value
...
def check_row(state, index, missing_msg=None, expand_msg=None):
    ...
    if missing_msg is None:
        missing_msg = "Система хочет проверить строку {{index + 1}} результата вашего запроса, но не может ее найти. Посмотрите еще раз."
    if expand_msg is None:
        expand_msg = "Еще раз взгляните на строку {{index + 1}} в результатах запроса."
...
def check_all_columns(state, allow_extra=True, too_many_cols_msg=None, expand_msg=None):
    ...
    if too_many_cols_msg is None:
        too_many_cols_msg = (
            "Результат вашего запроса содержит столбец {{col}}, но не должен."
        )
    if expand_msg is None:
...
def check_query(state, query, error_msg=None, expand_msg=None):
    ...
    if error_msg is None:
        error_msg = "Запуск `{{query}}` после отправки привел к ошибке."
    if expand_msg is None:
        expand_msg = "Автоматическая проверка проверила результат выполнения `{{query}}` в базе данных."
