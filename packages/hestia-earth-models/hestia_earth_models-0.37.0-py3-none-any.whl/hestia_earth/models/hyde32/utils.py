...
def _should_run(impact_assessment: dict, term_id: str, from_site_type: SiteSiteType):
    cycle = impact_assessment.get('cycle', {})
    product = get_product(impact_assessment)
    site = get_site(impact_assessment)
    land_occupation_m2_kg = land_occupation_per_kg(MODEL, term_id, cycle, site, product)
    land_transformation_factor = get_emission_factor(impact_assessment, 20, from_site_type)
...
def run_land_transformation(impact_assessment: dict, term_id: str, from_site_type: SiteSiteType):
    should_run, land_occupation_m2, factor = _should_run(impact_assessment, term_id, from_site_type)
