...
def create_thumbnail_heif(size):
    ...
    im_heif = pi_heif.from_pillow(_)
    im_heif.add_from_pillow(_.crop((0, 0, 256, 256)))
    pi_heif.add_thumbnails(im_heif[0], boxes=[128, 64])
    pi_heif.add_thumbnails(im_heif[1], boxes=[128, 64])
...
def test_heif_thumbnail():
    for i, img in enumerate(pi_heif.open_heif(heif_buf)):
        thumbnail = pi_heif.thumbnail(img)
        if i == 2:
            assert thumbnail.size == img.size
            assert isinstance(thumbnail, pi_heif.HeifImage)
        else:
            assert thumbnail.size != img.size
            assert isinstance(thumbnail, pi_heif.HeifThumbnail)
        assert len(thumbnail.data)
...
def test_pillow_thumbnail_one_image_loaded():
    ...
    thumbnail = pi_heif.thumbnail(im)
...
def test_heif_thumbnail_above_size():
    for img in pi_heif.open_heif(heif_buf):
        thumbnail = pi_heif.thumbnail(img, min_box=9999)
        assert thumbnail.size == img.size
        assert isinstance(thumbnail, pi_heif.HeifImage)
        assert len(thumbnail.data)
...
def test_heif_thumbnail_below_size():
    for i, img in enumerate(pi_heif.open_heif(heif_buf)):
        thumbnail = pi_heif.thumbnail(img, min_box=16)
        if i == 2:
            assert thumbnail.size == img.size
            assert isinstance(thumbnail, pi_heif.HeifImage)
        else:
...
def test_heif_thumbnail_references():
    ...
    img_from_pillow = pi_heif.from_pillow(pillow_img)
...
def test_heif_add_thumbs(thumbs, expected_after, heif_file_buf, method):
    output = BytesIO()
    heif_file = pi_heif.open_heif(heif_file_buf)
    if method == "HeifFile":
        pi_heif.add_thumbnails(heif_file, thumbs)
    else:
        pi_heif.add_thumbnails(heif_file[0], thumbs)
        pi_heif.add_thumbnails(heif_file[1], thumbs)
        pi_heif.add_thumbnails(heif_file[2], thumbs)
...
def test_pillow_add_thumbs(thumbs, expected_after):
    ...
    for frame in ImageSequence.Iterator(im):
        pi_heif.add_thumbnails(frame, thumbs)
...
def test_pillow_add_thumbs_empty_info(thumbs, expected_after):
    ...
    pi_heif.add_thumbnails(im, thumbs)
...
def test_heif_remove_thumbs():
    ...
    out_heif = pi_heif.open_heif(out_buffer)
