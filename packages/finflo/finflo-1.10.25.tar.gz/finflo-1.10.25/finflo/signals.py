...
def creates_finflo_work_models(sender, instance, created , **kwargs):
    ...
    if created:
        ...
        obj2 = workflowitems.objects.create(transitionmanager = obj ,model_type = a.upper() , initial_state = states.description ,interim_state = states.description, final_state = states.description)
        workevents.objects.create(workflowitems = obj2 , type = a.upper() , record_datas = gets_transition.get_record_datas(), initial_state = states.description ,interim_state = states.description,  final_state = states.description)
    pass
