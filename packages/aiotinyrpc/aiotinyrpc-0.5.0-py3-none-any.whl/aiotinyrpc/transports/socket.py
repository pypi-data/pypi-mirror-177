...
from aiotinrypc.auth import AuthProvider
...
class EncryptedSocketServerTransport(ServerTransport):
    ...
    async def handle_client(self, reader, writer):
        ...
        if self.auth_provider:
            ...
            _, challenge_reply = await self.receive_on_socket(peer, reader)
            challenge_reply = self.deserialize(challenge_reply)
            authenticated = self.auth_provider.verify_auth_message(challenge_reply)
            await self.send(
                writer, self.serialize(self.auth_provider.auth_reply_message())
            )
