...
from tdf_tools.tools.gitlab.python_gitlab_api import GitlabAPI
...
class CLIJsonConfig:
    ...
    def saveInConfig(featureBranch: str, shellModule: str, moduleList: list):
        ...
        CLIJsonConfig.notifyModuleConfigFile(moduleList)

    # 更新模块配置文件
    def notifyModuleConfigFile(moduleList: list):
        moduleJson = CLIJsonConfig.getModuleConfig()
        for item in moduleJson:
            if moduleList.__contains__('item'):
                moduleJson[item]['importSource'] = True
            else:
                moduleJson[item]['importSource'] = False
