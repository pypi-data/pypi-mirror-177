...
async def async_callback_threadsafe(client: IColaboFlowClientData, _lambda)->bool:
	...
	def _runLambdaInContext():
		print(f"{client.appNameTextWithContext('_runLambdaInContext')} start, futLambdaExecuted: {futLambdaExecuted}")
		_lambda()
		# futLambdaExecuted.set_result(True)
		loop.call_soon_threadsafe(futLambdaExecuted.set_result, True)
		print(f"{client.appNameTextWithContext('_runLambdaInContext')} end, futLambdaExecuted: {futLambdaExecuted}")

	print(f"{client.appNameTextWithContext('async_callback_threadsafe')} before add_callback_threadsafe, futLambdaExecuted: {futLambdaExecuted}")
	client.connection.add_callback_threadsafe(lambda: _runLambdaInContext())
	print(f"{client.appNameTextWithContext('async_callback_threadsafe')} awaiting futLambdaExecuted: {futLambdaExecuted}")
	await futLambdaExecuted
	print(f"{client.appNameTextWithContext('async_callback_threadsafe')} after add_callback_threadsafe, futLambdaExecuted: {futLambdaExecuted}")
...
async def client_executeTask(client: IColaboFlowClientData, taskId: str, taskInput: Any):
	...
	await async_callback_threadsafe(client, lambda: client.channel.queue_declare(queue=taskExecuteResponseQueue, exclusive=True))
	...
	await async_callback_threadsafe(client, lambda: client.channel.basic_publish(exchange='', routing_key=client.queueServiceOrchestrator, body=msgTaskExecuteReqStr))
