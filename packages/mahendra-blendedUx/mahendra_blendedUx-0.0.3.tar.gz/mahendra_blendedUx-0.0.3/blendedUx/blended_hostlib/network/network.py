...
from blendedUx.blended_hostlib.swagger_client.apis.packages_api import PackagesApi
from blendedUx.blended_hostlib.swagger_client.apis.accounts_api import AccountsApi
from blendedUx.blended_hostlib.swagger_client.rest import ApiException
...
sys.path.append('../blendedcli')
from blendedUx.blendedcli.settings import BLENDED_DATA_DIR 
...
class Network(object):
    ...
    def remove_account(self, slug, account_slug,):
        ...
        try:
            ...
            try:
                error_dict = json.loads(exc.body)
                error = "Error : Revoke Failed\nOnly organization can revoke to an Account and Email"
            except ValueError:
                raise BlendedException("Something went wrong on the Server")
            else:
    ...
    def acquire_package(self, account_slug, pk, body):
        ...
        try:
            ...
            try:
                ...
                error_dict = json.loads(exc.body)
                if 'errors' in error_dict and any('license_name' in elem for elem in error_dict['errors']):
                    error = 'Error : Package Acquisition Failed.\nlicense is required to be passed as flag'
                    raise BlendedException(error)
            except ValueError:
                raise BlendedException("Something went wrong on the Server")
            else:
