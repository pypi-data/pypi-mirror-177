...
//                                                                           //
/////////////////////////////////////////////////////////////////////////////*/
R1::Event* newR1DummyEvent()
{
    R1::Event* event = new R1::Event;
    ...
    uint8* pix_status    = reallocAs<uint8>(event->mutable_pixel_status(),                     ARRAYS_SIZE);
    uint8* first_cell_id = reallocAs<uint8>(event->mutable_first_cell_id(),                    ARRAYS_SIZE);
    uint8* clk_counter   = reallocAs<uint8>(event->mutable_module_hires_local_clock_counter(), ARRAYS_SIZE);
...
//                                                                           //
/////////////////////////////////////////////////////////////////////////////*/
void verifyR1EventData(const R1::Event* event)
{
    ...
    const uint8* pix_status    = readAs<uint8>(event->pixel_status());
    const uint8* first_cell_id = readAs<uint8>(event->first_cell_id());
    const uint8* clk_counter   = readAs<uint8>(event->module_hires_local_clock_counter());
    ...
    for (uint32 i=0;i<ARRAYS_SIZE;i++)
    {
        ...
        if (pix_status[i]    != (uint8)(INTEGER_VALUE+i+1)) throw runtime_error("Wrong pixel_status");
        if (first_cell_id[i] != (uint8)(INTEGER_VALUE+i+2)) throw runtime_error("Wrong first_cell_id");
        if (clk_counter[i]   != (uint8)(INTEGER_VALUE+i+3)) throw runtime_error("Wrong module_hires_local_clock_counter");
...
//                                                                           //
/////////////////////////////////////////////////////////////////////////////*/
R1::CameraConfiguration* newCameraConfiguration()
{
    R1::CameraConfiguration* config = new R1::CameraConfiguration;

    config->set_tel_id(          INTEGER_VALUE+0);
    config->set_local_run_id(    INTEGER_VALUE+1);
    config->set_config_time_s(   INTEGER_VALUE+2);
    config->set_config_time_qns( INTEGER_VALUE+3);
...
//                                                                           //
/////////////////////////////////////////////////////////////////////////////*/
void verifyR1CameraConfiguration(const R1::CameraConfiguration* config)
{
    ...
    if (config->config_time_qns()  != INTEGER_VALUE+3) throw runtime_error("Wrong config_time_qns");
...
int main(int argc, char** argv)
{
    ...
    R1::CameraConfiguration* config = input.readTypedMessage<R1::CameraConfiguration>(1);
    ...
    for (uint32 i=1;i<=input2.getNumMessagesInTable();i++)
    {
        ...
        R1::Event* event = input2.readTypedMessage<R1::Event>(i);
