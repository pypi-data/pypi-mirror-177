"""
Copyright (C) 2019-2020, 2022 Kunal Mehta <legoktm@debian.org>
...
def parse_args(groups: list, real_args=sys.argv[1:]):
    ...
    for group in groups:
        exclusive.add_argument(f'--only-{group}',
                               help=f'Only download {group} torrents',
                               action='store_true')
    for group in groups:
    ...
    enabled = []
    for group in groups:
        if args.__getattribute__(f'only_{group}'):
            enabled.append(group)

    if not enabled:
        # No --only flag was used
        for group in groups:
