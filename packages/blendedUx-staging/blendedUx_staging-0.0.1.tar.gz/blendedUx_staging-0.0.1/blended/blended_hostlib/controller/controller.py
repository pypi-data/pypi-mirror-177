...
from blended.blended_hostlib.utils import contextErrors
...
from blended.blendedcli.settings import BLENDED_DIR as theme_path
...
class Controller(object):
    ...
    def package_version(self, current_account, package_name, **kwargs):
        ...
        if canonical:
            try:
                ...
                date = datetime.fromisoformat(info.to_dict()['created_date'].split('.')[0]).strftime("%b %d %Y %H:%M:%S")
        ...
        try:
            ...
            for item in package_version_items:
                version_list.append({'canonical':item.canonical, 'is_set_canonical': item.is_set_canonical,
                                     'version': item.label, 'date': datetime.fromisoformat(item.created_date.split('.')[0]).strftime("%b %d %Y %H:%M:%S")})
        ...
        version_list.append({'canonical':0, 'is_set_canonical': 0, 
                             'version': 'draft', 'date': datetime.fromisoformat(pack_detail.created_date.split('.')[0]).strftime("%b %d %Y %H:%M:%S")})
        return version_list
    ...
    def get_package(self, package_name=None, format=None, version=None, trim=True, dependency=False, action=None, download=True):
        ...
        try:
            ...
            if not download:
                raise BlendedException({'message': "Package does not exist.", 'status_code': 9000})
                
            try:
                if version == 'draft':
                    hub_jptf = self.network.download_draft(account, package_slug)
                    hub_jptf, package_hash, canonical_type = self._remove_root_from_jptf(hub_jptf)
                    ...
                    self.save_local(
                            package_slug, intermediary_object=hub_package,
                            account=account, current_account=current_account,
                            version=version, dependency=dependency
                            )

                    package = self.backend.get_package(package_name, dependency=dependency, version=version, action=action)
                    self.backend.set_canonical(account, package_slug, version=canonical_type)
                else:
    ...
    def update_meta_location(self,location):
        ...
        if location[1] == 'src':
            ...
            location =  location[3] + '/' + '/'.join(location[5:])
            location = '/'.join(['_package.json','dependencies', location])
        return location
    
    def intermediary_to_context(self, package, obj_to_return=None,errors_context_building=None):
        """
        :param obj_to_return:
        :return:
        """
        ...
        for item in package:
            ...
            if isinstance(value.content, list):
                obj_to_return[key] = {}
                obj_to_return.update({key: self.intermediary_to_context(value, obj_to_return[key],errors_context_building=errors_context_building)})
                meta_value = {
                    'hash': item.hash,
                    'type':'directory',
                    'location': self.update_meta_location(item.location)
                }
                obj_to_return.update({'_meta_' + key : meta_value})
                ...
                obj_to_return.update({'_meta_'+key : image_dict_object.copy() })
            else:
                extension = key.split('.')[-1]
                if key.endswith('.json'):
                    obj_to_return['_meta_'+key] = {
                        'hash': item.hash,
                        'type': MIME_TYPE.get(extension,'undefined'),
                        'location': item.location,
                    }
                    try : 
                       value = json.loads(value.content)
                    except Exception as error:
                        absolutePath = item.location
                        meta_location = self.update_meta_location(item.location)
                        errorValidation = contextErrors.jsonParseError(absolutePath,meta_location)
                        value = value.content
                        if errors_context_building:
                            errors_context_building["syntax_errors"].extend(errorValidation)
                    if (key == '_index.json' or key == '_package.json'):
                        obj_to_return['_meta_'+key]['content'] = value                    
                else:
                    value = value.content
                    obj_to_return['_meta_'+key] = {
                        'hash': item.hash,
    ...
    def as_context(self, project, trim=True, dependency=False, errors_context_building=None):
        """
        ...
        """
        dependency_dict = {}    
        if not errors_context_building:
            errors_context_building = {
                "syntax_errors" : [],
                "null_undefined_pointer_error": []
            }
                
        package = self.intermediary_to_context(project, errors_context_building=errors_context_building)
        if len(errors_context_building['syntax_errors'])<=0:             
            ...
            self.resolve(None, package, dependency_dict, None, None, errors_context_building)  
            if trim:
    ...
    def trim_context(self, current_node):
        ...
        if isinstance(current_node_iter, dict) and current_node_iter:
            for key, value in current_node_iter.items():
                if key == '_index' and isinstance(value, dict):
                    ...
                    if key.startswith("_") and key.find('_meta_') != 0 :
    ...
    def resolve(self, current_node, project_object, dependency_dict, current_json_file, ref_path,errors_context_building):
        """
        ...
        def is_dict(var): return isinstance(var, (dict))
        def is_list(var): return isinstance(var, (list))
            ...
            for item in current_node:
                if item:
                    self.resolve(item, project_object, dependency_dict,
                                 current_json_file, ref_path,errors_context_building)
        elif is_dict(current_node):
            rand_list = current_node.copy().items()
            
            for key, value in rand_list:
                if key.find('_meta_') == 0 : 
                    continue
                ref_path.append(key)
                if key[-5:] == '.json':
                ...
                if is_dict(current_value):
                    if '$ref' in value:
                        ...
                        current_node[key] = self.resolve_pointer(current_value,
                                                                 ...
                                                                 ref_location_ptr, key, None,errors_context_building)
    ...
    def resolve_pointer(self, json_ref, project_object, dependency_dict,
        ...
        if (is_array(json_ref) and '$ref' in json_ref):
            ...
            if not pointer:           
                error = contextErrors.nullUndefinedError(ref_path,project_object)  
                error["message"] = "$ref value cannot be empty"
                errors_context_building['null_undefined_pointer_error'].append(error)
                return 
            if pointer in visited_set:
                ...
                json_ref = self.ref_get(pointer, project_object, current_json_file, dependency_dict, visited_set,
                                        ref_path, ref_location, ref_loc_key, curr_alias,errors_context_building)
                # if (not json_ref):
    ...
    def ref_get(self, ref_value, project_object, json_file, dependency_dict,
        ...
        """
        # if ref_path == ['meta','config.json','button','shape','active']:
        #     print("yes")
        #     import pdb; pdb.set_trace()
        preserved_ref_path = ref_path.copy()
        dicti_replace = {'#/': ''}
        dicti_replace1 = {'#': ''}
        dicti_replace2 = {'#/': '/'}
        if ref_value.find('@') == 0:
            ...
            ref_value = self.get_value_at_path(dependency_dict, json_file, project_object, resolve_path, visited_set,
                                               ref_location, ref_loc_key, '@',preserved_ref_path, errors_context_building)
            return ref_value
            ...
            if (curr_alias and curr_alias == '@'):
                ...
                ref_alias = '#'
            ref_value = self.get_value_at_path(dependency_dict, json_file, project_object, resolve_path, visited_set,
                                               ref_location, ref_loc_key, ref_alias,preserved_ref_path, errors_context_building)
            return ref_value
        else:
    ...
    def get_value_at_path(self, dependency_dict, json_file, project_object, resolve_path,
                          visited_set, ref_location, ref_loc_key, ref_identifier,preserved_ref_path, errors_context_building):
        """
        ...
        for value in resolve_path:
            ...
            try:
                if isinstance(temp_value,dict) and '$ref' in temp_value:
                ...
                if temp_value == 'undefined' and ref_identifier in ['/', '#','@']:
                    errorValidation = contextErrors.nullUndefinedError(preserved_ref_path,project_object)
                    errors_context_building['null_undefined_pointer_error'].append(errorValidation)
                    break 
            except KeyError:
            ...
            if '$ref' in temp_value:
                if (temp_value['$ref'].find("#/") == 0):
                    ...
                    temp_value = self.resolve_pointer(temp_value, project_object, dependency_dict, visited_set,
                                                      ...
                                                      ref_identifier, errors_context_building)
                else:
                    temp_value = self.resolve_pointer(temp_value, project_object, dependency_dict, visited_set,
                ...
                temp_value = self.clear_dep(None, project_object, dependency_dict, current_json_file, resolve_path,
                                            None, ref_identifier, True,errors_context_building)
        return temp_value

    def clear_dep(self, current_node, project_object, dependency_dict, current_json_file,
                  resolve_path, ref_path, curr_alias, path_flag=False,errors_context_building={}):
        '''
            ...
            if is_array(current_value):
                if '$ref' in value:
                    ...
                    current_node[key] = self.resolve_pointer(current_value,
                                                             ...
                                                             key, None,errors_context_building)
                    current_node['_meta_'+key]= {
                        'type': '$ref',
                        'location': '/'.join(ref_path),
                        'content': value
                    ...
                    self.clear_dep(value, project_object, dependency_dict,
                                   current_json_file, None, ref_path, None,False,errors_context_building)
    ...
    def get_resolved_path(self,object, curIdx, pathList):
        if curIdx == len(pathList)-1 : 
            return object['_meta_'+pathList[curIdx]]['location']
        return self.get_resolved_path(object[pathList[curIdx]], curIdx+1, pathList)

    def pre_validation_check(self, account_slug, slug, errors_context_building, label="draft"):
        ...
        theme = self.as_context(inter,errors_context_building=errors_context_building) # getting as context
    ...
    def package_validate(self, account_slug, slug, label="draft"):
        ...
        result = self.pre_validation_check(account_slug, slug, errors_context_building,label)
        ...
        results = {}        
        requireSanityCheckCode = '''
            const theme = %s;
            const sanityCheckFile = %s; 
            function requireSanityCheck() {
            var code = `
            (function a(){
                var module = { exports: {} };
                var exports = module.exports;
                var global = {};                
                ` +  sanityCheckFile['content'] + ` 
                return module.exports;})()`;
            return eval(code);           
            };
            
            const vengineSanityCheck = requireSanityCheck().vengineSanityCheck;
            function validateSanityCheck(theme) {
                const finalErrorList = vengineSanityCheck(theme);
                return finalErrorList
            };        
        '''%(themeInString, contextErrors.sanityCheckFile)
        
        # pre-validation check -> 1. json_parse_error 2. null_undefined_pointer_error
        error_validated = True
        for error_type in ['syntax_errors','null_undefined_pointer_error']: 
            if len(errors_context_building[error_type])>0: 
                results[error_type] = {}
                results[error_type]['errors'] = errors_context_building[error_type]
                error_validated = False
                break 
        
        # 3. vengineSanityCheck 
        if error_validated: 
            if os_name.startswith('linux') or os_name.startswith('darwin'):
                from py_mini_racer import py_mini_racer
                ...
                import execjs
                ctx2 = execjs.get()
            code = '(function(){ %s return validateSanityCheck(theme)})()'%(requireSanityCheckCode)
            errors = ctx2.eval(code)
            if len(errors)>0: 
                # update location format
                for i in range(len(errors)):
                    errors[i]['location']['templatePath'] = self.get_resolved_path(theme, 0,errors[i]['location']['templatePath'].split('.')[1:])
                results[error_type] = {}
                results[error_type]['errors'] = errors
                error_validated = False
        ...
        if error_validated: 
            ...
            for i in theme['validation']['src'] :
                if i.startswith("_meta_"): 
                    continue
                try:
                    code = '(function(theme){' + theme['validation']['src'][i] + 'return validate(theme)})(theme)'
                    results[i] = ctx.eval(code)
                except Exception as exc:
