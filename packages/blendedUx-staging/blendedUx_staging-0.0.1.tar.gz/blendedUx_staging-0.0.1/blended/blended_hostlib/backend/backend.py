...
class FileSystemBackend(object):
            ...
            if isinstance(data, BinaryFile):
                if (with_extension and extension.lower() in ALLOWED_IMAGE_TYPES):
                    ...
                    if isinstance(media_for_download, dict) and (directory_path not in media_for_download):
                        ...
                        image_response = urllib3.PoolManager().request('GET',value, timeout=40)
                        image_obj = image_response.data
                        # print(image_response)
                        # image_response = urlopen(value, timeout=40)  # urllib.request.urlopen doesn't have a inbuilt logger so we are using urllib3 instead
    ...
    def download_and_save_media(self, url_path_tuple):
        ...
        response = urllib3.PoolManager().request('GET',url,timeout=40)
        image_obj = response.data
        # response = urlopen(url,timeout=40) # urllib.request.urlopen doesn't have a inbuilt logger so we are using urllib3 instead
