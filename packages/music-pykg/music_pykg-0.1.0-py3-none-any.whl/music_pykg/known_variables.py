...
class KnownMusicVariables:
    ...
    def _vars(self) -> Sequence[Variable]:
        return [
            Variable(name="rho", legacy_name="rho", node=NODE_CENTER),
            Variable(name="e_int", legacy_name="e", node=NODE_CENTER),
            # --
            ...
            # --
            Variable(name="b_1", legacy_name="b_r", node=NODE_FACE_1),
            Variable(name="b_2", legacy_name="b_t", node=NODE_FACE_2),
            Variable(name="b_3", legacy_name="b_p", node=NODE_FACE_3),
            # --
            *[
                Variable(
                    name=f"scalar_{i}",
                ...
                for i in range(1, self.max_scalars + 1)
            ],
        ]
    ...
    def __getitem__(self, name: str) -> Variable:
        ...
        try:
            ...
            raise KeyError(
                f"Variable with name '{name}' unknown to KnownMusicVariables"
            )
    ...
    def legacy_to_new_names(self) -> Mapping[str, str]:
        """Return a mapping of legacy variable names to new names."""
        return MappingProxyType({v.legacy_name: v.name for v in self._vars})
