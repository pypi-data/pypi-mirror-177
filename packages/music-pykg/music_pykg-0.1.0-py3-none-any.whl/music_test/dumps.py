...
class MusicDump2(FileDump):
    ...
    filename: Union[str, PathLike, RelativePath]
    ndim: int
    # TYPE SAFETY: nvel is set to an integer in post_init
    nvel: int = None  # type: ignore
    nscalars: int = 0

    def __post_init__(self) -> None:
        if self.nvel is None:
            object.__setattr__(self, "nvel", self.ndim)
...
class ConcreteDump2(ConcreteFileDump):
    ...
    def header_and_data(self) -> Tuple[Dict[str, Any], Dict[str, np.ndarray]]:
        ...
        m_to_pym = music_vars.legacy_to_new_names()
        hdr, data = MusicNewFormatDumpFile(
            self.full_filename,
            MusicDumpInfo(
                num_space_dims=self.fdump.ndim,
                num_velocities=self.fdump.nvel,
                num_scalars=self.fdump.nscalars,
            ),
        ).read()
        data = {m_to_pym[name]: vals for name, vals in data.items()}
        return hdr, data
