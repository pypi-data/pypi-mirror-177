...
class CircularAlfvenWave2D:
    ...
    def _state_t0(self, x: np.float64, y: np.float64) -> Dict:
        ...
        return {
            "rho": self.rho0,
            "e_int": self.press0 / self.rho0 / (self.gamma - 1.0),
            "vel_1": vx,
            "vel_2": vy,
            "vel_3": vz,
            "b_1": bx,
...
def make_circular_alfven_2d_test_config(
    ...
    output_dump = MusicDump2(filename=LastFileNameInGlob("output/*.music"), ndim=2)
    ...
    return Test(
        ...
        tags=["long"] + (["scaled_up"] if ny > 64 else []),
        build_preset=Preset("2.5dim"),
        run=MusicRun(
            namelist="params.nml",
            start_dump=MusicDump2("alfven_wave.music", ndim=2),
