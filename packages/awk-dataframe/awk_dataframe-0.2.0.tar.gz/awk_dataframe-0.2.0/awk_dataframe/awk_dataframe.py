            ...
            if command == self.__commands__[0]:
                if command.type == "to_csv":
                    if len(self.__commands__) == 1:
                        ...
                        if not os.path.exists(os.path.dirname(__file__) + "/parse_complex_csv.exe"):
                            command_compile = "g++ -o " + os.path.dirname(__file__) + "/parse_complex_csv.exe " + os.path.dirname(__file__) + "/read_file_direct_output.cpp"
                            debug("Compilation command", command_compile)
                            subprocess.check_output(command_compile, shell=True, executable='/bin/bash')
                        complete_command = "cp " + self.__path__ + " " + command.options["path_output"] + " && " +  os.path.dirname(__file__) + "/parse_complex_csv.exe " + self.__path__
                    ...
                    #     command.options["line_based_code"] = "0"
                        ...
                        awk_command_to_file(command)
                        if command.options["number_files_input"] == "1":
                            complete_command = "gawk -M -f " + command.path_execution_file + " <(" + os.path.dirname(__file__) + "/parse_complex_csv.exe " + self.__path__ + ")"
                        else:
                            complete_command = "gawk -M -f " + command.path_execution_file + " " + command.path_temp_file + " <(" + os.path.dirname(__file__) + "/parse_complex_csv.exe " + self.__path__ + ")"
                ...
                if command.type == "to_csv":
                    if not os.path.exists(os.path.dirname(__file__) + "/save_csv.exe"):
                        command_compile = "g++ -o " + os.path.dirname(__file__) + "/save_csv.exe " + os.path.dirname(__file__) + "/write_to_file.cpp"
                        debug("Compilation command", command_compile)
                        subprocess.check_output(command_compile, shell=True, executable='/bin/bash')
                    
                           
                    
                    complete_command = os.path.dirname(__file__) + "/save_csv.exe " + command.options["path_output"] + " <(" + complete_command + ")"             
                    ...
                    if (self.__commands__[index - 1].options["is_last_command"] == "1"):
                        if int(self.__commands__[index - 1].options["level_simplicity_csv"]) > 0:
                            ...
                            awk_command_to_file(command)                
                            if command.options["number_files_input"] == "1":
                                complete_command = "gawk -M -f " + command.path_execution_file + " <(" + os.path.dirname(__file__) + "/parse_complex_csv.exe" + " <(" + complete_command + "))"             
                            else:
                                complete_command = "gawk -M  -f " + command.path_execution_file + " " + command.path_temp_file + " <(" + os.path.dirname(__file__) + "/parse_complex_csv.exe" + " <(" + complete_command + "))"           
                    else:
                        debug("Creating comand",command.type,command.options)
                        awk_command_to_file(command)                
                        if command.options["number_files_input"] == "1":
