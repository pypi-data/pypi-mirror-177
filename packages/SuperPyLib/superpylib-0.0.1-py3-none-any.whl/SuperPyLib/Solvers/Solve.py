...
def getDistributedSources(shapes, mesh, solver, tn):
    ...
    for node in tNodes:
        ...
        # miniTriNodes = np.array([])
        # for row in miniTri:
        #     # print(row)
        #     miniTriNodes = np.append(miniTriNodes, row)
        #     # miniTriNodes = np.append(miniTriNodes, row)
        #     # miniTriNodes = np.append(miniTriNodes, row)

        # miniTriNodes = np.unique(miniTriNodes)
        # miniTriNodes = miniTriNodes.astype(int)
        # miniTriNodes = np.sort(miniTriNodes)
        # print(miniTriNodes)
        ...
        # miniA = miniA.tolist()
        # minib = minib.tolist()
        # miniA = cp.array(miniA)
        # minib = cp.array(minib)
        # solver.solveminiCupy(miniA, minib)
        solver.solveminiNumpy(miniA, minib)
        # solver.solveminiScipy(miniA, minib)
        # solver.solveminiSadiku(miniA, minib+1, minib, 0.001)
        miniu = solver.miniu
        solver.getMiniLinNumericalMatrix(minix, miniy, miniTri)
        miniAn = solver.miniAn
        # print(type(miniAn), type(miniu))
        # print(miniu)
        # miniu = miniu.tolist()
        # miniu = np.array(miniu)
    ...
    mesh.nodeSourceDss = bcopy.copy()*ep0
        # count = count +1
        # print("Percentage of truncated nodes covered", count/len(tNodes)*100, end='\r')
