...
class CollectYumPlugin(SerialCollectorPlugin):
    ...
    def collect(self, comp: Any, repo_url: str, creds=None):
        ...
        self.get_logger().info(
            msg=f"Copying yum package from {comp.purl}", indent_level=2
        )
        ...
        if not found_url.startswith(repo_url):
            ...
            self.get_logger().error(
                msg=f"Yum download url:      {found_url}", indent_level=3
            )
            self.get_logger().error(
                msg=f"Expected url starting: {repo_url}", indent_level=3
            )
