...
static PyObject* BTAddBenchFunction(PyObject* self, PyObject* args){
    ...
    if (PyCallable_Check(f)){
        ...
        Py_INCREF(f);
        if(s){
            fs = calloc(sizeof(char), strlen(s));
            strcpy(fs, s);
        }
        addBenchmarkFunction(f, fs, i);
        Py_INCREF(Py_None);
        return Py_None;
    }
...
static PyMethodDef BenchToolsMethods[] = {
    {"addToBench", BTAddBenchFunction, METH_VARARGS, "Add a callable to run in the benchmark round"},
    {"benchRound", BTBenchmarkFunctions, METH_VARARGS, "Run the benchmark round"},
    {"clearRound", BTFreeFunctionTable, METH_NOARGS, "Clear the benchmark round"},
    {NULL}
};
