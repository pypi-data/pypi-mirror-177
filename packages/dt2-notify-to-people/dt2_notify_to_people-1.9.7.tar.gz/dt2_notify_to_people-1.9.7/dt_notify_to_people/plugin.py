...
class DingTalkNotifyPlugin(CorePluginMixin, notify.NotificationPlugin):
    ...
    def get_config(self, project, **kwargs):
        return [
            ...
            },
            {
                ...
                "required": False,
                "default": self.set_default(
                    project, "number_alarm", "DINGTALK_NUMBER_ALARM"
                ),
            },
            {
                "name": "phones",
    ...
    def _post(self, group, project):
        ...
        # 告警次数阀值
        number_alarm = self.get_option("number_alarm", project) if self.get_option("number_alarm", project) else 1
        ...
        payload = f"{payload} > 查看详情: <font color=blue>[{'Error: ' + group.message[0:50]}]({issue_link})</font> \n\n"
        # payload = f"{payload} > 具体报错: {group.message} \n\n"
        # payload = f"{payload} > 告警次数: {number_alarm} \n\n"
        # payload = f"{payload} > Hi: {event.__dict__} \n\n"
        # payload = f"{payload} > Hi: {event.data} \n\n"
        ...
        # 0 - 状态为未解决，并且次数大于设定值，才告警通知，并且分配为空
        # if group.status == 0 and group.times_seen > number_alarm and not bool(event.data):
        if group.status == 0 and group.times_seen >= number_alarm:
