...
class KBSession(requests.Session):
...
class KBClient:
    """A binding to Blackduck's KB REST API that provides a robust connection backed by a session object.
    ...
    def __init__(self,
                 ...
                 retries=3):
        """Instantiate a KBClient for use with Black Duck KB's REST-API

        Args:
            license_key (str): Black Duck KB license key
            base_url (str): e.g. "https://your.blackduck.url"
            session (requests.Session): custom session if specified.  For advanced users only.
                If not provided, a KBSession with recommended defaults will be generated and used.
                Any custom session must incorporate a base_url in every request as a plain
                requests.Session() will not work.  See KBSession implementation for an example.
            auth (requests.auth.AuthBase): custom authorization if specified. For advanced users only.
                If not provided, one based on the license key is generated and used.
        ...
        """
        self.base_url = base_url
        self.session = session or KBSession(base_url, timeout, retries, verify)
        self.session.auth = auth or BearerAuth(self.session, license_key)
