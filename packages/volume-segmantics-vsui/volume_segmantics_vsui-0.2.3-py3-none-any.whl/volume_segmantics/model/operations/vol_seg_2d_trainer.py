...
class VolSeg2dTrainer:
    ...
    def output_running_prediction_figure(self, title: str) -> None:
        ...
        for i in range(0,2):
            my_base64_jpgData = encode_image(labels[i].cpu(), title=title)
    ...
    def output_comparison_figures(self) -> None:
        ...
        for i in range(0,2):
            my_base64_jpgData = encode_image(inputs[i].squeeze().cpu(), title="Data")
            vsui_client.edit_element('Sample Images', {'reference': {'slice': i, 'img': my_base64_jpgData}})

            my_base64_jpgData = encode_image(torch.argmax(targets[i], dim=0).cpu(), title="Ground Truth")
            vsui_client.edit_element('Sample Images', {'reference': {'slice': i, 'img': my_base64_jpgData}})
