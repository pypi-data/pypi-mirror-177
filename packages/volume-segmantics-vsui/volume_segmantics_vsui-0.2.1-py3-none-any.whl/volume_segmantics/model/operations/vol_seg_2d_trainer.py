...
from volume_segmantics.utilities.img_encode import encode_image
...
class VolSeg2dTrainer:
    ...
    def output_running_prediction_figure(self, title: str) -> None:
        self.model.eval()  # prep model for evaluation
        batch = next(iter(self.validation_loader))  # Get first batch
        with torch.no_grad():
            inputs, targets = utils.prepare_training_batch(
        ...
        my_base64_jpgData = encode_image(labels[0].cpu(), title=title)
        vsui_client.edit_element('Sample Images', {'data': {'slice': 0, 'img': my_base64_jpgData}})
    ...
    def output_comparison_figures(self) -> None:
        ...
        my_base64_jpgData = encode_image(inputs[0].squeeze().cpu(), title="Data")
        vsui_client.edit_element('Sample Images', {'reference': {'slice': 0, 'img': my_base64_jpgData}})

        my_base64_jpgData = encode_image(torch.argmax(targets[0], dim=0).cpu(), title="Ground Truth")
        vsui_client.edit_element('Sample Images', {'reference': {'slice': 0, 'img': my_base64_jpgData}})