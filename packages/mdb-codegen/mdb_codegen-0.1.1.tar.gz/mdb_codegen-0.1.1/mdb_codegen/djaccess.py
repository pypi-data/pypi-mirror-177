...
from mdbtools import AccessColumn, AccessDatabase, AccessTable
...
class AccessTableToApiPostEntry(IOWriter):
    ...
    def __call__(self, table: AccessTable) -> None:
        ...
        decorator = f'api.post("/{self.table_namer.api(table)}", response={{HTTPStatus.CREATED: schema.{self.table_namer.model_schema(table)}, HTTPStatus.OK: schema.{self.table_namer.model_schema(table)}, HTTPStatus.BAD_REQUEST: schema.Error}})'
        def_name = (
