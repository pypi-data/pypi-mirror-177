...
class SentCode(Object):
    ...
    Parameters:
        type (:obj:`~pyroxd.enums.SentCodeType`):
            Type of the current sent code.

        phone_code_hash (``str``):
            Confirmation code identifier useful for the next authorization steps (either
            :meth:`~pyroxd.Client.sign_in` or :meth:`~pyroxd.Client.sign_up`).

        next_type (:obj:`~pyroxd.enums.NextCodeType`, *optional*):
            Type of the next code to be sent with :meth:`~pyroxd.Client.resend_code`.

        timeout (``int``, *optional*):
            Delay in seconds before calling :meth:`~pyroxd.Client.resend_code`.
    """
