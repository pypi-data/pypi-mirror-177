...
DESTINATION_PATH = Path("pyroxd/raw")
NOTICE_PATH = "NOTICE"
...
def get_docstring_arg_type(t: str, is_list: bool = False, is_pyrogram_type: bool = False):
    if t in CORE_TYPES:
        ...
        return "Any object from :obj:`~pyroxd.raw.types`"
    elif t == "!X":
        return "Any method from :obj:`~pyroxd.raw.functions`"
    elif t.lower().startswith("vector"):
        return "List of " + get_docstring_arg_type(t.split("<", 1)[1][:-1], True)
    else:
        return f":obj:`{t} <pyroxd.raw.base.{t}>`"
...
def get_references(t: str, kind: str):
    ...
    if t:
        return "\n            ".join(
            f"- :obj:`{i} <pyroxd.raw.functions.{i}>`"
            for i in t
        ), len(t)
...
def start(format: bool = False):
    ...
    for qualtype in types_to_constructors:
        ...
        items = "\n            ".join([f"- :obj:`{c} <pyroxd.raw.types.{c}>`" for c in constructors])
        ...
        for i, arg in enumerate(sorted_args):
            ...
            docstring_args.append(
                "{}{}: {}".format(
                    ...
                    get_docstring_arg_type(arg_type, is_pyrogram_type=c.namespace == "pyroxd")
                )
            )

        if c.section == "types":
            docstring += f"This object is a constructor of the base type :obj:`~pyroxd.raw.base.{c.qualtype}`.\n\n"
        else:
    ...
    with open(DESTINATION_PATH / "all.py", "w", encoding="utf-8") as f:
        ...
        for c in combinators:
            f.write(f'\n    {c.id}: "pyroxd.raw.{c.section}.{c.qualname}",')

        f.write('\n    0xbc799737: "pyroxd.raw.core.BoolFalse",')
        f.write('\n    0x997275b5: "pyroxd.raw.core.BoolTrue",')
        f.write('\n    0x1cb5c415: "pyroxd.raw.core.Vector",')
        f.write('\n    0x73f1f8dc: "pyroxd.raw.core.MsgContainer",')
        f.write('\n    0xae500895: "pyroxd.raw.core.FutureSalts",')
        f.write('\n    0x0949d9dc: "pyroxd.raw.core.FutureSalt",')
        f.write('\n    0x3072cfa1: "pyroxd.raw.core.GzipPacked",')
        f.write('\n    0x5bb8e511: "pyroxd.raw.core.Message",')
