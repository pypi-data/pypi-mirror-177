...
class Dispatcher:
    ...
    def __init__(self, client: "pyroxd.Client"):
        self.client = client
        ...
        async def message_parser(update, users, chats):
            return (
                await pyroxd.types.Message._parse(self.client, update.message, users, chats,
        ...
        async def callback_query_parser(update, users, chats):
            return (
                await pyroxd.types.CallbackQuery._parse(self.client, update, users),
                CallbackQueryHandler
            )

        async def user_status_parser(update, users, chats):
            return (
                pyroxd.types.User._parse_user_status(self.client, update),
                UserStatusHandler
            )

        async def inline_query_parser(update, users, chats):
            return (
                pyroxd.types.InlineQuery._parse(self.client, update, users),
                InlineQueryHandler
            )

        async def poll_parser(update, users, chats):
            return (
                pyroxd.types.Poll._parse_update(self.client, update),
                PollHandler
            )

        async def chosen_inline_result_parser(update, users, chats):
            return (
                pyroxd.types.ChosenInlineResult._parse(self.client, update, users),
                ChosenInlineResultHandler
            )

        async def chat_member_updated_parser(update, users, chats):
            return (
                pyroxd.types.ChatMemberUpdated._parse(self.client, update, users, chats),
                ChatMemberUpdatedHandler
            )

        async def chat_join_request_parser(update, users, chats):
            return (
                pyroxd.types.ChatJoinRequest._parse(self.client, update, users, chats),
                    ...
                    for group in self.groups.values():
                        for handler in group:
                            ...
                            try:
                                ...
                                raise
                            except pyroxd.ContinuePropagation:
                                continue
                            except Exception as e:
                                log.error(e, exc_info=True)
